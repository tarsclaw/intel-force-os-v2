...
| QuickBooks consent → **"Intel Force OS didn't connect"**; "View error details" says **"redirect_uri … is invalid … list it in the Redirect URIs section on your app's keys tab"** | The redirect URI is **not registered under the key set in use** (e.g. you added it under Production but `QB_CLIENT_ID` is a Development key, or it isn't added at all) | Add `http://localhost:3100/callback` under the **Development** tab → Redirect URIs, and confirm `QB_CLIENT_ID`/`SECRET` in `_secrets.env` are that **same Development** key pair |
| Pre-flight said **green** but the dance still failed at the redirect | The doctor probes the **authorize** endpoint, which lenient providers (Intuit) accept even when the redirect URI isn't registered — they only enforce it at the **final redirect** (needs a browser) | Pre-flight green = client_id recognised + scope accepted; you must STILL register the exact redirect URI under the matching keys. The doctor now prints this caveat. |
| Dance fails to call back even with redirect registered — **wrong PORT** | The callback port must match the port in the app's *registered* redirect URI. We hit this: TrueLayer + QuickBooks were registered on `:3000`, Xero on `:3100`. | Run the dance on the port matching that app's registered redirect: `IFOS_OAUTH_CALLBACK_PORT=3000 bash scripts/run-oauth-dances.sh quickbooks` (TL/QB), plain (3100) for Xero. The doctor's redirect probe can tell you which port an app accepts. |
| **Xero** `invalid_scope` on `accounting.transactions` for a confirmed Web app | Xero **granular scopes**: apps created after **2026-03-02** only have the new granular scopes; the broad `accounting.transactions` is retired for them. | Request granular scopes: `offline_access accounting.invoices.read accounting.payments accounting.contacts.read` (already the bootstrap default). Probe per-scope with the *registered* redirect to see which are granted. |
| **Xero** `getInvoice` live test fails "no invoices to probe" | The connected Xero org is empty (you picked a real/blank org at the picker, not **Demo Company (Global)** which ships sample data) | Either reconnect and pick Demo Company, or rely on the resilient test (it verifies live not-found handling when the org is empty). |
