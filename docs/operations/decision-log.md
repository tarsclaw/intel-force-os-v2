# Decision log — IFOS v2 chronological history

**Append-only history** of shipped items, day-by-day closes, and prior backlog items now closed. Newest first.

Live tactical state is in `.agents/current-priorities.md` (kept lean, ≤120 lines, refreshed each day). This log is the audit trail — every Day-N section that drops off the lean priorities lands here verbatim with its original framing.

Reading order if you're catching up:
1. `.agents/current-priorities.md` first (today's state + active blockers + clocks)
2. This file's most recent section for context on yesterday/last-week's work
3. Older sections only if you're hunting for the origin of a current decision

Established 2026-06-01 (W4 Day-26) as part of the priorities refactor (goal §1.2). Pre-refactor, all of this content lived in `.agents/current-priorities.md` itself (had grown to 1161 lines + accreted across 25 days). The refactor preserves every historical entry verbatim; only the surface presentation changed.
