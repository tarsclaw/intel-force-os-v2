...
- **The file bus.** Inter-agent handoff via shared filesystem directories. No queue, no API, no serialisation tax. Brief Decoder writes a parsed-brief file; Sourcing Scout's watcher picks it up in milliseconds; result lands in a sub-directory the Concierge is watching. Four-agent pipelines complete in seconds, not the 15-second cold-start tax Lambda imposes.
