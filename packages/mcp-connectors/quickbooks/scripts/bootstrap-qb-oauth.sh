#!/usr/bin/env bash
#
# bootstrap-qb-oauth.sh — one-time QuickBooks Online OAuth 2.0 bootstrap (Path A).
#
# Performs the human-in-the-loop authorization-code dance for a QuickBooks
# Online sandbox company and writes a QbTokens bundle (shape per
# packages/mcp-connectors/quickbooks/src/types.ts) to the vault so
# @ifos/quickbooks's MCP_LIVE_TESTS suite + Cash Conductor cycle.sh Step-1
# auth refresh path can load it.
#
#   Step 1 (authorize): browser consent -> auth code + realmId  (localhost callback)
#   Step 2 (token):     auth code        -> {access_token, refresh_token,
#                                            expires_in, x_refresh_token_expires_in}
#
# Intuit confidential web-app flow: client_secret (Basic auth) on the token
# exchange; PKCE is not used (that is Intuit's public-client variant). The
# realmId is returned ON THE CALLBACK (not in the token response) and is
# verified against QB_SANDBOX_REALM_ID.
#
# PATH A / SECRETS DISCIPLINE (per memory feedback-never-cat-secrets-files +
# review-mcp-connector §5):
#   - Credentials are READ from the secrets env file at runtime and NEVER echoed.
#   - This helper prints only HTTP statuses, the output path, token_type, and
#     expiry timestamps — never a token, code, secret, client_id, or realmId.
#   - The token file is written mode 0600 via atomic tmp+rename (in Python).
#
# VERIFICATION STATUS: authored against Intuit's documented OAuth 2.0 model
# (README §"OAuth bootstrap" + src/auth.ts QB_TOKEN_ENDPOINT). The browser-
# consent leg cannot be exercised headlessly; first run by the founder IS the
# verification. If Step 1 never returns a code, the helper times out after
# 300s — surface per the goal STOP condition; verify the registered redirect.
#
# PREREQUISITE (founder, one-time, in the Intuit app at https://developer.intuit.com):
#   Add the redirect URI EXACTLY as:  http://localhost:3100/callback
#   (override the port with IFOS_OAUTH_CALLBACK_PORT; the registered URI must match.)
#
# USAGE:
#   bash packages/mcp-connectors/quickbooks/scripts/bootstrap-qb-oauth.sh
#
# ENV (read from $IFOS_SECRETS_FILE, default ~/.ifos-local-vault/dev-sandbox/_secrets.env):
#   QB_CLIENT_ID         (required)
#   QB_CLIENT_SECRET     (required)
#   QB_SANDBOX_REALM_ID  (required; the sandbox company you will consent for)
#
# ENV (optional overrides):
#   IFOS_OAUTH_CALLBACK_PORT  (default 3100; must match the registered redirect URI)
#   IFOS_TOKEN_DIR            (default ~/.ifos-local-vault/dev-sandbox)
#   IFOS_OAUTH_TIMEOUT_SECONDS (default 600; how long to wait for the browser callback)
#   IFOS_QB_SCOPE            (default "com.intuit.quickbooks.accounting")
#
# OUTPUT:
#   ${IFOS_TOKEN_DIR}/qb-tokens.json   (mode 0600; QbTokens shape)

set -euo pipefail

SECRETS_FILE="${IFOS_SECRETS_FILE:-${HOME}/.ifos-local-vault/dev-sandbox/_secrets.env}"
TOKEN_DIR="${IFOS_TOKEN_DIR:-${HOME}/.ifos-local-vault/dev-sandbox}"
CALLBACK_PORT="${IFOS_OAUTH_CALLBACK_PORT:-3100}"

die() { printf 'ERROR: %s\n' "$1" >&2; exit 1; }

command -v python3 >/dev/null 2>&1 || die "required tool not found on PATH: python3"

[ -f "${SECRETS_FILE}" ] || die "secrets file not found: ${SECRETS_FILE} (run scripts/fill-dev-sandbox-secrets.sh first)"

# Path A: source secrets into the environment; never echo their values.
set -a
# shellcheck disable=SC1090
. "${SECRETS_FILE}"
set +a

missing=""
for var in QB_CLIENT_ID QB_CLIENT_SECRET QB_SANDBOX_REALM_ID; do
  if [ -z "${!var:-}" ]; then
    missing="${missing} ${var}"
  fi
done
[ -z "${missing}" ] || die "the following required vars are EMPTY in ${SECRETS_FILE}:${missing}
  -> run: bash scripts/fill-dev-sandbox-secrets.sh   (then re-run this bootstrap)"

mkdir -p "${TOKEN_DIR}"

printf 'QuickBooks OAuth bootstrap — callback http://localhost:%s/callback\n' "${CALLBACK_PORT}"
printf '  A browser window will open. Sign in + click "Connect" / "Authorize".\n'
printf '  (If no browser opens, copy the URL this prints into your browser.)\n\n'

export IFOS_TOKEN_DIR="${TOKEN_DIR}"
export IFOS_OAUTH_CALLBACK_PORT="${CALLBACK_PORT}"

# -u: unbuffered stdout so the authorize URL + step progress print in real time.
python3 -u - <<'PYEOF'
import base64, http.server, json, os, secrets, sys, threading, time, urllib.parse, urllib.request, webbrowser

AUTHORIZE = "https://appcenter.intuit.com/connect/oauth2"
TOKEN     = "https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer"
SCOPE     = os.environ.get("IFOS_QB_SCOPE", "com.intuit.quickbooks.accounting")

client_id     = os.environ["QB_CLIENT_ID"]
client_secret = os.environ["QB_CLIENT_SECRET"]
env_realm     = os.environ["QB_SANDBOX_REALM_ID"].strip()
port          = int(os.environ.get("IFOS_OAUTH_CALLBACK_PORT", "3100"))
token_dir     = os.environ["IFOS_TOKEN_DIR"]
timeout_s     = int(os.environ.get("IFOS_OAUTH_TIMEOUT_SECONDS", "600"))
redirect_uri  = f"http://localhost:{port}/callback"

def b64url(b): return base64.urlsafe_b64encode(b).rstrip(b"=").decode("ascii")
state = b64url(secrets.token_bytes(16))

# Encode spaces as %20 (quote), not + (quote_plus default), for strict providers.
authorize_url = AUTHORIZE + "?" + urllib.parse.urlencode({
    "client_id": client_id,
    "response_type": "code",
    "scope": SCOPE,
    "redirect_uri": redirect_uri,
    "state": state,
}, quote_via=urllib.parse.quote)

captured = {}
class Handler(http.server.BaseHTTPRequestHandler):
    def log_message(self, *a): pass
    def do_GET(self):
        q = urllib.parse.urlparse(self.path)
        if not q.path.startswith("/callback"):
            self.send_response(404); self.end_headers(); return
        params = urllib.parse.parse_qs(q.query)
        captured["code"]    = (params.get("code")    or [""])[0]
        captured["state"]   = (params.get("state")   or [""])[0]
        captured["realmId"] = (params.get("realmId") or [""])[0]
        captured["error"]   = (params.get("error")   or [""])[0]
        self.send_response(200)
        self.send_header("Content-Type", "text/html")
        self.end_headers()
        self.wfile.write(b"<html><body><h2>IFOS QuickBooks bootstrap: received.</h2>"
                         b"<p>You can close this tab and return to the terminal.</p></body></html>")

srv = http.server.HTTPServer(("127.0.0.1", port), Handler)
threading.Thread(target=srv.serve_forever, daemon=True).start()

print(f"  [1/2] authorize ... opening browser (timeout {timeout_s}s)")
print(f"        {authorize_url}\n")
try:
    webbrowser.open(authorize_url)
except Exception:
    pass

deadline = time.time() + timeout_s
while "code" not in captured and time.time() < deadline:
    time.sleep(0.4)
srv.shutdown()

if captured.get("error"):
    sys.exit(f"ERROR: Intuit returned error='{captured['error']}' at the consent screen. STOP + surface.")
if not captured.get("code"):
    sys.exit(f"ERROR: no auth code captured within {timeout_s}s. No callback reached "
             f"{redirect_uri}. Most likely one of:\n"
             f"  (a) redirect_uri not added EXACTLY as {redirect_uri} under the Intuit app's\n"
             "      DEVELOPMENT/Sandbox keys (not Production);\n"
             "  (b) QB_CLIENT_ID/SECRET in _secrets.env are Production, not Development keys;\n"
             "  (c) consent not completed in time (raise IFOS_OAUTH_TIMEOUT_SECONDS).\n"
             "  See docs/operations/oauth-sandbox-app-setup-runbook.md. STOP + surface.")
if captured.get("state") != state:
    sys.exit("ERROR: state mismatch (possible CSRF/stale tab). STOP + surface; re-run.")

realm = captured.get("realmId") or env_realm
if env_realm and captured.get("realmId") and captured["realmId"] != env_realm:
    print("  WARNING: consented realmId differs from QB_SANDBOX_REALM_ID in _secrets.env.")
    print("           The live suite reads QB_SANDBOX_REALM_ID — make sure they match,")
    print("           or you consented for a different company than you'll test against.")

print("  [2/2] token exchange ... ", end="", flush=True)
basic = base64.b64encode(f"{client_id}:{client_secret}".encode()).decode()
body = urllib.parse.urlencode({
    "grant_type": "authorization_code",
    "code": captured["code"],
    "redirect_uri": redirect_uri,
}).encode()
req = urllib.request.Request(TOKEN, data=body, method="POST", headers={
    "Authorization": f"Basic {basic}",
    "Content-Type": "application/x-www-form-urlencoded",
    "Accept": "application/json",
})
try:
    with urllib.request.urlopen(req, timeout=30) as r:
        tok = json.load(r)
except Exception as e:
    # Do NOT print the response body — Intuit may echo the refresh_token on error.
    sys.exit(f"\nERROR: token exchange failed ({type(e).__name__}). The auth code is single-use "
             "and now spent; re-run the bootstrap. STOP + surface.")
needed = ("access_token", "refresh_token", "expires_in", "x_refresh_token_expires_in")
if not all(tok.get(k) for k in needed):
    sys.exit("\nERROR: token response missing expected fields. STOP + surface.")
print("ok")

now_ms = int(time.time() * 1000)
tokens = {
    "access_token":                tok["access_token"],
    "refresh_token":               tok["refresh_token"],
    "expires_at_ms":               now_ms + int(tok["expires_in"]) * 1000,
    "refresh_token_expires_at_ms": now_ms + int(tok["x_refresh_token_expires_in"]) * 1000,
    "scope":                       tok.get("scope", SCOPE),
    "token_type":                  tok.get("token_type", "Bearer"),
}

def atomic_write(path, obj):
    tmp = f"{path}.tmp.{os.getpid()}"
    fd = os.open(tmp, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    with os.fdopen(fd, "w") as f:
        json.dump(obj, f, indent=2)
    os.replace(tmp, path)
    os.chmod(path, 0o600)

token_file = os.path.join(token_dir, "qb-tokens.json")
atomic_write(token_file, tokens)

print(f"\nDONE. Token bundle written: {token_file} (mode 0600)")
print(f"  token_type={tokens['token_type']}  expires_at_ms={tokens['expires_at_ms']}")
print(f"  refresh_token_expires_at_ms={tokens['refresh_token_expires_at_ms']}  (~100 days)")
print("  Verify shape (names only, NEVER cat the file for values):")
print(f"    python3 -c \"import json;print(list(json.load(open('{token_file}')).keys()))\"")
print("  Then run the live suite:")
print("    pnpm --filter @ifos/quickbooks test:live")
PYEOF
