...
codex
I’ll verify the cited source files locally, focusing on hard blockers: adapter-boundary mentions, autosend policy action names, ESC catalogue entries, schema fields, and cited master/ULTRAPLAN lines.
exec
/bin/zsh -lc "nl -ba agents/recruitment/concierge/agent.md | sed -n '1,260p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   ...
   121	     Microsoft Graph / Gmail auth + AgentMail (if v1.1+ enabled) +
...
exec
/bin/zsh -lc 'rg -n "W10|Concierge|A6|AgentMail|Composio|gmail_outlook_send_to_candidate|bullhorn_note_customer_visible|twilio_sms_send|calendar_invite_send|concierge_email_draft|ESC_ADDRESSEE|ESC_CONCIERGE|ESC_MS_GRAPH|ESC_GMAIL|ESC_APPROVAL|ESC_SEND_FAIL|ESC_AUTOSEND_ORANGE|ESC_AUTOSEND_SAMPLED|ESC_GATE_B_MISS|ESC_LIFECYCLE|ESC_CANDIDATE|ESC_TONE|ESC_PII|ESC_AGENT_OUTPUT_SHAPE|ESC_RATE_LIMIT_HIT|ESC_BULLHORN_AUTH" -S docs agents packages | head -200' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/_shared/autosend-policy.yaml:174:  concierge_email_draft:
...
agents/_shared/autosend-policy.yaml:185:  bullhorn_note_customer_visible:
agents/_shared/autosend-policy.yaml:193:  gmail_outlook_send_to_candidate:
agents/_shared/autosend-policy.yaml:200:  twilio_sms_send:
agents/_shared/autosend-policy.yaml:207:  calendar_invite_send:
...
agents/_shared/escalation-codes.md:328:#### `ESC_SEND_FAIL`
agents/_shared/escalation-codes.md:338:#### `ESC_AUTOSEND_ORANGE_PENDING`
agents/_shared/escalation-codes.md:345:#### `ESC_APPROVAL_BRIDGE_TIMEOUT`
agents/_shared/escalation-codes.md:361:#### `ESC_AUTOSEND_SAMPLED_SPOT_CHECK`
...
agents/_shared/escalation-codes.md:395:#### `ESC_CANDIDATE_DATA_INCOMPLETE`
agents/_shared/escalation-codes.md:402:#### `ESC_ADDRESSEE_MISMATCH`
agents/_shared/escalation-codes.md:424:#### `ESC_CONCIERGE_SLA_MISS`
...
agents/_shared/escalation-codes.md:438:#### `ESC_LIFECYCLE_STATE_UNKNOWN`
...
exec
/bin/zsh -lc "rg --files | rg '(master|ULTRAPLAN|autosend-safety-policy|vertical-schema|escalation-codes|voice-loader|v1.0-kill-criterion)'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
codex
The first pass already shows a likely hard boundary issue: the artefact names AgentMail directly inside `agent.md`. I’m checking whether there are additional concrete blockers so the rejection is complete rather than single-issue.
exec
/bin/zsh -lc "nl -ba agents/recruitment/concierge/agent.md | sed -n '260,520p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   ...
   397	| Q8 | AgentMail (v1.1+) — agent-identity sends. Should Concierge use AgentMail for rejection emails (less personal pressure on consultant approving) or always tenant-identity? | v1.0: tenant-identity (Microsoft Graph / Gmail). v1.1+: AgentMail experiment per tenant opt-in. |
   ...
   406	4. **Microsoft Graph vs Gmail per-tenant** — each tenant chooses based on their existing email stack. v1.0 supports both; v1.1+ may add AgentMail.
   407	
   408	---
   409	
...
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '160,215p' && nl -ba docs/decisions/autosend-safety-policy.md | sed -n '110,155p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "rg -n \"§8\\.2|Week 10|Concierge|W10|W10-13|W9-10\" docs -S" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
docs/_supplementary/strategic-plan.md:358:### Week 10: First paying client
...
docs/architecture/architecture-cohesion-review.md:239:| R13 | G9: bus-overrides location for the 4 kb shadow points | Medium | Brain UI Week-10+ design ADR | Claude Code | Week 10 |
...
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '250,445p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "function hh_load|hh_load_tone|hh_load_voice|hh_load_recent" agents/_shared/voice-loader.sh' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "rg --files docs | rg -i 'master|brief|source'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
docs/build-brief/00-MASTER-BRIEF.md
docs/_supplementary/planning-phase-brief.md

exec
/bin/zsh -lc 'rg -n "tenant_adapters|concierge_last_poll|concierge_send_window|recent_edit|approved_verbatim|approved_after_edit|decision_log|phase|action_type|payload" docs/verticals/recruitment -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "tenant_adapters|tenant_adapter" docs/verticals/recruitment/vertical-schema.yaml docs/verticals/recruitment/vertical-schema.v0.2-supplement.yaml docs/verticals/recruitment/migrations -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
docs/verticals/recruitment/vertical-schema.v0.2-supplement.yaml:415:      per-tenant override [30, 365] via tenant_adapters.config.pii_retention_days.
docs/verticals/recruitment/migrations/v0.2-to-v0.3-pii-purge.sql:61:-- §3 — Per-tenant retention override storage (tenant_adapters extension)
docs/verticals/recruitment/migrations/v0.2-to-v0.3-pii-purge.sql:64:-- amendment. Storage is reserved at tenant_adapters.config.pii_retention_days
...
exec
/bin/zsh -lc 'rg -n "hh_decision_(trigger|output|action)" agents/_shared/hook-helpers.sh' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
1. Adapter boundary violation: `AgentMail` is named directly in `agent.md`. Lines 16, 59, 121, 397, and 406 mention AgentMail; the top-level adapter boundary says AgentMail is never referenced in `agent.md`, even for deferred surfaces. Replace with adapter-neutral wording such as “deferred agent-identity email adapter.”

2. Gate A is weakened without an accepted source. ULTRAPLAN line 566 defines “every lifecycle event has a draft generated within 30 minutes” under Gate A, but lines 273 and 203-209 downgrade it to a Gate B warning; line 16 also invents a “≥90% 30-min SLA hit” threshold not present in ULTRAPLAN line 567. Either keep the 30-minute condition as Gate A or cite an accepted decision that explicitly changes it.

3. Workflow writes drafts to vault before Gate A validation. Lines 184-190 write `/vault/<tenant>/concierge-drafts/<draft_id>.md` before voice, tone, addressee, and PII gates complete at lines 192-217, while lines 262-277 say Gate A hard-fails before action and failed drafts go to `/tmp`. Generate to `/tmp`, validate, then promote to vault only after Gate A passes.

4. Required `hh_decision_*` calls are missing at output/action points. Step 7 produces the draft and writes it to vault at lines 184-190 but has no decision-log call; Step 11 routes to autosend approval at lines 219-225 but has no `hh_decision_action`/`hh_decision_output`. Add explicit decision-log writes at each output/action boundary.

5. Escalation codes are repurposed beyond the catalogue. Line 303 uses `ESC_ADDRESSEE_MISMATCH` for candidate email mismatch, but the catalogue defines it for Cash Conductor invoice/client reconciliation; line 307 uses `ESC_CONCIERGE_SLA_MISS` for “draft >30 min,” but the catalogue defines inbound brief/customer reply SLAs; line 301 uses `ESC_LIFECYCLE_STATE_UNKNOWN` for Concierge taxonomy misses, while the catalogue defines Janitor placement ambiguity. Add new catalogue entries or align the triggers to the existing definitions.

6. Autosend policy citations are inaccurate. Lines 16 and 104-108 cite `autosend-policy.yaml` lines 122-149 / 130 / 122 / 137 / 144 for action types, but the local YAML registers `concierge_email_draft` and orange sends at lines 174-213. Update the citations to the actual file and line numbers.

7. Unregistered structured config fields are assumed. Lines 128-129 and 392 reference `tenant_adapters.config.concierge_last_poll` and `tenant_adapters.config.concierge_send_window`, but these fields are not defined in `vertical-schema.yaml` or the v0.2 supplement. Add them to the schema/supplement before the agent depends on them.
tokens used
119,103
REJECTED
