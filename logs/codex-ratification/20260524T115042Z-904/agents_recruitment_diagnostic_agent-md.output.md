...
codex
I’ll verify the cited project sources against the agent-bundle checks, then return only the required verdict format.
exec
/bin/zsh -lc 'rg --files docs agents packages | rg "(master|ULTRAPLAN|ultraplan|escalation-codes|autosend|voice-loader|vertical-schema|v1.0-kill|sequencing-target|common-voice|common-target-patch|agent.md|validate.sh)$"' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "Diagnostic|W3-4|A1|ESC_VOICE_DRIFT|ESC_INPUT_VALIDATION_FAIL|ESC_AGENT_OUTPUT_SHAPE|ESC_RENDERER_FAILED|consultant_feedback|diagnostic_report_render|hh_load_voice_samples|hh_load_tone_rules|Trigger 8|Trigger 1|common-voice|common-target-patch|decision_log" docs agents packages -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/recruitment/diagnostic/cycle.sh:211:      -d "text=Diagnostic report ready: ${FIRM_NAME} → ${REPORT_PATH}%0A%0A${SUMMARY}..." \
...
agents/recruitment/diagnostic/validate.sh:238:  # violations at write time per catalogue line 163 — not for Diagnostic's
...
codex
I found the key local sources. I’m now checking exact line anchors and catalogue entries where the Diagnostic draft makes load-bearing claims.
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '1,120p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '100,210p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba agents/_shared/voice-loader.sh | sed -n '1,230p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	#!/usr/bin/env bash
     2	#
...
exec
/bin/zsh -lc 'rg --files docs | rg "(master|MASTER|ULTRA|ultra|kill|sequencing|autosend-safety-policy|operational-hygiene|tenant-lifecycle|codex-disagreement)"' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "Diagnostic|A1|30%|12|required sections|Companies House|LinkedIn|Gate B|W3|W3-4|Week 3|Trigger 8|Trigger 1|2026-06-03|2026-06-14|diagnostic_report_render|operator_notify_telegram|common-voice|voice_classifier_threshold" docs agents -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
agents/_shared/autosend-policy.yaml:29:  diagnostic_report_render:
...
agents/_shared/autosend-policy.yaml:143:    reason: "Outbound LinkedIn action subject to rate limits + reputation effects; sample for tone + targeting accuracy"
docs/verticals/recruitment/vertical-schema.v0.2-supplement.yaml:294:        Most-recent voice classifier score (0.0-1.0) from any outbound Concierge message about this candidate. Concierge's autosend Gate A per bullhorn-integration-path.md §4.1 A6. NULL means no outbound message yet sent for this candidate. Threshold default 0.75 per common-voice.json.voice_classifier_threshold.
...
agents/_shared/escalation-codes.md:315:- **Payload fields:** `endpoint`, `entity_type`, `entity_id`, `status_code`, `error_body_preview` (truncated 120 chars)
...
docs/operations/goal-option-c-diagnostic-end-to-end.md:47:5. **`cycle.sh` is wired to call the new connectors** at the right workflow steps (§4 Steps 2 + 3 + 6 + 8 + 10 per `agent.md`). LinkedIn paths remain stubbed (gracefully degraded per fixture 02-edge-case).
...
docs/operations/goal-option-c-diagnostic-end-to-end.md:198:- `_section_2` calls `web-scraper.headCheck` for {firm}.com + {firm}.co.uk + LinkedIn-stubbed
docs/operations/goal-option-c-diagnostic-end-to-end.md:199:- `_section_3-5` LinkedIn paths remain stubbed with explicit "no LinkedIn data" content (matches fixture 02)
...
docs/operations/goal-option-c-diagnostic-end-to-end.md:394:  7. [SHA] decision(ADR-005): Week 3 repurposed
...
agents/recruitment/cash-conductor/agent.md:232:12. (Mode=webhook only) Re-trigger eligibility check
...
docs/operations/goal-week-3-polish-and-scaffold.md:1:# /goal — Week 3: Diagnostic polish + 5 agent.md scaffolds + Codex ratification
docs/operations/goal-week-3-polish-and-scaffold.md:5:**Master plan citations:** Master brief §8.2 (build wave 1 = Diagnostic + downstream sequence) + ULTRAPLAN §8.1 (per-agent specs A1-A6) + `sequencing-target.md` §3.1 (build waves ratified) + `v1.0-kill-criterion.md` Trigger 2 (Diagnostic must render cleanly by 2026-06-14) + Trigger 3 (Janitor Bullhorn W5 gate) + ADR-005 (Week-3 acceleration sequencing).
docs/operations/goal-week-3-polish-and-scaffold.md:7:**Maps to:** Day-13 (today) → Day-20 (end of Week 3). 7-day execution window.
docs/operations/goal-week-3-polish-and-scaffold.md:8:**Builds on:** `docs/operations/goal-option-c-diagnostic-end-to-end.md` (Day-13 Diagnostic v0 — completed at commit `97a57a2`).
docs/operations/goal-week-3-polish-and-scaffold.md:19:4. **`docs/specs/ULTRAPLAN.md`** §8.1 in full (per-agent A1-A6 specs, lines 495-572) + §9 (Bullhorn critical path, lines 720-770) + §10 (risk rows + contingencies)
...
docs/operations/goal-week-3-polish-and-scaffold.md:44:2. **LinkedIn slug suffix-stripping landed.** The web-scraper's online-footprint section now correctly tries `linkedin.com/company/hays` BEFORE `linkedin.com/company/hays-plc`. Hays's LinkedIn page is found.
...
docs/operations/goal-week-3-polish-and-scaffold.md:77:14. **`docs/RISK-REGISTER.md`** updated: Risk #2 (Bullhorn) blast radius confirmed reduced; Risk #5 (Renderer) state per Diagnostic ratification.
docs/operations/goal-week-3-polish-and-scaffold.md:89:| Diagnostic CH integration with real key | Goal §1 criterion 1; brings §1/§10/§11 from degraded → full |
docs/operations/goal-week-3-polish-and-scaffold.md:90:| LinkedIn slug suffix-strip (already partially applied to web; extend to LinkedIn URL guess) | Goal §1 criterion 2 |
...
docs/operations/goal-week-3-polish-and-scaffold.md:92:| Diagnostic Codex ratification | Master brief §10.5 always-ratify |
docs/operations/goal-week-3-polish-and-scaffold.md:97:### OUT of scope (don't touch in Week 3)
docs/operations/goal-week-3-polish-and-scaffold.md:101:| Full agent BUILDS for any non-Diagnostic agent | Reserved for W4 (Cash Conductor) + W5+ (Bullhorn-touching). Week 3 = scaffold-only for the 5 new agent.md contracts. |
docs/operations/goal-week-3-polish-and-scaffold.md:105:| Proxycurl signup | Founder commercial action; gated on founder decision (cost ~$39/mo); if founder approves mid-week, wire LinkedIn deep data into Diagnostic |
...
docs/operations/goal-week-3-polish-and-scaffold.md:118:Credentials needed from founder during Week 3:
...
docs/operations/goal-week-3-polish-and-scaffold.md:187:#### Step 3 — LinkedIn slug suffix-strip extension (~30 min)
docs/operations/goal-week-3-polish-and-scaffold.md:191:1. Apply suffix-stripping to the LinkedIn URL candidate generation (currently only applied to website domain check). After: "Hays plc" tries `https://www.linkedin.com/company/hays/` BEFORE `https://www.linkedin.com/company/hays-plc/`.
docs/operations/goal-week-3-polish-and-scaffold.md:192:2. Return the FIRST reachable URL as `linkedInUrl`; mark `linkedInReachable=true` for HTTP 200 OR 999 (LinkedIn bot-detect response).
...
docs/operations/goal-week-3-polish-and-scaffold.md:242:- §2 LinkedIn URL is `linkedin.com/company/hays/` (not `/hays-plc/`)
...
docs/operations/goal-week-3-polish-and-scaffold.md:440:Manifest now includes 6 new items (5 agent.md scaffolds + Diagnostic re-ratify). Founder runs:
docs/operations/goal-week-3-polish-and-scaffold.md:460:2. **`docs/RISK-REGISTER.md`** — Risk #2 (Bullhorn) status per A+B response state; Risk #5 (Renderer) updated per Diagnostic ratification.
docs/operations/goal-week-3-polish-and-scaffold.md:496:| Founder slow to register CH API key (Step 2) | Continue with Step 3 (LinkedIn slug fix; no key needed); resume Step 4 when key arrives; if no key by Day 16, escalate to founder review (Step 2 is a HARD gate for Step 5 smoke test) |
...
docs/operations/goal-week-3-polish-and-scaffold.md:641:  ✓ sequencing-target.md §3.1 — Build Wave 1 (Diagnostic) ratified;
docs/operations/goal-week-3-polish-and-scaffold.md:643:  ✓ v1.0-kill-criterion.md Trigger 2 — Diagnostic ratified ahead of
docs/operations/goal-week-3-polish-and-scaffold.md:644:    2026-06-14 deadline by N days
docs/operations/goal-week-3-polish-and-scaffold.md:650:    - Diagnostic voice classifier microservice
docs/operations/goal-week-3-polish-and-scaffold.md:651:    - Diagnostic LinkedIn deep data (if Proxycurl signed up)
docs/operations/goal-week-3-polish-and-scaffold.md:659:  - <list any open D1/D2/D3 decisions surfaced during Week 3>
docs/operations/goal-week-3-polish-and-scaffold.md:681:| 2 | Founder unavailable for Steps 2/4/7/13 longer than 24h | Medium | Continue with parallel work that doesn't depend on founder gates; resume when available; Day-20 deadline soft (Week 3 extends to Day 22-23 if needed) |
...
docs/operations/goal-week-3-polish-and-scaffold.md:692:Week 3 is complete when:
docs/operations/goal-week-3-polish-and-scaffold.md:700:**Day 20 is the soft target. Day 22 is the hard cutoff** (Trigger 2 fires Day 21 if Diagnostic not ratified). If Day 22 hits without completion, escalate to founder review + scope-cut decision.
docs/operations/goal-week-3-polish-and-scaffold.md:704:*End of /goal prompt for Week 3 polish + scaffold.*
...
agents/recruitment/scribe/agent.md:33:  "duration_seconds": 1234,
...
agents/recruitment/scribe/agent.md:274:  - No verbatim quotes longer than 12 words from candidate (paraphrase for privacy)
agents/recruitment/scribe/agent.md:290:| Diagnostic ratified (first-agent precedent) | Week 3 Codex Round 4 Phase 1 | ⏸ |
...
agents/recruitment/janitor/agent.md:113:   → for each entity, check critical fields: candidate.location (line 124),
agents/recruitment/janitor/agent.md:121:6. Companies House enrichment (clients only)
agents/recruitment/janitor/agent.md:125:   → 7-day cache per tools.yaml; rate-limit budget shared with Diagnostic
...
agents/recruitment/janitor/agent.md:165:12. Session close
...
agents/recruitment/janitor/agent.md:254:| Companies House MCP connector | Day-13 shipped (`@ifos/companies-house`) | ✅ |
...
agents/recruitment/diagnostic/cleanup.sh:7:# gotcha §6.1 (LinkedIn ToS: no profile data persistence beyond audit).
agents/recruitment/diagnostic/cleanup.sh:11:# multi-tenant render. Most critical: LinkedIn profile data MUST NOT be
agents/recruitment/diagnostic/cleanup.sh:18:#   1  Cleanup failure on a critical path (LinkedIn cache still present)
agents/recruitment/diagnostic/cleanup.sh:31:# 1 — LinkedIn transient cache purge (CRITICAL per ToS gotcha §6.1)
...
agents/recruitment/diagnostic/cleanup.sh:53:  printf 'cleanup.sh: CRITICAL — LinkedIn cache not fully purged (%d dirs remain)\n' "${LEFTOVER}" >&2
agents/recruitment/diagnostic/cleanup.sh:57:    '{"escalation_code":"ESC_PII_LEAKAGE_RISK","reason":"LinkedIn cache purge incomplete"}' \
agents/recruitment/diagnostic/cleanup.sh:64:  printf 'cleanup.sh: purged %d LinkedIn cache dir(s)\n' "${PURGED}"
...
docs/decisions/codex-disagreement-2026-05-24-diagnostic-gate-a.md:324:**Cumulative empirical:** 9 Codex rounds total (Round 4-v1, 4-v2, 5, 6, 7 on Diagnostic/Janitor + Round 8 across all 6). **55+ unique findings catalogued across rounds, ~10-12 fixed via Cat-α mechanical disposition in this bilateral session; rest queued.**
docs/decisions/codex-disagreement-2026-05-24-diagnostic-gate-a.md:329:- Diagnostic: pre-build-with-honesty-notes; Round-8-reviewed; ~3 implementation-gap findings (Cat-δ) queued for W3-4 polish slice
docs/decisions/codex-disagreement-2026-05-24-diagnostic-gate-a.md:339:3. Per-agent validate.sh + cycle.sh + tools.yaml + cleanup.sh build slices (W3-W13)
docs/decisions/codex-disagreement-2026-05-24-diagnostic-gate-a.md:342:**Week 3 IS closed** per documented protocol: scaffolds at Pre-Build-Round-8-Reviewed status with Cat-α mechanical fixes applied + Cat-β/γ/δ/ε findings categorized + queued. Honest signal: 0/6 RATIFIED by Round 8; categorization shows residual findings are structural (schema landing, build-slice delivery) not relitigation of the 5-category dispositions. Diagnostic v0 Build (validate.sh + cycle.sh exist; just incomplete) remains the most ready for v3-W4 polish.
docs/decisions/codex-disagreement-2026-05-24-diagnostic-gate-a.md:344:*End of Codex bilateral disposition document (Phase 1 + 2 + 3 executed; Week 3 closed; v0.3 supplement is the unblock for next ratification).*
...
docs/_archive-build-pack/02-PRODUCT-VISION.md:157:- **Light + dark mode** matter; users will screenshot this for LinkedIn — it must look beautiful in both
...
agents/recruitment/sourcing-scout/agent.md:30:@ifos_bot scout --description "Senior React engineer, London, £120k, hybrid"
...
agents/recruitment/sourcing-scout/agent.md:177:     email / phone / LinkedIn URL / Bullhorn bullhorn_id per canonical schema)
agents/recruitment/sourcing-scout/agent.md:222:- **"each has a working contact method"** (email format + MX check OR E.164 phone OR LinkedIn URL OR Bullhorn bullhorn_id-with-contact)
...
agents/recruitment/diagnostic/fixtures/99-voice-drift-canary.yaml:46:  # Skip; this fixture focuses on §12 validation, not §1-§11 generation
agents/recruitment/diagnostic/fixtures/99-voice-drift-canary.yaml:49:      company_number: "12345678"
agents/recruitment/diagnostic/fixtures/99-voice-drift-canary.yaml:60:  V1_12_sections: PASS  # sections present
...
docs/_archive-build-pack/07-V1-INHERITED-CONTEXT.md:109:| Buying signal | Recent HR hire, recent HR-tool purchase, LinkedIn post about being overwhelmed |
...
agents/recruitment/diagnostic/fixtures/01-primary.yaml:1:# Diagnostic fixture — 01-primary
agents/recruitment/diagnostic/fixtures/01-primary.yaml:3:# Golden case. Well-formed UK fintech recruitment firm with all 12 sections
...
agents/recruitment/diagnostic/fixtures/01-primary.yaml:9:# Reading order: agent.md §3 (12 sections) + validate.sh V1-V6.
...
agents/recruitment/diagnostic/fixtures/01-primary.yaml:44:        appointment_date: "2019-04-12"
agents/recruitment/diagnostic/fixtures/01-primary.yaml:60:      avg_salary_band_gbp: "£70k-£120k"
agents/recruitment/diagnostic/fixtures/01-primary.yaml:82:  section_count: 12
agents/recruitment/diagnostic/fixtures/01-primary.yaml:89:  V1_12_sections: PASS
agents/recruitment/diagnostic/fixtures/01-primary.yaml:106:    action_type: diagnostic_report_render
...
docs/_supplementary/build-plan-original.md:670:- Platform format specs (LinkedIn: 1300 chars optimal, IG carousel: 10 slides max, etc.)
docs/_supplementary/build-plan-original.md:681:1. LinkedIn post (1 hook + 3 points + 1 CTA, <1300 chars)
docs/_supplementary/build-plan-original.md:690:- No cross-platform contamination (LinkedIn tone in an IG caption = fail)
...
agents/recruitment/diagnostic/validate.sh:93:if (( SECTION_COUNT == 12 )); then
...
agents/recruitment/diagnostic/validate.sh:131:# Extract §12 (conversation opener)
agents/recruitment/diagnostic/validate.sh:132:SECTION_12=$(awk '/^## .*[Cc]onversation [Oo]pener/{flag=1; next} /^## /{flag=0} flag' "${DRAFT}")
agents/recruitment/diagnostic/validate.sh:134:if [[ -z "${SECTION_12}" ]]; then
...
agents/recruitment/diagnostic/validate.sh:143:        "$(printf '%s' "${SECTION_12}" | python3 -c "import json,sys; print(json.dumps(sys.stdin.read()))")" \
...
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:4:# dead LinkedIn page (no posts in 12+ months), no job posts. Tests how
...
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:6:# sections 2-11 mostly "no signal found" content; section 12 still
...
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:34:      registered_office: "12 Mill Lane, Sheffield S1 2AB"
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:49:      last_post_at: "2024-11-12"  # 18 months ago
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:70:  section_count: 12              # all 12 still present
...
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:75:  V1_12_sections: PASS
...
agents/recruitment/diagnostic/fixtures/02-edge-case-no-online-footprint.yaml:82:# Special expected behaviour: the opener (§12) must avoid claiming
...
agents/recruitment/diagnostic/agent.md:49:| 1 | **Firm signal** | Companies House data: registered name, company number, incorporation date, latest filed accounts (revenue band + headcount band), registered office, recent director changes, share-class moves | Companies House API |
agents/recruitment/diagnostic/agent.md:50:| 2 | **Online footprint** | Primary website URL + last-updated signal; LinkedIn company page URL + follower count + last-post recency; careers page URL + state (active / placeholder / 404) | Web scraper (HEAD + first 200 lines); LinkedIn company-page fetch |
agents/recruitment/diagnostic/agent.md:51:| 3 | **Sector + role-type mix** | Sectors actively recruiting for (extracted from current job posts); ratio of permanent vs contract roles in last 90 days; technical-vs-commercial-vs-operational split | LinkedIn job posts + careers page job listings |
agents/recruitment/diagnostic/agent.md:52:| 4 | **Geography** | Office locations + current hiring locations + remote-vs-onsite-vs-hybrid mix | LinkedIn job posts (location field) + Companies House registered office |
agents/recruitment/diagnostic/agent.md:53:| 5 | **Deal-size band proxy** | Salary bands or day-rate ranges visible in job posts; level distribution (junior / mid / senior / executive); recent placements visible via LinkedIn employees-of-firm field changes | LinkedIn job posts + LinkedIn employee timeline scan |
...
agents/recruitment/diagnostic/agent.md:56:| 8 | **Pain signals** | Phrases on careers page suggesting urgency ("rapid growth", "we're scaling fast", "looking to triple the team"); LinkedIn director posts mentioning hiring pressure or "we need help"; Glassdoor reviews mentioning workload/burnout (if accessible) | Careers page scrape + LinkedIn director post search + optional Glassdoor scrape |
agents/recruitment/diagnostic/agent.md:57:| 9 | **Competitor positioning** | Other recruitment firms visible in the candidate flow: LinkedIn employee profiles showing previous-employer agency names; @firm tags in LinkedIn recruitment-agency posts; LinkedIn "Who's hiring this firm" inference where visible | LinkedIn profile scrapes + agency-tag search |
agents/recruitment/diagnostic/agent.md:58:| 10 | **Recent activity** | LinkedIn company posts in last 90 days (count + summary); press releases or news mentions (basic Google search); funding events visible on Companies House (share allotments, new director appointments) | LinkedIn company page + Google web search + Companies House filing history |
agents/recruitment/diagnostic/agent.md:59:| 11 | **Decision-maker map** | Named people likely to be buyers: head of talent / chief people officer / hiring manager equivalents. LinkedIn profile URL per person. Tenure at firm. Recent activity. | LinkedIn employee search filtered by title |
...
agents/recruitment/diagnostic/agent.md:64:- Fewer than 12 sections present
agents/recruitment/diagnostic/agent.md:66:- Section 12 (conversation opener) failing voice-classifier with score < 0.75
agents/recruitment/diagnostic/agent.md:86:2. Companies House lookup (Section 1)
agents/recruitment/diagnostic/agent.md:87:   → companies_house_lookup(firm_name) via Companies House MCP connector
...
agents/recruitment/diagnostic/agent.md:99:   → LinkedIn job posts API (filtered to firm) — up to 50 most recent
agents/recruitment/diagnostic/agent.md:102:   → Gotcha §6: store summarised data only; do not retain raw profiles per LinkedIn ToS
agents/recruitment/diagnostic/agent.md:105:   → LinkedIn search for {firm} employees with titles matching head-of-talent / chro / chief people / hiring-manager / talent-acquisition variants
agents/recruitment/diagnostic/agent.md:110:   → regex pass over careers-page + LinkedIn director posts for:
agents/recruitment/diagnostic/agent.md:123:   → LinkedIn company posts in last 90 days (count + first 100 chars of each)
agents/recruitment/diagnostic/agent.md:125:   → Companies House filing history last 90 days
agents/recruitment/diagnostic/agent.md:127:9. Conversation opener generation (Section 12)
agents/recruitment/diagnostic/agent.md:136:    → render report from sections 1-12 using Diagnostic-specific template
...
agents/recruitment/diagnostic/agent.md:146:12. Session close
agents/recruitment/diagnostic/agent.md:147:    → hh_decision_action("diagnostic_report_render", "firm:{slug}", payload_hash, payload_preview)
agents/recruitment/diagnostic/agent.md:158:Per master brief §8.1 Change 2 + autosend-safety-policy §4. Diagnostic's `validate.sh` enforces the following SPEC. **Honesty note (per bilateral-disposition Cat-5):** the v0 `validate.sh` at `agents/recruitment/diagnostic/validate.sh` implements most of these checks today but warns-only on two upstream-unavailable cases (voice-classifier URL unreachable, firm-domain whitelist absent). Hard-fail behaviour on those two cases is a W4 polish item; current v0 honesty-flags them in `decision_log` payload with `validate_check_skipped=true` instead of hard-failing. The spec below describes the W4-complete contract; the W4 build slice closes the two gaps.
agents/recruitment/diagnostic/agent.md:160:- All 12 sections present in the assembled report (count + heading check) — **v0: hard-fails as specified**
agents/recruitment/diagnostic/agent.md:162:- Section 12 voice classifier score ≥ 0.75 (`hh_load_voice_samples` returns ANN match + classifier; score computed via tenant's voice classifier per Ultraplan §5.3) — **v0: warns + flags `validate_check_skipped=true` if voice-classifier URL unreachable; W4 polish closes to hard-fail**
...
agents/recruitment/diagnostic/agent.md:191:- `ESC_AUTOSEND_*` — Diagnostic's only action is `diagnostic_report_render` (green tier per autosend-policy.yaml)
agents/recruitment/diagnostic/agent.md:192:- `ESC_VAULT_*` — Diagnostic writes to one file per invocation; no concurrent-write contention
agents/recruitment/diagnostic/agent.md:198:Section 12 (conversation opener) is voice-classified. The agent integrates with `_shared/voice-loader.sh`:
agents/recruitment/diagnostic/agent.md:205:- **`hh_load_recent_edits` last 30 days for `concierge` + `diagnostic`**: surfaces patterns of how consultant edits agent drafts. Per-run `ESC_VOICE_DRIFT` fires when the §12 voice classifier score is below 0.75 after 3 retries. Aggregate `ESC_VOICE_DRIFT_TENANT` fires per `escalation-codes.md` ESC_VOICE_DRIFT_TENANT trigger — ≥5 `ESC_VOICE_DRIFT` rows from the same tenant within a rolling 7-day window (per the nightly voice-drift cron). Edit-distance metrics are tracked separately for analytics but do NOT fire ESC_VOICE_DRIFT_TENANT directly.
agents/recruitment/diagnostic/agent.md:211:## §8 — Build dependencies (W3 prerequisites)
agents/recruitment/diagnostic/agent.md:213:Diagnostic build cannot start until ALL of the following are confirmed:
agents/recruitment/diagnostic/agent.md:219:| Tenancy audit passes 12 invariants | `bash scripts/run-tenancy-audit.sh` | ⏸ Founder action |
agents/recruitment/diagnostic/agent.md:220:| Q1 design partner LOI signed | Risk #3 + kill-criterion §2 Trigger 1 | ⏸ Jack's lane |
...
agents/recruitment/diagnostic/agent.md:249:| Q5 | What's the "firm-slug" canonical form for the output filename? Companies House registration number? URL-slugified firm name? | Recommend Companies House number for stability; canonical-slug fallback for non-UK firms (v1.1+). |
agents/recruitment/diagnostic/agent.md:252:### Gotchas (carried forward from Ultraplan §8.1 A1)
agents/recruitment/diagnostic/agent.md:254:1. **LinkedIn ToS — cannot store profile data beyond the audit.** Profile fetches are cached only for the duration of the report generation (typ. 10-15 min). After report writes to vault, raw profile data is dropped from agent memory + no persistence in Postgres.
...
agents/recruitment/diagnostic/agent.md:266:- Founder approves §3's 12-section list as canonical
...
agents/recruitment/diagnostic/README.md:32:2. **Surfaces design questions early** (e.g., what are the 12 sections? — answered in agent.md §3 + open questions §9)
...
agents/recruitment/diagnostic/README.md:48:- ⏸ Tenancy audit passes 12 invariants (`scripts/run-tenancy-audit.sh`)
...
agents/recruitment/diagnostic/cycle.sh:112:# v0 build (Day 13): web-scraper + companies-house wired; LinkedIn deep
agents/recruitment/diagnostic/cycle.sh:130:# Load Companies House API key from tenant secrets if present
agents/recruitment/diagnostic/cycle.sh:163:# Step 12 — Validate (Gate A)
agents/recruitment/diagnostic/cycle.sh:167:  "${FIRM_NAME} 12-section draft (pre-validate)" >/dev/null
agents/recruitment/diagnostic/cycle.sh:182:hh_decision_action "diagnostic_report_render" "firm:${FIRM_SLUG}" \
agents/recruitment/diagnostic/cycle.sh:187:printf '\n\033[1;32mDiagnostic report written:\033[0m %s\n' "${REPORT_PATH}"
...
agents/recruitment/diagnostic/cycle.sh:211:      -d "text=Diagnostic report ready: ${FIRM_NAME} → ${REPORT_PATH}%0A%0A${SUMMARY}..." \
agents/recruitment/diagnostic/cycle.sh:219:# Call cleanup (drop transient LinkedIn cache per ToS gotcha §6.1)
agents/recruitment/diagnostic/tools.yaml:1:# Diagnostic agent — MCP tool declarations
agents/recruitment/diagnostic/tools.yaml:3:# Status: Proposed (pre-W3-build scaffold; Day-11 author, awaits W3-4 build).
...
agents/recruitment/diagnostic/tools.yaml:50:  # ── §4 Step 3+4+5+7+8+10+11: LinkedIn read-only ───────────────────────
...
docs/artefacts/diagnostic-hays-plc-2026-05-24-v2.md:19:LinkedIn page at [https://www.linkedin.com/company/hays-plc/](https://www.linkedin.com/company/hays-plc/) not reachable (HTTP n/a). Firm may use a different LinkedIn slug or have no company page.
...
docs/artefacts/diagnostic-hays-plc-2026-05-24-v2.md:90:> Hi — looking at Hays plc's public footprint, the standard signals (CH filings, careers page, LinkedIn) aren't surfacing the usual indicators of hiring pressure. That itself is interesting — happy to compare what we're seeing across recruitment sector peers if useful.
...
docs/_supplementary/strategic-plan.md:87:5. **Repurposer** — one pillar piece → LinkedIn, IG carousel, YouTube Short script, email, thread
docs/_supplementary/strategic-plan.md:373:## 8. Go-to-Market (weeks 8–12, overlapping with build)
docs/_supplementary/strategic-plan.md:400:5. **Paid (LinkedIn Ads → video case study → audit funnel)** — only after 10+ case studies. Don't burn money here in v1.
...
docs/artefacts/diagnostic-hays-plc-2026-05-24.md:19:LinkedIn page at [https://www.linkedin.com/company/hays-plc/](https://www.linkedin.com/company/hays-plc/) not reachable (HTTP n/a). Firm may use a different LinkedIn slug or have no company page.
...
docs/artefacts/diagnostic-hays-plc-2026-05-24.md:90:> Hi — looking at Hays plc's public footprint, the standard signals (CH filings, careers page, LinkedIn) aren't surfacing the usual indicators of hiring pressure. That itself is interesting — happy to compare what we're seeing across recruitment sector peers if useful.
...
agents/recruitment/concierge/agent.md:82:| 12 | 90-day check-in | Nurture-check-in + relationship | Candidate + Client | Establishes ongoing relationship; offers "is there anyone in your network looking?" |
agents/recruitment/concierge/agent.md:88:event_type: <one of 12 above>
agents/recruitment/concierge/agent.md:102:Each draft: `decision_log` row with `agent_name='concierge'`, `phase='output'`, `action_type='concierge_email_draft'` (registered yellow tier per autosend-policy.yaml), `tier='yellow'`, payload includes `event_type` + `voice_score` + `recipient` + `escalation_position` (event-type is a payload field, not part of action_type — keeps action_type stable across 12 lifecycle events).
...
agents/recruitment/concierge/agent.md:133:   → ESC_LIFECYCLE_STATE_UNKNOWN if state transition not in 12-event taxonomy
...
agents/recruitment/concierge/agent.md:373:| `cycle.sh` orchestration (15-step) | Build at W10-11 (~5 days; lifecycle state machine + 12 event types) | ⏸ |
...
agents/recruitment/concierge/agent.md:375:| Comms-template library v0.1 (12 event types × 2 recipient roles = 24 templates minimum) | Build at W12-13 (~5 days) | ⏸ |
...
exec
/bin/zsh -lc "find . -path '*/common-voice.json' -o -path '*/common-target-patch.json' -o -path '*/tools.yaml' | sort" in /Users/madsadmin/code/CortexOS
 succeeded in 363ms:
...
exec
/bin/zsh -lc "nl -ba packages/agents-runtime/_shared/common-voice.json | sed -n '1,180p'; nl -ba packages/agents-runtime/_shared/common-target-patch.json | sed -n '1,220p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	{
...
exec
/bin/zsh -lc "nl -ba agents/recruitment/diagnostic/validate.sh | sed -n '254,330p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba agents/_shared/hook-helpers.sh | sed -n '180,255p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   254	  ESC_PAYLOAD=$(printf '{"escalation_code":"%s","draft_path":"%s","failures":%s}' \
   255	    "${ESC_CODE}" \
   256	    "${DRAFT}" \
...
exec
/bin/zsh -lc "nl -ba docs/decisions/autosend-safety-policy.md | sed -n '70,115p'; nl -ba docs/decisions/autosend-safety-policy.md | sed -n '628,640p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    70	
    71	---
    72	
   ...
   637	
   638	**For Risk #3 (design partner gap).** §10 pilot-agreement liability text is a prerequisite for a signed LOI. Final legal review must happen before LOI signing per §10. This is a non-trivial path-dependency: design-partner acquisition cannot complete without legal review of this section.
   639	
   640	**For Codex Day-7 ratification queue.** This policy joins the queue. Ratification reviews:

exec
/bin/zsh -lc 'rg -n "validate_gate_a_fail|consultant_feedback|diagnostic_email_send|diagnostic_calendar_invite|diagnostic_report_render|operator_notify_telegram" agents/_shared/autosend-policy.yaml docs/decisions/autosend-safety-policy.md agents/recruitment/diagnostic -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
docs/decisions/autosend-safety-policy.md:81:| Diagnostic | `diagnostic_report_render` | Internal artefact write; no external comms; idempotent (re-render overwrites) |
docs/decisions/autosend-safety-policy.md:108:| Diagnostic | `diagnostic_email_send` | Outbound diagnostic report to prospect; sales-stage outreach; reputation |
docs/decisions/autosend-safety-policy.md:109:| Diagnostic | `diagnostic_calendar_invite` | Books intro call with prospect; reputation + scheduling friction |
...
1. Gate A weakens the ULTRAPLAN source-data requirement. Line 16 and §3 lines 45, 64-67 define Gate A as section-count plus per-section citation coverage, while `docs/specs/ULTRAPLAN.md` lines 496-497 require all 12 sections and “no claims unsupported by source data.” The artefact explicitly defers per-claim citation validation to W4, so the current contract is weaker than the cited source. Fix by making unsupported-claim detection a Gate A requirement or by amending the upstream ULTRAPLAN requirement before ratification.

2. The Gate A contract is internally inconsistent. §1 line 16 and §3 lines 62-67 say voice score <0.75 and PII boundary failures hard-fail Gate A, but §5 lines 158, 162, and 165 say v0 only warns and sets `validate_check_skipped=true` when the voice classifier or firm-domain whitelist is unavailable. This violates “Quality gates before features” because the implementation caveat weakens the output contract. Fix by either hard-failing those cases now or narrowing §1/§3 to the actual v0 behaviour with explicit W4 status-flip criteria.

3. Build-dependency status is not honest. §5 line 158 says `agents/recruitment/diagnostic/validate.sh` exists and implements most checks today, but §8 line 226 marks “`validate.sh` Gate A logic” as “⏸ Not built”; the repository also contains the full sibling bundle and three fixtures. The artefact cannot simultaneously be a pre-build scaffold with validate.sh “not built” and a v0 implementation with known warn-only gaps. Fix §8 to reflect actual built files and separate “exists” from “W4 hard-fail completion.”

4. §6 lists a non-Diagnostic escalation code under “Diagnostic uses these ESC codes.” Lines 177-186 introduce the table as codes Diagnostic uses, then include `ESC_RENDERER_FAILED` while line 186 says it is “not Diagnostic's concern; renderer escalation only.” That muddies the escalation subset required for this agent and conflicts with the catalogue, where `ESC_RENDERER_FAILED` is owned by `_renderer`. Fix by removing it from the “uses” table and, if needed, mention renderer failures only in §10 ratification evidence.

5. §6 says Diagnostic has only one action, contradicting §4. Line 191 says Diagnostic’s only action is `diagnostic_report_render`, but §4 lines 140-143 define an optional `operator_notify_telegram` action with its own `hh_decision_action` call. This makes the autosend surface inaccurate. Fix by listing both `diagnostic_report_render` and `operator_notify_telegram` as Diagnostic action types with their green-tier policy rows.
tokens used
78,204
REJECTED
