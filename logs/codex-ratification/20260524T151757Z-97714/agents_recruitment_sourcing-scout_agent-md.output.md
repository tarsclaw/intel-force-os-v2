...
session id: 019e5a90-78a2-7e72-8298-5cbf3dd269e9
--------
user
...
codex
I’ll verify the artefact’s cited project files before issuing the verdict, focusing on schema/config claims, escalation-code existence, and line/section accuracy.
exec
/bin/zsh -lc 'rg -n "Sourcing Scout|A5|blocked_recipients|auto_source_on_brief_create|ESC_LINKEDIN_AUTH|ESC_REED_AUTH|ESC_CVLIBRARY_AUTH|ESC_GATE_B_MISS|ESC_AGENT_OUTPUT_SHAPE|ESC_BRIEF_AMBIGUITY|ESC_DNC_FILTER_HIT|send_to_blocked_recipient|consultant_feedback|hh_load_tone_rules|hh_load_voice_samples|hh_load_recent_edits" docs agents migrations packages -S' in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '1,260p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/vertical-schema.yaml | sed -n '70,115p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "rg --files | rg 'master|ULTRAPLAN|autosend-safety-policy|v0\\.2-to-v0\\.3|voice-loader|hook-helpers|v1\\.0-kill-criterion'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/recruitment/sourcing-scout/agent.md:3:**Status:** Proposed (2026-05-24 pre-W9-build scaffold; awaits Q1 LOI + Bullhorn Sub-decisions A+B + Proxycurl + Reed + CV-Library commercial signups + W9 build slice. tenant_adapters.config.blocked_recipients is registered in v0.2-to-v0.3.sql §5 allowlist; auto_source_on_brief_create is v0.4-supplement-pending — webhook auto-source code path blocked until v0.4 lands.)
...
agents/recruitment/sourcing-scout/agent.md:109:     + DNC list from `tenant_adapters.config.blocked_recipients (registered in `migrations/v0.2-to-v0.3.sql §5` validator allowlist — Postgres-backed structured state per ADR-002; canonical source for v1.0 DNC list)` (Postgres-
...
agents/recruitment/sourcing-scout/agent.md:191:   → load tenant DNC list from `tenant_adapters.config.blocked_recipients (registered in `migrations/v0.2-to-v0.3.sql §5` validator allowlist — Postgres-backed structured state per ADR-002; canonical source for v1.0 DNC list)`
...
agents/recruitment/sourcing-scout/agent.md:267:Measured via consultant feedback loop: each candidate in a Sourcing Scout report gets a "useful / not useful" tag from the consultant via Brain UI v1.1 OR Telegram reply (v1.0). Aggregate over rolling 30-day window per tenant. NOTE: Bullhorn-note feedback is NOT a v1.0 path (Sourcing Scout is read-only on Bullhorn per §1 + §6 contracts; would require write capability not in v1.0 scope).
...
agents/recruitment/sourcing-scout/agent.md:338:| Tenant DNC list populated in `tenant_adapters.config.blocked_recipients (registered in `migrations/v0.2-to-v0.3.sql §5` validator allowlist — Postgres-backed structured state per ADR-002; canonical source for v1.0 DNC list)` (Postgres-backed structured state per ADR-002 vault/Postgres split) | Tenant onboarding | ⏸ |
...
agents/recruitment/sourcing-scout/agent.md:361:| Q5 | Gate B 6-of-10 metric — measured via consultant feedback (Brain UI v1.0 doesn't have feedback UX yet) | v1.0: Telegram reply with "/scout-feedback <candidate-id> useful|not-useful" → `decision_log` row via `consultant_feedback` green-tier action_type. v1.1: Brain UI button. NOTE: Bullhorn-note-based feedback is NOT a v1.0 path — Sourcing Scout is read-only on Bullhorn (no write capability); Bullhorn note creation would require tools.yaml write capability + autosend/decision logging which v1.0 explicitly excludes. |
...
codex
The first pass found a few citation-sensitive areas worth checking directly: the A5 source lines, the migration allowlist, and the provider auth/escalation entries.
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '90,130p;335,355p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    90	    tier: green
    91	    agent: concierge
...
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '256,435p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   256	#### `ESC_LINKEDIN_AUTH`
   257	- **Severity:** **blocking** — Sourcing Scout enters degraded mode (no profile fetches; cached only)
   258	- **Trigger:** LinkedIn session cookie / OAuth token revoked or expired (LinkedIn has aggressive anti-bot session invalidation)
...
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/migrations/v0.2-to-v0.3.sql | sed -n '360,415p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "hh_decision_trigger|hh_decision_output|hh_decision_action|hh_decision_" agents/_shared/hook-helpers.sh' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "auto_source_on_brief_create|blocked_recipients|sourcing_scout|external_ref|candidate_id|brief_id" docs/verticals/recruitment -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba docs/decisions/autosend-safety-policy.md | sed -n '1,120p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "tools.yaml|proxycurl|reed|cvlibrary|Bullhorn|sourcing-scout" agents/recruitment -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/recruitment/cash-conductor/agent.md:36:Per-provider webhook auth handled in `tools.yaml`.
...
agents/recruitment/cash-conductor/agent.md:390:| `tools.yaml` MCP capability declarations (xero_oauth, quickbooks_oauth, sage_oauth, open_banking_truelayer / plaid, telegram_notify) | Build at W7 start (~1 day) | ⏸ |
...
agents/recruitment/diagnostic/tools.yaml:186:# End of tools.yaml
...
agents/recruitment/diagnostic/cleanup.sh:35:# in /tmp/ifos-linkedin-cache-<pid>/. Even though tools.yaml sets ttl=0,
agents/recruitment/diagnostic/cleanup.sh:71:# Companies House cache stays per tools.yaml (7-day TTL).
agents/recruitment/diagnostic/cleanup.sh:72:# Web scraper cache stays per tools.yaml (1-hour TTL).
...
agents/recruitment/diagnostic/agent.md:221:| Other bundle files (cycle.sh, tools.yaml, cleanup.sh) | `agents/recruitment/diagnostic/` | ✅ All built |
...
agents/recruitment/sourcing-scout/agent.md:129:       Sourcing-Scout-specific interpretation: skip Bullhorn source +
agents/recruitment/sourcing-scout/agent.md:132:       "report-only with Bullhorn data omitted from the sourcing set")
agents/recruitment/sourcing-scout/agent.md:146:3. Bullhorn passive-match query
agents/recruitment/sourcing-scout/agent.md:156:   → ESC_RATE_LIMIT_HIT on Bullhorn 429 (payload.upstream='bullhorn')
agents/recruitment/sourcing-scout/agent.md:160:   → proxycurl.search_people(query=brief_key_dimensions, location=brief_location,
agents/recruitment/sourcing-scout/agent.md:168:   → reed.search_candidates(query=brief_dimensions, location, salary_band)
agents/recruitment/sourcing-scout/agent.md:171:     (payload.upstream='reed')
agents/recruitment/sourcing-scout/agent.md:172:   → hh_decision_output("reed_query", "brief:<id>", "results:<N>")
agents/recruitment/sourcing-scout/agent.md:175:   → cvlibrary.search_candidates(query, location, salary_band)
agents/recruitment/sourcing-scout/agent.md:179:   → hh_decision_output("cvlibrary_query", "brief:<id>", "results:<N>")
agents/recruitment/sourcing-scout/agent.md:185:   → annotate each row with source provenance (e.g., "from Bullhorn + LinkedIn
agents/recruitment/sourcing-scout/agent.md:218:     "source:<bullhorn|linkedin|reed|cvlibrary>; confidence:<N>; voice_score:<N>; included:<bool>") — emitted PER CANDIDATE per §3 contract (one row per candidate proposed; dropped candidates also get a row with included=false + drop_reason)
agents/recruitment/sourcing-scout/agent.md:249:- **"each has a working contact method"** (email format + MX check OR E.164 phone OR LinkedIn URL OR Bullhorn bullhorn_id-with-contact)
agents/recruitment/sourcing-scout/agent.md:261:**Honesty note (per bilateral-disposition Cat-5):** Sourcing Scout `validate.sh` does NOT exist yet — this scaffold describes the intended Gate A contract for the W9 build slice. The W9 build delivers `agents/recruitment/sourcing-scout/validate.sh` against the contract above. Current text is the spec the build slice implements against, not a description of running code.
agents/recruitment/sourcing-scout/agent.md:267:Measured via consultant feedback loop: each candidate in a Sourcing Scout report gets a "useful / not useful" tag from the consultant via Brain UI v1.1 OR Telegram reply (v1.0). Aggregate over rolling 30-day window per tenant. NOTE: Bullhorn-note feedback is NOT a v1.0 path (Sourcing Scout is read-only on Bullhorn per §1 + §6 contracts; would require write capability not in v1.0 scope).
agents/recruitment/sourcing-scout/agent.md:281:| `ESC_BULLHORN_AUTH` | Bullhorn OAuth refresh fails | **blocking** (per catalogue §2.3) | operator + ifos_oncall |
...
agents/recruitment/sourcing-scout/agent.md:295:- `ESC_BULLHORN_WRITE_FAIL` — Bullhorn read-only
agents/recruitment/sourcing-scout/agent.md:326:| Janitor ratified (Bullhorn-read substrate) | W5 Codex Round | ⏸ |
agents/recruitment/sourcing-scout/agent.md:328:| **Bullhorn Sub-decisions A+B Accepted** | Bullhorn partnerships response | ⏸ |
agents/recruitment/sourcing-scout/agent.md:329:| Bullhorn MCP read capability | W3-W4-W5 build chain | ⏸ |
...
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml | sed -n '400,430p;680,725p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "''^## §4|Gate A|validate.sh|hard-fail" docs/decisions/autosend-safety-policy.md agents/_shared/autosend-policy.yaml docs/build-brief/00-MASTER-BRIEF.md docs/specs/ULTRAPLAN.md -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
agents/_shared/autosend-policy.yaml:116:    reason: "Internal audit row written by validate.sh when Gate A enforcement fails. Carries the specific ESC code (e.g. ESC_AGENT_OUTPUT_SHAPE | ESC_PII_LEAKAGE_RISK) in the payload; the action itself is just the audit-row write, not an external send. Each agent's validate.sh emits this row before exit 1."
...
docs/specs/ULTRAPLAN.md:342:**Layer 3 — Validation (validate.sh).**
...
docs/decisions/autosend-safety-policy.md:34:- Vault local writes by the agent's own validate.sh / context.sh / fixture runs
