...
session id: 019e8811-66f4-7ae1-acfb-ec1cd960b501
--------
user
...
codex
I’m checking the connector against the ratification rules and cross-references, especially the capability surface and autosend policy claims that aren’t fully contained in the artefact.
exec
...
exec
/bin/zsh -lc 'rg -n "open_banking|open-banking|truelayer|plaid" agents packages/mcp-connectors/open-banking docs/decisions agents/_shared/autosend-policy.yaml agents/_shared/escalation-codes.md' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "rg -n \"console\\.(log|error|warn).*\\"'$'"\\{.*(KEY|SECRET|PASSWORD|TOKEN|BEARER)|throw\\s+new\\s+Error.*\\"'$'"\\{.*(KEY|SECRET|PASSWORD)|composio|agentmail|packages/harness/cortextos|decision_log|migration-test|test-tenant-b\" packages/mcp-connectors/open-banking" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
codex
I found the local references and I’m drilling into the line-level evidence now. The main thing I’m testing is whether the README’s declared guarantees are actually implemented, not just documented.
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/src/client.ts | sed -n '1,230p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/src/auth.ts | sed -n '1,240p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/README.md | sed -n '1,270p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	# @ifos/open-banking
     2	
   ...
   240	---
   241	
   242	*v0.1.0 — scaffold landed 2026-06-01 (W4 Day-26).*

codex
The declared tools and autosend policy line up, but the implementation has a few load-bearing mismatches I’m validating with the package’s own checks.
exec
/bin/zsh -lc 'pnpm --filter @ifos/open-banking typecheck' in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/tests/scaffold.test.ts | sed -n '1,90p'; nl -ba packages/mcp-connectors/open-banking/tests/capabilities.test.ts | sed -n '1,235p'; nl -ba packages/mcp-connectors/open-banking/tests/auth.test.ts | sed -n '1,190p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     ...
     5	  VERSION,
     6	  OpenBankingClient,
     7	  TRUELAYER_API_PROD,
     8	  TRUELAYER_API_SANDBOX,
     9	  loadTokens,
    10	  saveTokens,
    11	  refreshTokens,
    12	  shouldRefresh,
    13	  getTokenAgeStage,
    14	  listTransactionsSince,
    15	  getAccountBalance,
    16	  OpenBankingCache,
    17	  OpenBankingError,
    18	  OpenBankingAuthError,
    19	  OpenBankingConsentExpiredError,
    20	  OpenBankingRateLimitError,
    21	  OpenBankingNotFoundError,
    22	  NotImplementedError,
    23	} from "../src/index.js";
    24	
    25	describe("@ifos/open-banking package surface", () => {
    26	  it("exports VERSION 0.1.0", () => {
    27	    expect(VERSION).toBe("0.1.0");
    28	  });
    29	
    30	  it("exports OpenBankingClient + base URLs (production + sandbox)", () => {
    31	    expect(typeof OpenBankingClient).toBe("function");
    32	    expect(TRUELAYER_API_PROD).toBe("https://api.truelayer.com");
    33	    expect(TRUELAYER_API_SANDBOX).toBe("https://api.truelayer-sandbox.com");
    34	  });
    35	
    36	  it("exports all 2 capability functions", () => {
    37	    expect(typeof listTransactionsSince).toBe("function");
    38	    expect(typeof getAccountBalance).toBe("function");
    39	  });
    40	
    41	  it("exports auth helpers (load/save/refresh/shouldRefresh/getTokenAgeStage)", () => {
    42	    expect(typeof loadTokens).toBe("function");
    43	    expect(typeof saveTokens).toBe("function");
    44	    expect(typeof refreshTokens).toBe("function");
    45	    expect(typeof shouldRefresh).toBe("function");
    46	    expect(typeof getTokenAgeStage).toBe("function");
    47	  });
    48	
    49	  it("exports OpenBankingCache + full error hierarchy including ConsentExpired + NotImplementedError", () => {
    50	    expect(typeof OpenBankingCache).toBe("function");
    51	    expect(new OpenBankingAuthError("x") instanceof OpenBankingError).toBe(true);
    52	    expect(new OpenBankingConsentExpiredError("x") instanceof OpenBankingAuthError).toBe(true);
    53	    expect(new OpenBankingRateLimitError("x") instanceof OpenBankingError).toBe(true);
    54	    expect(new OpenBankingNotFoundError("x") instanceof OpenBankingError).toBe(true);
    55	    expect(new NotImplementedError("x") instanceof OpenBankingError).toBe(true);
    56	  });
    57	});
...
> vitest run
 ...
 ✓ tests/token-aging.test.ts (6 tests) 1ms
 ✓ tests/rate-limit.test.ts (5 tests) 3ms
 ✓ tests/scaffold.test.ts (5 tests) 3ms
 ✓ tests/auth.test.ts (8 tests) 69ms
 ✓ tests/capabilities.test.ts (7 tests) 978ms
   ✓ open-banking capabilities — TrueLayer (v1.0) > listTransactionsSince: persistent 429 surfaces as OpenBankingRateLimitError after retries 312ms
   ✓ open-banking capabilities — TrueLayer (v1.0) > getAccountBalance: persistent 500 surfaces as OpenBankingError after retries 577ms

 Test Files  5 passed (5)
      Tests  31 passed (31)
   Start at  12:22:24
   Duration  1.89s (transform 308ms, setup 0ms, collect 500ms, tests 1.05s, environment 2ms, prepare 473ms)


exec
/bin/zsh -lc 'rg -n "ESC_PROVIDER_FETCH_FAIL|ESC_OPEN_BANKING_AUTH|ESC_OPEN_BANKING_TOKEN_AGING|ESC_RATE_LIMIT_HIT" agents/_shared/escalation-codes.md agents/_shared/autosend-policy.yaml agents/recruitment/cash-conductor/agent.md' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "''^  [A-Z_]+:|ESC_PROVIDER_FETCH_FAIL|PROVIDER_FETCH|OPEN_BANKING|RATE_LIMIT" agents/_shared/escalation-codes.md | sed -n '"'1,220p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/src/types.ts | sed -n '1,90p'; nl -ba packages/mcp-connectors/open-banking/src/transactions.ts | sed -n '1,95p'; nl -ba packages/mcp-connectors/open-banking/src/balance.ts | sed -n '1,70p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
codex
The package’s own typecheck and vitest suite pass. I still have enough concrete spec-level failures for a ratification decision, so I’m preparing the verdict with file/line citations.
exec
/bin/zsh -lc "rg -n \"open_banking|plaid_uk|truelayer|consent_expires|raw_provider_payload|posted_at|available|current\" docs/verticals/recruitment/vertical-schema.yaml docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml docs/verticals/recruitment -g '*.yaml'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba agents/recruitment/cash-conductor/cycle.sh | sed -n '85,135p'; nl -ba agents/recruitment/cash-conductor/agent.md | sed -n '150,175p'; nl -ba agents/recruitment/cash-conductor/agent.md | sed -n '330,355p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
1. Connector still violates the single-upstream-provider contract. `README.md` lines 3, 15, and 222 describe a TrueLayer + Plaid UK abstraction, while `src/types.ts` line 9 exposes `provider: "truelayer" | "plaid_uk"` and `src/client.ts` lines 47-82 carry Plaid routing/stub logic. This fails `review-mcp-connector`’s single-provider premise and top-level §3 “No defensive additions” because Plaid scaffolding has no v1.0 consumer. Remove Plaid from this connector or split it into a future `plaid-uk` connector.

2. The documented 401 retry policy does not match the implementation. `README.md` line 156 says “force-refresh access_token, retry once,” but `src/client.ts` lines 87 and 145-154 allow the 401 branch to refresh on attempt 0 and again on attempt 1 under the default `max_retries = 2`. OAuth refresh is state-changing, so this violates `review-mcp-connector` §4 explicit retry policy. Track a dedicated `didForceRefresh` flag or set the 401 path to one forced refresh regardless of GET retry budget.

3. The README cites the wrong Cash Conductor step for balance reads. `README.md` line 21 and the architecture diagram at lines 218-220 claim `getAccountBalance` is Step 10, but `agents/recruitment/cash-conductor/tools.yaml` lines 121-124 declares `cycle_step: 13`, and `cycle.sh` lines 108-110 routes weekly reports to Step 13. This fails top-level Rule 5 honest signal / citation accuracy. Update the README and source comments to Step 13, or change the agent bundle consistently.
tokens used
57,486
REJECTED
