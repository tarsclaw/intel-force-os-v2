...
session id: 019e5ddb-c088-7110-9aad-59a3213b1b7b
--------
user
...
codex
I’ll verify the cited source files and schema/policy references before deciding, with the agent-bundle checks layered on top of the ratification rules.
exec
/bin/zsh -lc "nl -ba agents/recruitment/janitor/agent.md | sed -n '1,260p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     ...
     2	
     3	**Status:** Pre-Build-Round-12-Bilateral-Applied (Day-20; W4 bilateral pass closed all 4 R11 residuals: 3 closed by post-R11 commit `e5a2c74` (Gate A ESC routing per-condition split, recent_edit.resolution citation, v0.3 supplement §2a authority); Finding 4 closed today by adding `janitor_dedup_threshold` + `janitor_last_run` declarations to v0.3 supplement §4 tenant_adapters_config_additions + agent.md citations updated. v0.3 supplement edit is additive — re-ratification queued. Awaits Q1 LOI + Bullhorn Sub-decisions A+B Accepted + W5 build slice).
     4	**Date:** 2026-05-24.
    ...
    42	- Tenant-admin override for dedup confidence threshold (default 0.85; per-tenant via `tenant_adapters.config.janitor_dedup_threshold` — declared in `vertical-schema.v0.3-supplement.yaml §4 tenant_adapters_config_additions.janitor_dedup_threshold`; enforced by `migrations/v0.2-to-v0.3.sql §5` validator allowlist)
    43	
    44	---
    45	
    46	## §3 — Output shape
    ...
    59	| 4 | **Tacit-note coverage** | Notes harvested from `recent_edit` table rows with `resolution='approved_after_edit'` (per v0.2-supplement.yaml recent_edit definition; v0.3 supplement §2a grants Janitor R access); attached to relevant Bullhorn entities; coverage rate over the 30-day window |
    ...
    95	     tenant_adapters.config.janitor_last_run — declared in vertical-schema
    96	     v0.3 supplement §4 tenant_adapters_config_additions.janitor_last_run;
    97	     enforced by migrations/v0.2-to-v0.3.sql §5 validator allowlist)
    98	   → hh_decision_output("janitor_scan", "tenant:<slug>", "<N> entities scanned")
    99	   → ESC_RATE_LIMIT_HIT if Bullhorn 429 (60s backoff per ESC_RATE_LIMIT_HIT catalogue §2.5 standard handling)
   100	
   101	3. Dedup pass — candidate entity type
   102	   → fuzzy-match across (name, email, phone, linkedin_url) tuples
   103	   → compute confidence per pair: name × 0.3 + email × 0.4 + phone × 0.2 + linkedin × 0.1
   104	   → discard pairs <0.85 confidence (per Gate A)
   105	   → discard pairs where EITHER candidate has Bullhorn activity in last 90d
   106	     (per ULTRAPLAN A2 line 510 verbatim)
   107	   → batch into proposed-merge list (in-memory intermediate; not persisted —
   108	     hh_decision_action emitted at Step 9 when each merge actually writes)
   109	
   110	4. Dedup pass — contractor entity type
   111	   → same algorithm as candidates; separate entity_type per Q1 Day-6 resolution
   112	     (vertical-schema.yaml §1)
   113	
   114	5. Field completeness audit (canonical field names per vertical-schema.yaml)
   115	   → for each entity, check critical fields: candidate.location (line 124),
   116	     client.industry (line 238), client.size_employees (line 243),
   117	     contractor.day_rate_min/day_rate_max (lines 193-197),
   118	     brief.salary_min/salary_max (lines 371-376)
   119	   → identify missing-field rows
   120	   → batch enrichment calls
   121	   → hh_decision_output("field_completeness_audit", tenant, "<N> missing-field rows")
   122	
   123	6. Companies House enrichment (clients only)
   124	   → companies_house.search(client.name per vertical-schema.yaml line 235) → CRN →
   125	     profile → fill canonical schema fields client.industry (line 238),
   126	     client.companies_house_number (line 252)
   127	   → 7-day cache per tools.yaml; rate-limit budget shared with Diagnostic
   128	   → ESC_RATE_LIMIT_HIT on 429
   129	
   130	7. LinkedIn enrichment (candidates; v1.1 via Proxycurl)
   131	   → at v1.0: skipped (LinkedIn deep data deferred to W4 polish; Proxycurl
   132	     signup commercial decision)
   133	   → at v1.1: linkedin.profile_fetch(candidate.linkedin_url) → location +
   134	     current_company → write back
   135	
   136	8. Tacit-note harvest
   137	   → query the `recent_edit` v0.2 table directly (per vertical-schema.v0.2-supplement.yaml
   138	     recent_edit definition; v0.3 supplement §2a grants Janitor R access):
   139	     SELECT FROM recent_edit WHERE resolved_at > now() - interval '30 days'
   140	     AND resolution='approved_after_edit' AND tenant_slug=$tenant
   141	   → join to decision_log only if action-context lookups needed
   142	   → group by target_entity_type (candidate / contractor / contact / brief / etc.)
   143	   → for each group, generate narrative summary via voice-classified LLM
   144	     (voice corpus + tone rules; ESC_VOICE_DRIFT if classifier <0.75)
   145	
   146	9. Bullhorn write batch (yellow tier — spot-check sampling)
   147	   → for each proposed merge / backfill / note: emit hh_decision_action with
   148	     tier='yellow'; spot-check sample rate per autosend-policy.yaml row
   149	   → atomic per-write transaction (BEGIN/COMMIT)
   150	   → on 4xx: emit ESC_BULLHORN_WRITE_FAIL; skip; continue
   151	   → on 5xx: emit ESC_BULLHORN_WRITE_FAIL; retry once with 30s backoff
   152	
   153	10. Day-30 report assembly
   154	   → SELECT from decision_log WHERE agent_name='janitor' AND created_at >
   155	     now() - interval '30 days' AND tenant_slug=$tenant
   156	   → group by action_type; tally success/fail; compute Gate-B metric
   157	   → 8-section Markdown report (per §3 above)
   158	   → write to /vault/<tenant>/janitor-reports/day-30-<ISO-date>.md
   159	   → hh_decision_output("day_30_report", "<path>", "Gate-B score: <N>")
   160	
   161	11. Operator notification (Telegram)
   162	   → if Gate-B target met: green-tier notification with summary
   163	   → if Gate-B target missed: yellow-tier notification + 200-char executive
   164	     summary suggesting consultant follow-up
   165	   → hh_decision_action("operator_notify_telegram", "tenant:<slug>",
   166	     notification_hash, "gate_b_state:met|missed; chars:<N>")
   167	
   168	12. Session close
   169	   → update tenant_adapters.config.janitor_last_run = now()
   170	   → hh_decision_action("janitor_run_complete", "tenant:<slug>", payload_hash, payload_preview)
   171	   → exit code 0 (or 1 if Gate-B missed for 3 consecutive runs → ESC_GATE_B_MISS)
   172	```
   173	
   174	---
   175	
   176	## §5 — Gates
   177	
   178	### Gate A — validate.sh (hard-fail before action)
   179	
   180	Per master brief §8.1 Change 2 + autosend-safety-policy §4. Janitor's `validate.sh` enforces:
   181	
   182	- Bullhorn auth refresh succeeded in Step 1 (no stale token writes)
   183	- Every proposed merge has confidence ≥ 0.85 per ULTRAPLAN A2 line 510
   184	- No merge proposal where EITHER candidate has activity (placement / interview / note) in last 90 days (per ULTRAPLAN A2 line 510 verbatim)
   185	- No field-backfill where source confidence <0.7 (CH 404 / LinkedIn empty / no derivation source)
   186	- Tacit-note narratives pass voice classifier ≥ 0.75
   187	- Bullhorn write batch size ≤ 100 per minute (rate-limit defensive)
   188	- No PII outside firm boundary in tacit-note narratives (regex pass)
   189	
   190	Gate A failure routing by class (per catalogue §2.5):
   191	- PII detected outside firm boundary → `ESC_PII_LEAKAGE_RISK` (blocking; operator + ifos_oncall per catalogue routing)
   192	- Tacit-note voice classifier <0.75 → `ESC_VOICE_DRIFT` (warn; operator_chat_id)
   193	- Tone-rule violations → `ESC_TONE_RULE_VIOLATION` (warn; operator_chat_id per catalogue §2.10)
   194	- Output-shape failures (section count, write-batch size, dedup confidence below threshold for action) → `ESC_AGENT_OUTPUT_SHAPE` (warn; operator_chat_id)
   195	
   196	`ESC_DUPLICATE_DETECTED` (catalogue §2.5) is NOT a Gate A failure code — per catalogue trigger it's for dedup confidence ≥0.85 review-required cases (SUCCESS path; Telegram approval gate). Sub-0.85 confidence pairs silently drop in Step 3 algorithm; no ESC fire. Draft report stays in `/tmp` (not vault); operator-review required. `ESC_SCHEMA_VIOLATION` (catalogue line 163) is NOT used by Janitor — reserved for vertical-schema field-constraint violations at write-time.
   197	
   198	### Gate B — Outcome threshold (success metric, not block)
   199	
   200	Per ULTRAPLAN A2 line 511 verbatim: **"day-30 before/after report shows ≥15% dedup, ≥10% field completeness improvement"**.
   201	
   202	Two independent thresholds (both must pass): dedup improvement ≥15% AND field-completeness improvement ≥10%. NOT a composite — composite would let one cover the other.
   203	
   204	Gate B doesn't block the agent. The day-30 dedup + field-completeness improvement is Janitor's local Gate B metric per ULTRAPLAN A2 line 511 verbatim. It contributes evidence (alongside other agents' Gate-B metrics) to kill-criterion §2 Trigger 8 (average Gate-B revenue uplift after 3 completed pilots per `v1.0-kill-criterion.md` lines 158-166) — but Janitor does NOT directly claim Trigger 8 status. DSO improvement is Cash Conductor's territory per ULTRAPLAN A4 line 540, not Janitor's.
   205	
   206	Failing either threshold for 3 consecutive runs → fire `ESC_GATE_B_MISS` → flag for operator review (heuristic tuning may be needed; not a kill).
   207	
   208	---
   209	
   210	## §6 — Escalation codes
   211	
   212	All codes are registered in `agents/_shared/escalation-codes.md` (catalogue extended to 52 codes per `2026-05-24` bilateral disposition; see disagreement-doc + `catalogue(bilateral)` commit).
   213	
   214	| Code | Trigger | Severity | Routing |
   215	|---|---|---|---|
   216	| `ESC_BULLHORN_AUTH` | OAuth refresh fails after 2 retries | **blocking** | operator + ifos_oncall |
   217	| `ESC_BULLHORN_WRITE_FAIL` | Bullhorn 4xx/5xx on merge/backfill/note write | warn | operator_chat_id |
   218	| `ESC_RATE_LIMIT_HIT` | Bullhorn or Companies House 429 | warn | operator_chat_id |
   219	| `ESC_VOICE_DRIFT` | Tacit-note narrative voice classifier <0.75 (after 3 retries) | warn | operator_chat_id |
   220	| `ESC_PII_LEAKAGE_RISK` | PII detected in tacit-note outside firm boundary | **blocking** | operator + ifos_oncall |
   221	| `ESC_AGENT_OUTPUT_SHAPE` | Gate A failure (section count or per-section citation missing in day-30 report) | warn | operator_chat_id |
   222	| `ESC_DUPLICATE_DETECTED` | Per catalogue §2.5 trigger: dedup confidence ≥0.85 review-required cases (SUCCESS path; Telegram approval gate fires). NOT a Gate A failure code. | warn | operator_chat_id (via Telegram approval gate per catalogue routing) |
   223	| `ESC_GATE_B_MISS` | Per catalogue §2.10 trigger: per-agent local Gate B metric threshold missed for the per-agent window. For Janitor: dedup-improvement <15% OR field-completeness-improvement <10% (two independent thresholds; missing EITHER triggers; per §5 Gate B). Catalogue routing: operator_chat_id | warn | operator_chat_id |
   224	| `ESC_AUTOSEND_SAMPLED_SPOT_CHECK` | Yellow-tier sample row selected for spot-check | info | operator_chat_id |
   225	
   226	Janitor does NOT use:
   227	
   228	- `ESC_VOICE_DRIFT_TENANT` — fired by nightly voice-drift cron (per `escalation-codes.md` line 170-175 trigger: ≥N `ESC_VOICE_DRIFT` rows from same tenant in rolling 7d window). Janitor only fires the per-run `ESC_VOICE_DRIFT`; aggregate `_TENANT` rollup is handled by the canary not Janitor.
   229	- `ESC_AUTOSEND_BLOCKED` — that's for red-tier blocks (`escalation-codes.md` line 41); Janitor writes are yellow tier only
   230	- `ESC_BULLHORN_OAUTH_REVOKED` — escalated handling; Concierge owns the cross-agent escalation
   231	- `ESC_SCHEMA_VIOLATION` (line 163) — that's for vertical-schema field-constraint violations at write-time; Janitor's Gate A failures map to `ESC_AGENT_OUTPUT_SHAPE` (output-shape constraint) instead, per the catalogue's intended-use distinction
   232	
   233	---
   234	
   235	## §7 — Voice + tone constraints
   236	
   237	Step 8 (tacit-note narrative generation) is the only voice-classified output. The agent integrates with `_shared/voice-loader.sh`:
   238	
   239	- **`hh_load_tone_rules` filtered by `applies_to_agents` containing `janitor`** — surfaces rules like:
   240	  - No identifying language about candidates beyond what's in their CV / Bullhorn record
   241	  - No commercial sensitive information (rates / placement fees / commission %)
   242	  - No external-party PII (clients of clients)
   243	- **`hh_load_voice_samples` ANN query against tenant voice_corpus**: top-5 chunks matching "internal note summary" task context. Feeds LLM prompt as voice exemplars.
   244	- **`hh_load_recent_edits` last 30 days for `janitor` agent**: detects if consultants are heavily editing Janitor's tacit-note drafts. Per-run `ESC_VOICE_DRIFT` fires when the tacit-note voice classifier score is below 0.75 after 3 retries (Janitor emits per-run). Aggregate `ESC_VOICE_DRIFT_TENANT` is fired by the nightly voice-drift cron per `escalation-codes.md` line 170-175 trigger (≥N `ESC_VOICE_DRIFT` rows from the same tenant within a rolling 7d window); Janitor does NOT fire `_TENANT` directly. Edit-distance metrics are tracked separately for analytics but do NOT fire ESC codes — they inform the canary's threshold tuning over time.
   245	
   246	Per master brief §8.1 Change 1: voice is per-tenant; never cross-tenant.
   247	
   248	---
   249	
   250	## §8 — Build dependencies (W5 prerequisites)
   251	
   252	Janitor build cannot start until ALL of the following are confirmed:
   253	
   254	| Dependency | Source | Status |
   255	|---|---|---|
   256	| Renderer + `_shared/` substrate | Day-8 + Round-3 ratified | ✅ |
   257	| Diagnostic ratified (first-agent precedent) | Week 3 Codex Round 4 Phase 1 | ⏸ Week 3 in progress |
   258	| First pilot tenant onboarded (provision-tenant.sh) | Post Q1-LOI | ⏸ Founder action; Trigger 1 fires 2026-06-03 if no LOI |
   259	| **Bullhorn Sub-decision A Accepted** | Bullhorn partnerships response | ⏸ Form submitted 2026-05-24; 2-5 business days |
   260	| **Bullhorn Sub-decision B Accepted** | Bullhorn developer support routing | ⏸ Same |

exec
/bin/zsh -lc 'rg -n "bullhorn_candidate_dedupe|bullhorn_field_backfill|bullhorn_note_attach|ESC_BULLHORN_AUTH|ESC_BULLHORN_WRITE_FAIL|ESC_RATE_LIMIT_HIT|ESC_VOICE_DRIFT|ESC_PII_LEAKAGE_RISK|ESC_AGENT_OUTPUT_SHAPE|ESC_DUPLICATE_DETECTED|ESC_GATE_B_MISS|ESC_AUTOSEND_SAMPLED_SPOT_CHECK|ESC_VOICE_DRIFT_TENANT|ESC_SCHEMA_VIOLATION|ESC_TONE_RULE_VIOLATION" agents/_shared docs -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
docs/operations/goal-week-3-polish-and-scaffold.md:430:- **§7 Voice + tone:** highest stakes of any agent — customer-facing sends. Voice classifier required (no skip). Tone rules strict (block-severity hit = ESC_TONE_RULE_VIOLATION).
...
exec
/bin/zsh -lc "rg -n \"§8\\.2|Janitor|A2|day-30|dedup|field completeness|Trigger 3|Trigger 8|Companies House|recent_edit|janitor_dedup_threshold|janitor_last_run|candidate:|contractor:|client:|brief:\" docs agents migrations -S" in /Users/madsadmin/code/CortexOS
 exited 2 in 0ms:
...
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:460:      - Diagnostic (R)     # v0.3 NEW — reads Companies House data for sales-tool context
...
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:799:      Per-tenant dedup confidence threshold for candidate merges. v1.0 default
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:800:      0.85 per ULTRAPLAN A2 line 510. Tenant-admin override range [0.75, 0.95]
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:803:      authority added in W4 bilateral pass (Day-20) closing Janitor R11
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:806:  janitor_last_run:
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:812:      Janitor nightly cron updates at session-close (Step 12). Next run queries
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:816:      (Day-20) closing Janitor R11 Codex Finding 4.
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:857:    - v0.2 migration applied (voice_corpus + voice_corpus_chunks + tone_rule + recent_edit tables exist)
...
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:939:      A: Maintain 6 states; manually map at Janitor dedup time (Bullhorn → IFOS).
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:982:  4-of-5 remaining agent.md scaffolds (Janitor, Scribe, Cash Conductor,
docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml:1003:    - Janitor candidate.linkedin_url + recent_edit/tone_rule access valid
agents/_shared/tests/test-hook-helpers.sh:119:test_lookup_yellow() { local t; t=$(autosend_policy_lookup "bullhorn_candidate_dedupe"); _assert_eq "yellow" "${t}" "yellow"; }
...
agents/recruitment/scribe/agent.md:40:v1.0 supports Fathom (HMAC-SHA256 signed) and Fireflies (bearer token) providers; auth handled per-provider in `tools.yaml` capability declarations. Ringover (OAuth-protected) is deferred to v1.1+ — not in the v0 build dependencies and not registered in v1.0 `tools.yaml`.
agents/recruitment/scribe/agent.md:133:     corpus (for tacit-note voice) + tone rules + recent_edits (drift)
agents/recruitment/scribe/agent.md:243:- **Quality:** consultant edit-rate ≤20% on structured fields (measured via `recent_edit` rows for `agent_name='scribe'`)
agents/recruitment/scribe/agent.md:245:Gate B doesn't block individual runs. Tracked monthly via day-30 metrics roll-up (similar to Janitor's day-30 report; Scribe metrics merge into the tenant's monthly executive summary).
...
agents/recruitment/scribe/agent.md:300:| Janitor ratified (Bullhorn auth substrate via W5 build) | W5 Codex Round | ⏸ |
agents/recruitment/scribe/agent.md:333:| Q6 | Consultant edit-rate ≤20% metric — how to measure when consultants edit Bullhorn entities outside our `recent_edit` audit path? | Use Bullhorn's audit log API + cross-reference with our writes. Founder approve approach at W6 design review. |
...
agents/recruitment/sourcing-scout/agent.md:185:7. Aggregate + dedupe
agents/recruitment/sourcing-scout/agent.md:187:   → dedupe across sources by (name + email) OR (name + phone) OR
agents/recruitment/sourcing-scout/agent.md:188:     (LinkedIn URL) — same fuzzy matcher as Janitor (confidence ≥0.85)
agents/recruitment/sourcing-scout/agent.md:191:   → hh_decision_output("aggregate_dedupe", "brief:<id>",
agents/recruitment/sourcing-scout/agent.md:192:     "pre_dedupe:<N>; post_dedupe:<N>")
agents/recruitment/sourcing-scout/agent.md:212:   → hh_decision_output("dnc_filter", "brief:<id>",
...
agents/recruitment/sourcing-scout/agent.md:331:| Janitor ratified (Bullhorn-read substrate) | W5 Codex Round | ⏸ |
...
agents/recruitment/janitor/agent.md:106:     (per ULTRAPLAN A2 line 510 verbatim)
agents/recruitment/janitor/agent.md:123:6. Companies House enrichment (clients only)
agents/recruitment/janitor/agent.md:137:   → query the `recent_edit` v0.2 table directly (per vertical-schema.v0.2-supplement.yaml
agents/recruitment/janitor/agent.md:138:     recent_edit definition; v0.3 supplement §2a grants Janitor R access):
agents/recruitment/janitor/agent.md:139:     SELECT FROM recent_edit WHERE resolved_at > now() - interval '30 days'
agents/recruitment/janitor/agent.md:158:   → write to /vault/<tenant>/janitor-reports/day-30-<ISO-date>.md
agents/recruitment/janitor/agent.md:169:   → update tenant_adapters.config.janitor_last_run = now()
agents/recruitment/janitor/agent.md:180:Per master brief §8.1 Change 2 + autosend-safety-policy §4. Janitor's `validate.sh` enforces:
agents/recruitment/janitor/agent.md:183:- Every proposed merge has confidence ≥ 0.85 per ULTRAPLAN A2 line 510
agents/recruitment/janitor/agent.md:184:- No merge proposal where EITHER candidate has activity (placement / interview / note) in last 90 days (per ULTRAPLAN A2 line 510 verbatim)
...
agents/recruitment/janitor/agent.md:196:`ESC_DUPLICATE_DETECTED` (catalogue §2.5) is NOT a Gate A failure code — per catalogue trigger it's for dedup confidence ≥0.85 review-required cases (SUCCESS path; Telegram approval gate). Sub-0.85 confidence pairs silently drop in Step 3 algorithm; no ESC fire. Draft report stays in `/tmp` (not vault); operator-review required. `ESC_SCHEMA_VIOLATION` (catalogue line 163) is NOT used by Janitor — reserved for vertical-schema field-constraint violations at write-time.
agents/recruitment/janitor/agent.md:200:Per ULTRAPLAN A2 line 511 verbatim: **"day-30 before/after report shows ≥15% dedup, ≥10% field completeness improvement"**.
agents/recruitment/janitor/agent.md:202:Two independent thresholds (both must pass): dedup improvement ≥15% AND field-completeness improvement ≥10%. NOT a composite — composite would let one cover the other.
agents/recruitment/janitor/agent.md:204:Gate B doesn't block the agent. The day-30 dedup + field-completeness improvement is Janitor's local Gate B metric per ULTRAPLAN A2 line 511 verbatim. It contributes evidence (alongside other agents' Gate-B metrics) to kill-criterion §2 Trigger 8 (average Gate-B revenue uplift after 3 completed pilots per `v1.0-kill-criterion.md` lines 158-166) — but Janitor does NOT directly claim Trigger 8 status. DSO improvement is Cash Conductor's territory per ULTRAPLAN A4 line 540, not Janitor's.
agents/recruitment/janitor/agent.md:218:| `ESC_RATE_LIMIT_HIT` | Bullhorn or Companies House 429 | warn | operator_chat_id |
...
agents/recruitment/janitor/agent.md:222:| `ESC_DUPLICATE_DETECTED` | Per catalogue §2.5 trigger: dedup confidence ≥0.85 review-required cases (SUCCESS path; Telegram approval gate fires). NOT a Gate A failure code. | warn | operator_chat_id (via Telegram approval gate per catalogue routing) |
...
agents/recruitment/janitor/agent.md:226:Janitor does NOT use:
agents/recruitment/janitor/agent.md:228:- `ESC_VOICE_DRIFT_TENANT` — fired by nightly voice-drift cron (per `escalation-codes.md` line 170-175 trigger: ≥N `ESC_VOICE_DRIFT` rows from same tenant in rolling 7d window). Janitor only fires the per-run `ESC_VOICE_DRIFT`; aggregate `_TENANT` rollup is handled by the canary not Janitor.
agents/recruitment/janitor/agent.md:229:- `ESC_AUTOSEND_BLOCKED` — that's for red-tier blocks (`escalation-codes.md` line 41); Janitor writes are yellow tier only
...
agents/recruitment/janitor/agent.md:252:Janitor build cannot start until ALL of the following are confirmed:
agents/recruitment/janitor/agent.md:263:| Companies House MCP connector | Day-13 shipped (`@ifos/companies-house`) | ✅ |
agents/recruitment/janitor/agent.md:272:**Until ALL ⏸ items resolve to ✅, W5 build slice does not start.** Per kill-criterion §2 Trigger 3 (JANITOR-BULLHORN-AUTH-W5): if Bullhorn auth not cleared by end of W5, Janitor + Scribe defer to W7-8 per ULTRAPLAN §10 Risk #2 contingency.
...
agents/recruitment/janitor/agent.md:287:| Q4 | Tacit-note attribution — should notes attribute to "Intel Force OS Janitor" or just "Internal note"? Tenant brand preference. | Per-tenant config at first-pilot onboarding. |
agents/recruitment/janitor/agent.md:291:### Gotchas (carried forward from ULTRAPLAN A2 line 513)
agents/recruitment/janitor/agent.md:293:1. **Bullhorn MCP server doesn't exist yet — this is the critical-path build for v1.0.** Estimate 1 week for the MCP server, 1 week for the agent itself (per ULTRAPLAN A2 line 513).
agents/recruitment/janitor/agent.md:295:3. **Bullhorn webhook coverage is patchy** (per ULTRAPLAN A6 line 569 — Concierge note applies cross-agent). Janitor relies on polling not webhooks; safer for nightly cron pattern.
agents/recruitment/janitor/agent.md:316:*End of Janitor agent.md draft.*
...
agents/recruitment/diagnostic/cycle.sh:135:# Load Companies House API key from tenant secrets if present
docs/_supplementary/PRD-autonomous-agent.md:766:    candidate: c,
docs/_supplementary/PRD-autonomous-agent.md:1106:export async function critiqueBrief(brief: ProductionBrief): Promise<CritiqueResult> {
docs/_supplementary/PRD-autonomous-agent.md:1166:  brief: ProductionBrief
docs/_supplementary/PRD-autonomous-agent.md:1352:        defaultProps={{ brief: {} as ProductionBrief }}
docs/_supplementary/PRD-autonomous-agent.md:1361:        defaultProps={{ brief: {} as ProductionBrief }}
docs/_supplementary/PRD-autonomous-agent.md:1370:        defaultProps={{ brief: {} as ProductionBrief }}
docs/_supplementary/PRD-autonomous-agent.md:1379:        defaultProps={{ brief: {} as ProductionBrief }}
docs/_supplementary/PRD-autonomous-agent.md:1395:  brief: ProductionBrief,
docs/_supplementary/PRD-autonomous-agent.md:1451:  brief: ProductionBrief;
docs/_supplementary/PRD-autonomous-agent.md:1504:  brief: ProductionBrief;
docs/_supplementary/PRD-autonomous-agent.md:1589:export async function resolveAssets(brief: ProductionBrief): Promise<ProductionBrief> {
docs/_supplementary/PRD-autonomous-agent.md:1665:  brief: ProductionBrief,
docs/_supplementary/PRD-autonomous-agent.md:1677:function buildReviewCaption(brief: ProductionBrief, critique: CritiqueResult): string {
docs/_supplementary/PRD-autonomous-agent.md:1707:export async function postToDiscord(brief: ProductionBrief, videoPath: string): Promise<void> {
docs/_supplementary/PRD-autonomous-agent.md:1741:export async function processApproval(brief: ProductionBrief): Promise<void> {
docs/_supplementary/PRD-autonomous-agent.md:1760:  brief: ProductionBrief,
...
agents/recruitment/diagnostic/agent.md:51:| 1 | **Firm signal** | Companies House data: registered name, company number, incorporation date, latest filed accounts (revenue band + headcount band), registered office, recent director changes, share-class moves | Companies House API ✅ | (no change) |
agents/recruitment/diagnostic/agent.md:53:| 3 | **Sector + role-type mix** | Sectors actively recruiting for; ratio of permanent vs contract roles in last 90 days; technical-vs-commercial-vs-operational split | **v0: stub with Companies House fallback citation** | LinkedIn job posts + careers-page scrape |
agents/recruitment/diagnostic/agent.md:54:| 4 | **Geography** | Office locations + current hiring locations + remote-vs-onsite-vs-hybrid mix | Companies House registered office ✅ | + LinkedIn job posts location field |
...
agents/recruitment/diagnostic/agent.md:59:| 9 | **Competitor positioning** | Other recruitment firms visible in the candidate flow (previous-employer agencies; @firm tags; "Who's hiring this firm" inference) | **v0: stub with Companies House fallback** | LinkedIn profile + agency-tag search |
agents/recruitment/diagnostic/agent.md:60:| 10 | **Recent activity** | LinkedIn company posts in last 90 days; press releases or news mentions; funding events visible on Companies House | Companies House filing history + web search ✅ | + LinkedIn company-page posts |
agents/recruitment/diagnostic/agent.md:61:| 11 | **Decision-maker map** | Named people likely to be buyers: head of talent / CPO / hiring manager equivalents; LinkedIn profile URL per person; tenure | **v0: stub with Companies House director list** | LinkedIn employee search filtered by title |
agents/recruitment/diagnostic/agent.md:97:     - §1 Companies House lookup (cached 7d per gotcha §6.2)
...
agents/recruitment/diagnostic/agent.md:215:| Companies House MCP connector | `packages/mcp-connectors/companies-house/` | ✅ Shipped Day 13; 13/13 tests passing |
agents/recruitment/diagnostic/agent.md:240:| Q3 | Proxycurl vs alternative LinkedIn API surface? Cost + ToS implications. | v0 (Day 13 ship) skipped Proxycurl: LinkedIn deep-data sections (§3, §5, §7, §9, §11) use Companies House fallback citations per §3 table (status confirmed in §8 dependencies row "LinkedIn read-only via web-scraper" — Proxycurl deferred to W4). W4 polish decision path: founder evaluates commercial Proxycurl signup ($39/mo Proxycurl pricing low-volume) vs alternative (PhantomBuster, Bright Data, scraper.api) vs continued Companies House fallback. Resolution due before first pilot tenant onboarding (Risk #3 LOI close). |
agents/recruitment/diagnostic/agent.md:242:| Q5 | What's the "firm-slug" canonical form for the output filename? Companies House registration number? URL-slugified firm name? | Recommend Companies House number for stability; canonical-slug fallback for non-UK firms (v1.1+). |
...
agents/recruitment/diagnostic/validate.sh:208:# At W3 build: extract firm domain from Companies House link to scope
...
agents/recruitment/concierge/agent.md:293:- **Send-as-is rate:** % of drafts approved by consultant without edits (consultant clicks "approve" not "edit-and-approve"). Target ≥60%. Measured via `recent_edit` rows with `resolution='approved_verbatim'` vs `approved_after_edit`.
...
agents/recruitment/concierge/agent.md:364:| Janitor + Scribe ratified (Bullhorn R+W substrate) | W5-W6 Codex Rounds | ⏸ |
...
docs/operations/goal-week-3-polish-and-scaffold.md:165:#### Step 2 — Founder gate: Companies House API key registration
...
docs/operations/goal-week-3-polish-and-scaffold.md:313:- **§9 Open questions:** 4-6 questions covering dedup-heuristic confidence threshold, field-completeness priority order, Bullhorn write batch size, day-30 report distribution path.
...
docs/operations/goal-week-3-polish-and-scaffold.md:504:| Companies House API key returns 401 (invalid key) | Founder re-registers (free; key may be revoked if not used in N days per CH policy); retry after new key saved |
docs/operations/goal-week-3-polish-and-scaffold.md:525:- Step 2: Companies House API key registration
...
codex
The first pass shows several citations need closer checking: policy filenames differ in places, and some line anchors appear likely stale. I’m narrowing this to blockers only, especially schema fields, autosend tier assignments, and required workflow helper calls.
exec
/bin/zsh -lc "nl -ba agents/recruitment/janitor/agent.md | sed -n '260,380p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   260	| **Bullhorn Sub-decision B Accepted** | Bullhorn developer support routing | ⏸ Same |
   261	| Bullhorn MCP connector built | W3-W4 conditional (per ADR-005 sequencing) | ⏸ Not started |
   262	| Bullhorn client_id + client_secret obtained | Tenant pilot OAuth ticket (or marketplace credentials if A=marketplace) | ⏸ Post-A+B Accept |
   263	| Companies House MCP connector | Day-13 shipped (`@ifos/companies-house`) | ✅ |
   264	| Tenant `target_patch.json` + `_secrets.env` provisioned | provision-tenant.sh ran for first pilot | ⏸ Post-LOI |
   265	| Voice corpus seeded for first pilot tenant | Tenant-admin onboarding | ⏸ Post-LOI |
   266	| `validate.sh` Gate A logic | Build at W5 start (~0.5 day) | ⏸ |
   267	| `context.sh` hydration | Build at W5 start (~0.5 day) | ⏸ |
   268	| `cycle.sh` orchestration (12-step) | Build at W5 start (~2 days) | ⏸ |
   269	| Dedup heuristic + confidence scorer | Build at W5 start (~3 days) | ⏸ |
   270	| 3 fixtures with golden outputs | Build at W5 start (~1 day) | ⏸ |
   271	
   272	**Until ALL ⏸ items resolve to ✅, W5 build slice does not start.** Per kill-criterion §2 Trigger 3 (JANITOR-BULLHORN-AUTH-W5): if Bullhorn auth not cleared by end of W5, Janitor + Scribe defer to W7-8 per ULTRAPLAN §10 Risk #2 contingency.
   273	
   274	---
   275	
   276	## §9 — Status + open questions
   277	
   278	**Status:** Proposed. Awaits Bullhorn A+B Accepted + Q1 LOI + W5 build slice start.
   279	
   280	### Open questions for founder review
   281	
   282	| # | Question | Resolution path |
   283	|---|---|---|
   284	| Q1 | Dedup confidence threshold default — 0.85 per ULTRAPLAN A2 line 510, but should this be per-tenant overridable via `tenant_adapters.config.janitor_dedup_threshold`? | Founder review at agent.md ratification. Recommend: default 0.85; per-tenant override [0.75, 0.95]. |
   285	| Q2 | Field-completeness priority order — which missing fields are highest-impact to backfill first? | Founder review with first pilot tenant; varies by tenant focus (perm vs contract). |
   286	| Q3 | Bullhorn write batch size — current default 100/min. Bullhorn published rate-limit isn't documented; we may need to negotiate. | Bullhorn commercial conversation Sub-decision B output. |
   287	| Q4 | Tacit-note attribution — should notes attribute to "Intel Force OS Janitor" or just "Internal note"? Tenant brand preference. | Per-tenant config at first-pilot onboarding. |
   288	| Q5 | Day-30 report distribution path — vault only OR also forwarded to tenant's hiring leader via email? | v1.0: vault only. v1.1: opt-in email forward via Concierge. |
   289	| Q6 | Gate B exception handling — what if a tenant's day-0 baseline is already exceptionally clean (low headroom for improvement)? | Founder review at first 3-tenant cohort; may need per-tenant Gate B calibration. |
   290	
   291	### Gotchas (carried forward from ULTRAPLAN A2 line 513)
   292	
   293	1. **Bullhorn MCP server doesn't exist yet — this is the critical-path build for v1.0.** Estimate 1 week for the MCP server, 1 week for the agent itself (per ULTRAPLAN A2 line 513).
   294	2. **Dedup is hard; start conservative.** High-confidence merges only (≥0.85); tune up the threshold over time as data builds.
   295	3. **Bullhorn webhook coverage is patchy** (per ULTRAPLAN A6 line 569 — Concierge note applies cross-agent). Janitor relies on polling not webhooks; safer for nightly cron pattern.
   296	
   297	---
   298	
   299	## §10 — When this document ratifies
   300	
   301	Per `.codex/ratification/review-agent-bundle.md` skill (built Day 19, commit `825ebd4`): this agent.md ratifies when Codex Round 4 Phase 2 (Day 20) returns RATIFIED verdict.
   302	
   303	Status flips Proposed → Accepted (pre-build) when:
   304	- Codex Round 4 Phase 2 ratifies
   305	- Founder approves §9 open questions Q1 + Q2 + Q4 + Q5 + Q6
   306	- Q3 resolves via Bullhorn Sub-decision B answer
   307	
   308	Status flips Accepted → In Force when:
   309	- W5 build slice produces all 5 sibling bundle files + 3 fixtures
   310	- First production run against migration-test tenant succeeds (per ADR-003 §4 + ADR-004 Decision 7 audit row)
   311	- Day-30 baseline measured for first pilot tenant
   312	- Codex re-ratifies post-build via `review-agent-bundle.md` skill (when built)
   313	
   314	Until then: this document is a forward-looking scaffold. Conservative pre-build clarity — not a binding contract until ratification.
   315	
   316	*End of Janitor agent.md draft.*

exec
/bin/zsh -lc "nl -ba docs/specs/ULTRAPLAN.md | sed -n '500,514p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   500	
...
exec
/bin/zsh -lc "nl -ba docs/build-brief/00-MASTER-BRIEF.md | sed -n '588,606p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   588	
...
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '140,170p' && nl -ba docs/decisions/autosend-safety-policy.md | sed -n '86,96p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/vertical-schema.v0.2-supplement.yaml | sed -n '300,360p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/vertical-schema.yaml | sed -n '112,130p;186,200p;232,255p;365,378p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/vertical-schema.v0.3-supplement.yaml | sed -n '780,820p;700,730p;570,590p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   570	  tone_rule:
...
exec
/bin/zsh -lc "find . -path '*v0.2-to-v0.3.sql' -o -path '*autosend-safety-policy.yaml' -o -path '*review-agent-bundle.md' | sort" in /Users/madsadmin/code/CortexOS
 succeeded in 339ms:
./.codex/ratification/review-agent-bundle.md
./docs/verticals/recruitment/migrations/v0.2-to-v0.3.sql

exec
/bin/zsh -lc "nl -ba docs/verticals/recruitment/migrations/v0.2-to-v0.3.sql | sed -n '120,190p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba docs/v1.0-kill-criterion.md | sed -n '130,175p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
nl: docs/v1.0-kill-criterion.md: No such file or directory
...
exec
/bin/zsh -lc "nl -ba agents/_shared/voice-loader.sh | sed -n '1,40p;120,260p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	#!/usr/bin/env bash
     2	#
...
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '90,135p;145,190p;305,390p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "rg --files | rg 'kill-criterion|v1\\.0'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "janitor_run_complete|operator_notify_telegram|session_start|janitor_scan|field_completeness_audit|day_30_report" agents/_shared/autosend-policy.yaml agents/_shared -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/_shared/autosend-policy.yaml:65:  operator_notify_telegram:
agents/_shared/autosend-policy.yaml:71:  janitor_run_complete:
agents/_shared/hook-helpers.sh:169:  local trigger_type="${1:-session_start}"
agents/_shared/tests/test-hook-helpers.sh:98:  hh_decision_trigger "session_start" "boot"
agents/_shared/tests/test-hook-helpers.sh:102:    && _assert_contains "${row}" '"trigger_type":"session_start"'

exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '56,76p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba docs/decisions/v1.0-kill-criterion.md | sed -n '130,175p;80,125p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    80	
...
exec
/bin/zsh -lc "nl -ba docs/decisions/v1.0-kill-criterion.md | sed -n '48,82p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    48	
...
exec
/bin/zsh -lc "nl -ba agents/_shared/voice-loader.sh | sed -n '70,118p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    70	}
    71	
    72	# ────────────────────────────────────────────────────────────────────────
...
1. Header status is not an allowed honest-signal state. Line 3 uses `Pre-Build-Round-12-Bilateral-Applied...`, while §9 line 278 says `Status: Proposed`; the ratification rules require a clear Proposed/Accepted/In Force status. Proposed fix: set header `Status: Proposed` and move the round/history text into a non-status note.

2. The autosend policy citation is inaccurate. Line 16 cites `autosend-safety-policy.yaml`, and line 180 cites `autosend-safety-policy §4`, but the shared executable policy in this repo is `agents/_shared/autosend-policy.yaml`; the decision doc is `docs/decisions/autosend-safety-policy.md`, not YAML. Proposed fix: cite the actual policy file/section used for tier dispatch.

3. The output contract is narrower than the workflow. Line 16 names duplicate `candidate` merges only, and §3 line 57 reports candidate dedup pairs only, but §4 lines 110-112 add a contractor dedup pass. Proposed fix: either remove contractor dedup from the W5 scaffold or add contractor dedup to §1, §3, Gate A/B, and audit-row signatures.

4. Step 8 produces an output without a `hh_decision_*` call. Lines 142-144 group edits and generate narrative summaries, but no `hh_decision_output` or `hh_decision_action` is emitted until the later write batch; §4 line 79 requires a helper call at every output/action step. Proposed fix: add an audit row for tacit-note narrative generation or move generation fully inside Step 9’s per-write audited action.

5. The `ESC_GATE_B_MISS` citation does not match the catalogue. Lines 206 and 223 say missing either Janitor threshold triggers the code, but `agents/_shared/escalation-codes.md` §2.10 currently defines the Janitor example as dedup confidence `<15%` AND field-completeness uplift `<10%`. Proposed fix: align the agent.md to the catalogue or update the catalogue first, then cite the updated definition.
tokens used
70,143
REJECTED
