...
session id: 019e8820-3330-7ee3-b861-481f24e38b48
--------
user
...
codex
I’m going to verify the cited source files and catalogue entries against the local repo before giving the verdict. The likely failure modes here are citation drift, invented action types/ESC codes, and boundary issues in the agent body.
exec
/bin/zsh -lc 'rg -n "ESC_CONCIERGE_SLA_MISS|ESC_ADDRESSEE_MISMATCH|ESC_MS_GRAPH_AUTH|ESC_GMAIL_AUTH|ESC_APPROVAL_BRIDGE_TIMEOUT|ESC_AGENT_OUTPUT_SHAPE|ESC_AUTOSEND_ORANGE_PENDING|ESC_AUTOSEND_SAMPLED_SPOT_CHECK|ESC_SEND_FAIL|ESC_LIFECYCLE_STATE_UNKNOWN|ESC_GATE_B_MISS|ESC_VOICE_DRIFT|ESC_CANDIDATE_DATA_INCOMPLETE|ESC_AUTOSEND_BLOCKED" agents/_shared/escalation-codes.md -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "hh_load_tone_rules|hh_load_voice_samples|hh_load_recent_edits|hh_decision_trigger|hh_decision_output|hh_decision_action" agents/_shared -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'rg -n "Composio|AgentMail|packages/harness/cortextos|cortextOS stock|stock KB|tenant_adapters|recent_edit|tone_rule|voice_corpus|decision_log|concierge_last_poll|approved_verbatim|approved_after_edit" agents/recruitment/concierge/agent.md docs/verticals/recruitment/vertical-schema.yaml docs/verticals/recruitment/* -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/recruitment/concierge/agent.md:123:     voice corpus + tone rules + recent_edits + tenant comms-template
...
agents/recruitment/concierge/agent.md:348:- **`hh_load_tone_rules` filtered by `applies_to_agents` containing `concierge`** — surfaces rules like:
agents/recruitment/concierge/agent.md:355:- **`hh_load_voice_samples` ANN query against tenant voice_corpus**: top-5 chunks matching the specific event_type (e.g., "rejection email" task context surfaces rejection-style samples).
agents/recruitment/concierge/agent.md:356:- **`hh_load_recent_edits` last 30 days for `concierge` agent**: drift signal. Per-run `ESC_VOICE_DRIFT` fires when a draft's voice classifier score is below the position-specific threshold after 3 retries. Aggregate `ESC_VOICE_DRIFT_TENANT` is fired by the nightly voice-drift cron per `escalation-codes.md` §2.5 (≥N `ESC_VOICE_DRIFT` rows from the same tenant in rolling 7d window); Concierge does NOT fire `_TENANT` directly. Edit-distance metrics are tracked for analytics; they inform the canary's threshold tuning but do not fire ESC codes from Concierge.
agents/recruitment/concierge/agent.md:388:| Tenant tone_rule table seeded for concierge | Tenant-admin | ⏸ |
...
exec
/bin/zsh -lc 'rg -n "Concierge|A6|W10|candidate-ghosted|30-min|no candidates emailed|biggest v1.0|Trigger 3|D1|concierge_approval_routed|gmail_outlook_send_to_candidate|bullhorn_note_customer_visible|twilio_sms_send|calendar_invite_send|concierge_email_draft" docs agents packages -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/_shared/autosend-policy.yaml:248:  concierge_email_draft:
agents/_shared/autosend-policy.yaml:252:    reason: "Concierge drafts customer-facing email per lifecycle event (acknowledgement / prep / debrief / rejection / placement / nurture etc); draft is internal (not yet sent); the actual customer-facing send is a separate orange-tier action_type (gmail_outlook_send_to_candidate or bullhorn_note_customer_visible). Voice-classified at draft time; sample for ongoing quality review"
agents/_shared/autosend-policy.yaml:259:  bullhorn_note_customer_visible:
agents/_shared/autosend-policy.yaml:267:  gmail_outlook_send_to_candidate:
agents/_shared/autosend-policy.yaml:274:  twilio_sms_send:
agents/_shared/autosend-policy.yaml:281:  calendar_invite_send:
...
packages/harness/cortextos/src/hooks/hook-crash-alert.ts:194:  // reclassify as rate-limited. Prevents the 30-minute 🚨 CRASH buzz storm
...
packages/harness/cortextos/src/daemon/index.ts:22://      (with a 30-min cooldown). PM2's max_restarts: 10 is the final
...
agents/recruitment/cash-conductor/agent.md:259:      orange-approval-bridge; Concierge handles autosend-bridge call to
agents/recruitment/cash-conductor/agent.md:260:      operator (D1 path per Founder Decision)
agents/recruitment/cash-conductor/agent.md:262:11. (Operator approves via Concierge's bridge → Concierge transports the
agents/recruitment/cash-conductor/agent.md:267:      Concierge does NOT write its own action row for this — Concierge's
agents/recruitment/cash-conductor/agent.md:269:    → Concierge fires webhook back: chase_sent (transport completed)
...
agents/recruitment/scribe/agent.md:221:     30-min/1h is the alerting threshold (less false alarms).
...
docs/operations/founder-legal-setup-guide.md:47:| 1 | Free 30-min consultation with 2-3 UK SaaS solicitors. Goal: understand if SeedLegals templates are sufficient for *your* specific data-processing posture, OR if you need bespoke drafting. Most solicitors offer a free first call. | £0 | ~1.5 hours of your time across 2-3 calls |
docs/operations/founder-legal-setup-guide.md:61:| 1 | Free 30-min consultation with 2-3 UK tech/SaaS solicitors | £0 | ~1.5 hours of your time |
docs/operations/founder-legal-setup-guide.md:116:Bring this list. Most solicitors will give you a 30-min free initial call. Cover as many as you can.
docs/operations/founder-legal-setup-guide.md:151:2. Email 3-4 solicitors from §3 above asking for a 30-minute free consultation. Template email below.
docs/operations/founder-legal-setup-guide.md:196:> Would you be open to a 30-minute free initial consultation in the next week to discuss whether your firm could support our setup, your hourly rates, and your view on the right approach?
...
agents/recruitment/concierge/fixtures/01-primary.yaml:9:# Step 15 records the SLA hit (well under 30-min Gate B threshold).
...
agents/recruitment/concierge/fixtures/01-primary.yaml:117:    sla_elapsed_seconds: 600   # event → send under the 30-min (1800s) Gate B threshold
...
agents/recruitment/concierge/cycle.sh:156:# "correct addressee resolution; no candidates emailed under another's name".
...
agents/recruitment/concierge/tools.yaml:201:#   bullhorn_note_customer_visible     orange              [canonical orange]
...
agents/recruitment/concierge/agent.md:107:- `bullhorn_note_customer_visible` (orange tier; canonical orange per autosend-policy.yaml §ORANGE) when channel=Bullhorn note with isExternal=true
...
agents/recruitment/concierge/agent.md:168:     "correct addressee resolution; no candidates emailed under another's name")
...
agents/recruitment/concierge/agent.md:248:    → hh_decision_action("gmail_outlook_send_to_candidate",
agents/recruitment/concierge/agent.md:250:      [or `bullhorn_note_customer_visible` if channel=Bullhorn-note]
...
agents/recruitment/concierge/agent.md:285:- **"correct addressee resolution (no candidates emailed under another's name)"** — hard-fail (Step 4 critical; ESC_ADDRESSEE_MISMATCH)
...
agents/recruitment/concierge/agent.md:295:**Honesty note (per bilateral-disposition Cat-5):** Concierge `validate.sh` does NOT exist yet — this scaffold describes the intended Gate A contract for the W10-13 build slice. The W10-13 build delivers `agents/recruitment/concierge/validate.sh` against the contract above. Current text is the spec the build slice implements against, not a description of running code.
...
agents/recruitment/concierge/agent.md:312:Concierge uses these ESC codes from `agents/_shared/escalation-codes.md`:
agents/recruitment/concierge/agent.md:321:| (Concierge does NOT use `ESC_CANDIDATE_DATA_INCOMPLETE` — per catalogue §2.10 that code is reserved for Sourcing Scout shortlist completeness. Concierge's missing-Bullhorn-context case fires `ESC_AGENT_OUTPUT_SHAPE` per Gate A discipline below.) | — | — |
agents/recruitment/concierge/agent.md:330:| `ESC_GATE_B_MISS` | Ghosted-rate >5% OR send-as-is <60% OR 30-min SLA hit-rate <90% for 30 consecutive days | warn | founder + operator |
agents/recruitment/concierge/agent.md:334:Concierge has the largest escalation surface of any v1.0 agent — appropriate for the highest-stakes customer-facing comms.
agents/recruitment/concierge/agent.md:336:Concierge does NOT use:
agents/recruitment/concierge/agent.md:338:- `ESC_AUTOSEND_BLOCKED` — reserved for red-tier action attempts per catalogue line 41; Concierge has no red-tier actions. Gate A failures fire `ESC_AGENT_OUTPUT_SHAPE` instead.
agents/recruitment/concierge/agent.md:339:- `ESC_SCHEMA_VIOLATION` — reserved for vertical-schema field-constraint violations at write time per catalogue line 163; Concierge's Gate A failures are output-shape or addressee-resolution failures.
agents/recruitment/concierge/agent.md:340:- `ESC_VOICE_DRIFT_TENANT` (direct firing) — that's fired by the nightly voice-drift cron per catalogue §2.5; Concierge fires only per-run `ESC_VOICE_DRIFT`.
...
agents/recruitment/concierge/agent.md:367:## §8 — Build dependencies (W10-13 prerequisites)
agents/recruitment/concierge/agent.md:369:Concierge build cannot start until ALL of the following are confirmed:
...
agents/recruitment/concierge/agent.md:382:| Microsoft Graph MCP connector | W10 build start (~3 days) | ⏸ |
agents/recruitment/concierge/agent.md:383:| Gmail MCP connector | W10 build start (~3 days) | ⏸ |
...
agents/recruitment/concierge/agent.md:390:| `validate.sh` Gate A logic | Build at W10 start (~2 days; most complex of v1.0 validators) | ⏸ |
agents/recruitment/concierge/agent.md:391:| `context.sh` hydration | Build at W10 start (~1 day) | ⏸ |
agents/recruitment/concierge/agent.md:392:| `cycle.sh` orchestration (15-step) | Build at W10-11 (~5 days; lifecycle state machine + 12 event types) | ⏸ |
agents/recruitment/concierge/agent.md:397:**Until ALL ⏸ items resolve to ✅, W10 build slice does not start.** XL build complexity = 4-week duration not negotiable.
...
agents/recruitment/concierge/agent.md:411:| Q3 | Sending hours per tenant — should Concierge respect tenant's "no sends after 6pm" or "no weekend sends" policies? Per-tenant config? | Per-tenant config in `tenant_adapters.config.concierge_send_window` (default: M-F 09:00-17:00 tenant-timezone). |
agents/recruitment/concierge/agent.md:412:| Q4 | Rejection emails (event 6) — Position 3 (voice ≥0.82). Is this enough, or should rejections route to consultant for full draft (not just approve)? | Founder review with first pilot consultant. Recommend: Concierge drafts; consultant approves; never bypasses voice gate. |
agents/recruitment/concierge/agent.md:416:| Q8 | agent-identity email adapter (deferred) (v1.1+) — agent-identity sends. Should Concierge use agent-identity email adapter (deferred) for rejection emails (less personal pressure on consultant approving) or always tenant-identity? | v1.0: tenant-identity (Microsoft Graph / Gmail). v1.1+: agent-identity email adapter (deferred) experiment per tenant opt-in. |
...
agents/recruitment/concierge/agent.md:441:- W10-13 build slice produces all 5 sibling bundle files + 5 fixtures (broader fixture coverage warranted by XL complexity)
agents/recruitment/concierge/agent.md:448:Until then: this document is a forward-looking scaffold. Concierge is the most complex v1.0 agent; its ratification cycles may surface architectural decisions not visible at scaffold stage. Founder review at each iteration is expected.
agents/recruitment/concierge/agent.md:450:*End of Concierge agent.md draft.*
...
docs/decisions/ADR-007-concierge-gate-a-30min-sla-hybrid.md:21:- `agent.md` §1 line 17 (30-min framing as Gate B target)
docs/decisions/ADR-007-concierge-gate-a-30min-sla-hybrid.md:23:- `agent.md` §5 lines 280-293 (Gate A scope; the 30-min SLA exclusion is stated at line 291)
...
packages/harness/cortextos/tests/integration/phase5-failure-modes.test.ts:920:  it('every-30-min cron fires consistently regardless of timezone', async () => {
...
packages/harness/cortextos/tests/unit/daemon/crash-handlers.test.ts:111:  it('respects 30-min cooldown after previous alert', () => {
packages/harness/cortextos/dashboard/src/components/charts/chart-theme.ts:10:export const CHART_GOLD_DARK = '#A67C00';

codex
The first pass found the relevant local source entries for autosend action types and ESC codes. I’m narrowing now on the exact line-number citations and schema claims that the ratification skill treats as blocking.
exec
/bin/zsh -lc "nl -ba docs/master-brief* 2>/dev/null | sed -n '560,620p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
zsh:1: no matches found: docs/master-brief*

exec
/bin/zsh -lc "rg --files docs | rg -i 'ultra|master|kill|operational-hygiene|ADR-007|founder-decisions'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba docs/ULTRAPLAN.md 2>/dev/null | sed -n '550,575p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
exec
/bin/zsh -lc "nl -ba agents/recruitment/concierge/agent.md | sed -n '1,470p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   ...
   255	      sent at <ISO>")
   256	    → maintains audit trail in Bullhorn itself
   257	    → hh_decision_output("bullhorn_activity_logged",
   258	      "candidate:<bullhorn_id>", "event_type:<type>")
   259	
   260	14. Lifecycle state advance (Bullhorn write, conditional)
   261	    → some events trigger Bullhorn state changes (e.g., interview-completed
   262	      sent → advances state to "post-interview" if tenant policy says so)
   263	    → per-tenant policy; opt-in; not all tenants want this
   264	    → hh_decision_action("concierge_send_complete", "candidate:<bullhorn_id>",
   265	      payload_hash, "event=<type> elapsed=<seconds>")
   266	
   267	15. Session close + Gate B metric
   268	    → compute elapsed (event → send) for Gate B SLA tracking
   269	    → check ghosted-rate metric (any candidate with no Concierge action in
   270	      14 days post-state-change → contributes to ghosted-rate)
   271	    → if ghosted-rate >5% for tenant in 30-day rolling: ESC_GATE_B_MISS
   272	    → hh_decision_action("concierge_run_complete", session_id, run_mode)
   273	    → exit code 0
   274	```
   275	
   276	---
   ...
   278	## §5 — Gates
   279	
   280	### Gate A — validate.sh (hard-fail before action)
   281	
   282	Per master brief §8.1 Change 2 + autosend-safety-policy §4 + ULTRAPLAN A6 line 566 (interpreted per bilateral-disposition Cat-5):
   283	
   284	- **"voice classifier score ≥ position-specific threshold"** (≥0.75 position 1, ≥0.78 position 2, ≥0.82 position 3) — hard-fail
   285	- **"correct addressee resolution (no candidates emailed under another's name)"** — hard-fail (Step 4 critical; ESC_ADDRESSEE_MISMATCH)
   286	- No tone-rule block-severity violations — hard-fail (ESC_TONE_RULE_VIOLATION)
   287	- No PII outside firm boundary — hard-fail (ESC_PII_LEAKAGE_RISK)
   288	- Anti-duplicate guard passed (Step 2) — hard-fail (skip if true duplicate)
   289	- All Bullhorn context fields present (no missing candidate name / no missing email) — hard-fail (ESC_AGENT_OUTPUT_SHAPE; ESC_CANDIDATE_DATA_INCOMPLETE is Sourcing Scout's per catalogue §2.10)
   290	
   291	The 30-minute draft SLA (ULTRAPLAN A6 line 566) is interpreted as a Gate B leading metric (90% target) per §1 framing, NOT a per-draft Gate A hard-fail. Polling-fallback delays would otherwise block legitimate drafts. Per-draft SLA misses fire `ESC_CONCIERGE_SLA_MISS` (warn, aggregated).
   292	
   293	Gate A failures fire `ESC_ADDRESSEE_MISMATCH` or `ESC_TONE_RULE_VIOLATION` or `ESC_PII_LEAKAGE_RISK` or `ESC_AGENT_OUTPUT_SHAPE` (output-shape constraint — missing Bullhorn context OR voice threshold misses persistently) — all blocking; draft to `/tmp`; operator notified immediately.
   294	
   295	**Honesty note (per bilateral-disposition Cat-5):** Concierge `validate.sh` does NOT exist yet — this scaffold describes the intended Gate A contract for the W10-13 build slice. The W10-13 build delivers `agents/recruitment/concierge/validate.sh` against the contract above. Current text is the spec the build slice implements against, not a description of running code.
   296	
   297	### Gate B — Outcome thresholds (success metrics, not block)
   298	
   299	Per ULTRAPLAN A6 line 567 + ADR-007 amendment: **"<5% candidate-ghosted rate; ≥60% send-as-is rate on drafts; ≥90% 30-min SLA hit rate"**. The third metric is ADR-007's Gate B reframe of ULTRAPLAN line 566's per-draft 30-min hard-fail (NOT in original line 567 wording).
   300	
   301	Three metrics:
   302	- **Ghosted-rate:** % of candidates with a lifecycle state change in the last 30 days who received no Concierge comm within 14 days of that change. Target <5%. (per ULTRAPLAN line 567)
   303	- **Send-as-is rate:** % of drafts approved by consultant without edits (consultant clicks "approve" not "edit-and-approve"). Target ≥60%. Measured via `recent_edit` rows with `resolution='approved_verbatim'` vs `approved_after_edit`. (per ULTRAPLAN line 567)
   304	- **30-min SLA hit rate:** % of drafts generated within 30 minutes of lifecycle-event DETECTION. Target ≥90%. Per-draft misses fire `ESC_CONCIERGE_SLA_MISS`; rolling-window aggregate <90% fires `ESC_GATE_B_MISS`. (per ADR-007 + amended ULTRAPLAN line 567)
   305	
   306	Gate B doesn't block individual sends. Tracked monthly via the tenant's day-30 metrics roll-up. Any of the three metrics below target for 30 consecutive days → `ESC_GATE_B_MISS` → founder + operator review (likely indicates LLM drift, lifecycle-event detection gaps, OR tenant-specific style mismatch).
   307	
   308	---
   309	
   310	## §6 — Escalation codes
   311	
   312	Concierge uses these ESC codes from `agents/_shared/escalation-codes.md`:
   313	
   314	| Code | Trigger | Severity | Routing |
   315	|---|---|---|---|
   316	| `ESC_BULLHORN_AUTH` | OAuth refresh fails (payload.failure_type='refresh_failed' or 'revoked_401' covers the 6+ consecutive failure case) | **blocking** | operator + ifos_oncall |
   317	| `ESC_RATE_LIMIT_HIT` | 429 from Bullhorn or email provider (payload.upstream identifies which) | warn | operator_chat_id |
   318	| `ESC_MS_GRAPH_AUTH` | Microsoft Graph OAuth fail | **blocking** | operator + ifos_oncall + tenant-admin (token re-auth required) |
   319	| `ESC_GMAIL_AUTH` | Gmail OAuth fail | **blocking** | operator + ifos_oncall + tenant-admin |
   320	| `ESC_LIFECYCLE_STATE_UNKNOWN` | Bullhorn state transition not in 12-event taxonomy | warn (handler logs + skips draft) | operator_chat_id |
   321	| (Concierge does NOT use `ESC_CANDIDATE_DATA_INCOMPLETE` — per catalogue §2.10 that code is reserved for Sourcing Scout shortlist completeness. Concierge's missing-Bullhorn-context case fires `ESC_AGENT_OUTPUT_SHAPE` per Gate A discipline below.) | — | — |
   322	| `ESC_ADDRESSEE_MISMATCH` | Step 4 critical — wrong recipient | **blocking** | operator + ifos_oncall |
   323	| `ESC_VOICE_DRIFT` | Voice classifier below the position-specific threshold (≥0.75/0.78/0.82) after 3 retries. The Gate A hard-fail (validate.sh exits non-zero; draft not sent) is expressed through `validate_gate_a_fail`, NOT through this code's severity: per catalogue (escalation-codes.md lines 120-125) `ESC_VOICE_DRIFT` is `warn` → `operator_chat_id` for ALL positions. Position-specific paging urgency (e.g. oncall on rejections) would require a catalogue amendment adding position-severity semantics — not yet made. | warn | operator_chat_id |
   324	| `ESC_TONE_RULE_VIOLATION` | Block-severity tone rule hit | **blocking** | operator + ifos_oncall |
   325	| `ESC_PII_LEAKAGE_RISK` | PII outside firm boundary | **blocking** | operator + ifos_oncall |
   326	| `ESC_CONCIERGE_SLA_MISS` | Draft >30 min after lifecycle event | warn | (logged; aggregated to Gate B) |
   327	| `ESC_APPROVAL_BRIDGE_TIMEOUT` | No consultant approval within the policy timeout (default PT4H per escalation-codes.md lines 348-353 + autosend-policy.yaml lines 235-239) | warn | operator + tenant-admin |
   328	| `ESC_SEND_FAIL` | Email provider 4xx/5xx | warn | operator_chat_id |
   329	| `ESC_AGENT_OUTPUT_SHAPE` | Gate A failure (output-shape constraint per catalogue line 184) — distinct from ESC_AUTOSEND_BLOCKED which is for red-tier action attempts only | warn | operator_chat_id |
   ...
   391	| `context.sh` hydration | Build at W10 start (~1 day) | ⏸ |
   392	| `cycle.sh` orchestration (15-step) | Build at W10-11 (~5 days; lifecycle state machine + 12 event types) | ⏸ |
   393	| Lifecycle event-detection polling fallback | Build at W11-12 (~3 days; Bullhorn webhook coverage gaps) | ⏸ |
   394	| Comms-template library v0.1 (12 event types × 2 recipient roles = 24 templates minimum) | Build at W12-13 (~5 days) | ⏸ |
   395	| 5 fixtures with golden outputs (broader than 3 for other agents; XL complexity warrants) | Build at W13 (~2 days) | ⏸ |
   396	
   397	**Until ALL ⏸ items resolve to ✅, W10 build slice does not start.** XL build complexity = 4-week duration not negotiable.
   398	
   399	---
   400	
   401	## §9 — Status + open questions
   402	
   403	**Status:** Proposed. Awaits Bullhorn A+B + per-tenant email-provider signups + D1 founder decision + Q1 LOI + W10-13 build slice.
   404	
   405	### Open questions for founder review
   406	
   407	| # | Question | Resolution path |
   408	|---|---|---|
   409	| Q1 | **D1 founder decision (autosend orange-tier path)** — A (bridge to cortextOS), B (Telegram shim), C (no autosend; manual). Concierge ABSOLUTELY needs this resolved before build. | Founder decision per `docs/decisions/2026-05-20-codex-round-1-founder-decisions.md` §D1. Recommend D1-B (Telegram shim) for v1.0 ship; D1-A as v1.1 upgrade. |
   410	| Q2 | Lifecycle event taxonomy — 12 events proposed in §3. Founder confidence each is correct + complete? Missing: "candidate referred to another role internally"? "Client cancelled brief"? | Founder review with first pilot consultants. Recommend: ship 12-event v1.0; expand v1.1+ based on real patterns. |
   411	| Q3 | Sending hours per tenant — should Concierge respect tenant's "no sends after 6pm" or "no weekend sends" policies? Per-tenant config? | Per-tenant config in `tenant_adapters.config.concierge_send_window` (default: M-F 09:00-17:00 tenant-timezone). |
   412	| Q4 | Rejection emails (event 6) — Position 3 (voice ≥0.82). Is this enough, or should rejections route to consultant for full draft (not just approve)? | Founder review with first pilot consultant. Recommend: Concierge drafts; consultant approves; never bypasses voice gate. |
   413	| Q5 | Comms-template customisation — every tenant edits these. Per-event-type, per-recipient-role × per-tenant = 24+ templates each. Authoring tool? | v1.0: Markdown files at `/vault/<slug>/concierge-templates/<event>-<role>.md`. v1.1: Brain UI WYSIWYG editor. |
   414	| Q6 | Send-as-is rate (Gate B ≥60%) — measurement requires consultant to differentiate "approve" from "edit-and-approve". Brain UI v1.0 has no such control yet. Telegram-based approval? | Telegram-based for v1.0: `/approve <draft-id>` vs `/approve-edit <draft-id> <revised-body>`. Brain UI v1.1+ adds inline edit UX. |
   415	| Q7 | Anti-duplicate guard window — 24h proposed in Step 2. Edge case: webhook + poll cycle both fire same event within 5 min → second skipped. What if first failed silently? | Anti-duplicate also checks decision_log for `phase='action'` not just `phase='trigger'` — if first didn't send, second can attempt. |
   416	| Q8 | agent-identity email adapter (deferred) (v1.1+) — agent-identity sends. Should Concierge use agent-identity email adapter (deferred) for rejection emails (less personal pressure on consultant approving) or always tenant-identity? | v1.0: tenant-identity (Microsoft Graph / Gmail). v1.1+: agent-identity email adapter (deferred) experiment per tenant opt-in. |
   417	| Q9 | Cross-tenant lifecycle handling — what if a candidate placed at Tenant A's client interviews at Tenant B 6 weeks later? Bullhorn has separate tenant slugs; no cross-tenant leak. But operator visibility? | v1.0: strict tenant isolation (no cross-tenant data visibility). v1.1+: separate agent for tenant-network-graph if commercial demand. |
   418	| Q10 | 90-day check-in (event 12) — relationship-building tone. Should Concierge also surface "anyone in your network looking?" referral request? | Founder review with first pilot consultant + tenant brand voice. Recommend: opt-in via tenant config. |
   419	
   420	### Gotchas (carried forward from ULTRAPLAN A6 line 569-570)
   421	
   422	1. **Lifecycle event detection from Bullhorn is the unreliable bit.** Bullhorn's webhook coverage is patchy; polling fallbacks are required. Step 1 polling at 5-min cycle + anti-duplicate guard at Step 2 is the architecture.
   423	2. **Voice quality on rejections is the hardest test case.** Position-3 threshold (≥0.82) + sensitive-event escalation routing (Step 6). Get this wrong and it costs the tenant a candidate relationship.
   424	3. **Comms-template library is per-tenant, per-event-type, per-recipient-role.** 24+ templates per tenant minimum. Authoring effort is significant; consider this in pilot onboarding scoping.
   425	4. **Microsoft Graph vs Gmail per-tenant** — each tenant chooses based on their existing email stack. v1.0 supports both; v1.1+ may add agent-identity email adapter (deferred).
   426	
   427	---
   428	
   429	## §10 — When this document ratifies
   430	
   431	Per `.codex/ratification/review-agent-bundle.md` skill (built Day 19, commit `825ebd4`): this agent.md ratifies when Codex Round 4 Phase 2 (Day 20) returns RATIFIED verdict.
   432	
   433	Status flips Proposed → Accepted when:
   434	- Codex Round 4 Phase 2 ratifies
   435	- **ADR-007 (Concierge Gate A 30-min SLA hybrid) RATIFIED** — closes the documented deviation from ULTRAPLAN A6 line 566 wording. ADR drafted at `docs/decisions/ADR-007-concierge-gate-a-30min-sla-hybrid.md` (Day 20 W4 bilateral pass); ratifies via `.codex/ratification/review-architecture-decision.md` skill. Until RATIFIED, agent.md's Gate B framing of the 30-min SLA is documented disposition only.
   436	- **Founder Decision D1 RESOLVED** (Q1 above) — without this, Concierge build cannot start
   437	- Founder approves §9 Q2 (lifecycle taxonomy) + Q3 (send window) + Q4 (rejection routing) + Q5 (template authoring UX) + Q6 (Gate B UX)
   438	- Q7-Q9 documented decisions captured
   439	
   440	Status flips Accepted → In Force when:
   441	- W10-13 build slice produces all 5 sibling bundle files + 5 fixtures (broader fixture coverage warranted by XL complexity)
   442	- First production lifecycle webhook processed end-to-end against migration-test tenant
   443	- 12-event taxonomy validated against first pilot tenant's actual Bullhorn state-change patterns
   444	- Voice classifier microservice production-ready (per-tenant; position-3 ≥0.82 sustained)
   445	- Gate B feedback loop operational (consultant approve/edit distinguishable)
   446	- Codex re-ratifies post-build via `review-agent-bundle.md` skill
   447	
   448	Until then: this document is a forward-looking scaffold. Concierge is the most complex v1.0 agent; its ratification cycles may surface architectural decisions not visible at scaffold stage. Founder review at each iteration is expected.
   449	
   450	*End of Concierge agent.md draft.*

exec
/bin/zsh -lc "nl -ba docs/specs/ULTRAPLAN.md | sed -n '555,572p'" in /Users/madsadmin/code/CortexOS
exec
...
exec
/bin/zsh -lc "nl -ba docs/decisions/ADR-007-concierge-gate-a-30min-sla-hybrid.md | sed -n '1,145p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '90,115p'; nl -ba agents/_shared/autosend-policy.yaml | sed -n '235,285p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    90	    tier: green
    91	    agent: concierge
...
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '210,470p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba agents/_shared/escalation-codes.md | sed -n '1,210p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   210	- **Severity:** warn
...
exec
/bin/zsh -lc 'rg -n "concierge_send_complete|concierge_run_complete|ESC_BULLHORN_AUTH|ESC_RATE_LIMIT_HIT|ESC_TONE_RULE_VIOLATION|ESC_PII_LEAKAGE_RISK|ESC_SCHEMA_VIOLATION" agents/_shared/autosend-policy.yaml agents/_shared/escalation-codes.md -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
1. False autosend-policy line citation. Lines 238-240 and 327 cite `autosend-policy.yaml lines 235-239` for the default PT4H approval timeout, but those lines are the `bullhorn_candidate_write` yellow-tier entry, not the Concierge timeout. The relevant Concierge orange actions are around autosend-policy lines 259-270. Fix the citation to the actual timeout lines or remove the exact line reference.

2. ESC severity is repurposed beyond the catalogue. Lines 323-329 mark `ESC_TONE_RULE_VIOLATION` / `ESC_AGENT_OUTPUT_SHAPE` as blocking Gate A failures, but the catalogue defines both as `warn` (`ESC_AGENT_OUTPUT_SHAPE` lines 184-189; `ESC_TONE_RULE_VIOLATION` lines 385-390). Fix by distinguishing “Gate A blocks the draft” from “ESC severity is warn,” or amend the catalogue before using blocking severity.

3. Honest-signal status is internally contradictory. Line 4 says ADR-007 ratification is still awaited; line 17 says ADR-007 is Accepted + Codex RATIFIED and the blocker is satisfied; line 435 again says “Until RATIFIED.” Line 4 also uses the relative date “today,” which violates the artefact date discipline. Fix by using absolute dates and making ADR-007’s status consistent across §1, §9, and §10.

4. Step 15 specifies an invalid `hh_decision_action` call. Line 272 calls `hh_decision_action("concierge_run_complete", session_id, run_mode)`, but `_shared/hook-helpers.sh` defines `hh_decision_action <action_type> <target> <payload_hash> <payload_preview>`. Fix Step 15 to include target, payload hash, and preview.

5. The Bullhorn activity-log write is audited as output instead of an action. Lines 253-258 call `bullhorn.create_activity_log(...)`, which is an external Bullhorn write, then record `hh_decision_output("bullhorn_activity_logged", ...)` rather than a gated/registered action row. Fix by registering and using a proper action_type for this Bullhorn write, or explicitly prove why this write is exempt from action-phase gating.
tokens used
90,585
REJECTED
