...
session id: 019e8809-4451-7f70-ba52-92c1657c1647
--------
user
...
exec
/bin/zsh -lc 'rg -n "xero|accounting_reconciliation_write|ESC_ACCOUNTING|ESC_RATE_LIMIT|ESC_PROVIDER_FETCH_FAIL" agents/_shared/autosend-policy.yaml agents/_shared/escalation-codes.md agents/recruitment/cash-conductor/tools.yaml packages/mcp-connectors/xero -S' in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc 'rg -n "composio|agentmail|packages/harness/cortextos|decision_log|migration-test|test-tenant-b" packages/mcp-connectors/xero -i' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
packages/mcp-connectors/xero/README.md:197:- ✓ No Composio / AgentMail references
...
exec
/bin/zsh -lc "rg -n \"export (async function|function|class|const)|export \\{\" packages/mcp-connectors/xero/src/index.ts packages/mcp-connectors/xero/src/*.ts && rg -n \"Capability ID|xero_\" packages/mcp-connectors/xero/README.md agents/recruitment/cash-conductor/tools.yaml" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
agents/recruitment/cash-conductor/tools.yaml:191:#   xero_reminder_send_customer        line 263   orange    [Cash Conductor OWNS this]
agents/recruitment/cash-conductor/tools.yaml:196:#   xero_oauth                         green
packages/mcp-connectors/xero/README.md:15:| Capability ID (tools.yaml) | Function (src/index.ts) | Purpose | Cash Conductor cycle.sh step | action_type | Tier |
packages/mcp-connectors/xero/README.md:17:| `xero_oauth` | `refreshTokens(config, current, fetchFn?)` | OAuth 2.0 refresh; concurrent-safe dedup per tenant; atomic-file-write persistence | Step 1 (auth refresh) | `xero_oauth` | green |
packages/mcp-connectors/xero/README.md:18:| `xero_list_open_invoices` | `listOpenInvoices(client, options?)` | Page through AUTHORISED+SUBMITTED invoices with AmountDue > 0 | Step 4 (invoice register ingest) | n/a (read-only) | n/a |
packages/mcp-connectors/xero/README.md:19:| `xero_get_invoice` | `getInvoice(client, invoiceId, options?)` | Fetch single invoice by Xero InvoiceID | Step 9 (chase-draft validation) | n/a | n/a |
packages/mcp-connectors/xero/README.md:20:| `xero_list_payments` | `listPayments(client, options?)` | List ACCRECPAYMENT records (reconciliation context) | Step 5 (reconciliation pass) | n/a | n/a |
...
codex
I’m checking the connector against the ratification surface rather than just the embedded artefact text: package exports, Cash Conductor tool registrations, autosend tiers, and the boundary strings. Next I’m going to run the package checks where feasible and inspect the policy entries with line numbers.
exec
/bin/zsh -lc "nl -ba agents/recruitment/cash-conductor/tools.yaml | sed -n '24,58p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '150,165p;236,248p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/xero/README.md | sed -n '1,35p;110,120p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	# @ifos/xero
     2	
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/xero/src/client.ts | sed -n '90,132p;168,192p;210,225p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
codex
The main contract looks mostly aligned: the README capability table, `src/index.ts`, and Cash Conductor’s `tools.yaml` all name the same five bus-routed capabilities, and the two documented state-changing action types are present with matching tiers. I’m doing one more verification pass for executable/type-level issues, because this connector is TypeScript and build failures are ratification blockers under the quality-gate rule.
exec
/bin/zsh -lc "rg -n \"console\\.(log|error|warn).*\\"'$'"\\{.*(KEY|SECRET|PASSWORD|TOKEN|BEARER)|throw\\s+new\\s+Error.*\\"'$'"\\{.*(KEY|SECRET|PASSWORD)|JSON\\.stringify\\(envWithSecrets\\)\" packages/mcp-connectors/xero/src packages/mcp-connectors/xero/tests -S" in /Users/madsadmin/code/CortexOS
 exited 1 in 0ms:
exec
...
> vitest run
 ...
 ✓ tests/auth.test.ts (7 tests) 78ms
 ✓ tests/rate-limit.test.ts (7 tests) 770ms
   ✓ xero rate-limit > daily bucket (5000/day per Xero published limit) > triggers daily-soft backoff at 4000 used 304ms
   ✓ xero rate-limit > daily bucket (5000/day per Xero published limit) > blocks at daily-hard 5000 (consume returns false on the 5001st call) 462ms
 ✓ tests/capabilities.test.ts (9 tests) 1036ms
   ✓ xero capabilities — invoices > listOpenInvoices: persistent 429 surfaces as XeroRateLimitError after retries 468ms

 Test Files  4 passed (4)
      Tests  28 passed (28)
   Start at  12:13:23
   Duration  2.17s (transform 264ms, setup 0ms, collect 485ms, tests 1.89s, environment 1ms, prepare 659ms)


codex
RATIFIED

`pnpm --filter @ifos/xero typecheck` passed.
`pnpm --filter @ifos/xero test` passed: 28/28 vitest tests.
tokens used
47,922
RATIFIED
