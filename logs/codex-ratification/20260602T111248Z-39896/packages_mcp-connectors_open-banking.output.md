...
session id: 019e880a-ff9a-7010-8028-6d506a6e7921
--------
user
...
codex
I’m going to verify the connector against the referenced policy/schema files and the actual package surface, then return only the ratification verdict.
exec
/bin/zsh -lc 'rg --files packages/mcp-connectors/open-banking agents/recruitment/cash-conductor agents/_shared docs/verticals/recruitment | sort' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc 'find packages/mcp-connectors/open-banking -maxdepth 3 -type f | sort' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
packages/mcp-connectors/open-banking/README.md
packages/mcp-connectors/open-banking/dist/index.d.ts
packages/mcp-connectors/open-banking/dist/index.js
packages/mcp-connectors/open-banking/dist/index.js.map
packages/mcp-connectors/open-banking/fixtures/truelayer-balance.json
packages/mcp-connectors/open-banking/fixtures/truelayer-transactions.json
packages/mcp-connectors/open-banking/node_modules/.bin/tsc
packages/mcp-connectors/open-banking/node_modules/.bin/tsserver
packages/mcp-connectors/open-banking/node_modules/.bin/tsup
packages/mcp-connectors/open-banking/node_modules/.bin/tsup-node
packages/mcp-connectors/open-banking/node_modules/.bin/tsx
packages/mcp-connectors/open-banking/node_modules/.bin/vitest
packages/mcp-connectors/open-banking/package.json
packages/mcp-connectors/open-banking/src/auth.ts
packages/mcp-connectors/open-banking/src/balance.ts
packages/mcp-connectors/open-banking/src/cache.ts
...
exec
/bin/zsh -lc 'rg -n "open_banking|open-banking|truelayer|plaid|ESC_RATE_LIMIT_HIT|ESC_OPEN_BANKING|ESC_PROVIDER_FETCH_FAIL|composio|agentmail" agents packages docs -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
docs/specs/_archive-build-handoff.md:12:6. `intelforce-os-composio-and-agentmail-adoption.md` — the adapter boundary
docs/specs/_archive-build-handoff.md:392:Per `intelforce-os-composio-and-agentmail-adoption.md`:
docs/decisions/2026-06-02-codex-cluster-f-r3-justification.md:20:| Live-test gating claim in xero + qb README | R2 (new) | I fixed for `@ifos/open-banking` in R1 commit `47dbce1` but missed applying the same honest-signal fix to xero + qb READMEs. | Fix (mirror OB pattern) |
docs/decisions/2026-06-02-codex-cluster-f-r3-justification.md:22:| OB type-enum mismatch (`plaid-uk` vs schema's `plaid_uk`) | R2 (new) | Top-level Rule 2 (schema before code) violation — `vertical-schema.v0.3-supplement.yaml` line 636 uses `plaid_uk`; `types.ts` uses `plaid-uk`. | Fix (align connector to schema) |
...
agents/recruitment/cash-conductor/fixtures/99-token-aging-canary.yaml:68:        - provider: truelayer
agents/recruitment/cash-conductor/fixtures/99-token-aging-canary.yaml:69:        - connection_id: <tenant-truelayer-account-id>
...
agents/recruitment/cash-conductor/tools.yaml:107:  # NOTE: open_banking_plaid_uk_oauth was declared here in R1 (commit 47dbce1)
agents/recruitment/cash-conductor/tools.yaml:112:  # Plaid UK remains as an internal stub in @ifos/open-banking src/ for v1.1+
agents/recruitment/cash-conductor/tools.yaml:115:  - id: open_banking_list_transactions
agents/recruitment/cash-conductor/tools.yaml:116:    package: "@ifos/open-banking"
agents/recruitment/cash-conductor/tools.yaml:121:  - id: open_banking_get_account_balance
agents/recruitment/cash-conductor/tools.yaml:122:    package: "@ifos/open-banking"
agents/recruitment/cash-conductor/tools.yaml:198:#   open_banking_truelayer             green
agents/recruitment/cash-conductor/tools.yaml:199:#   open_banking_plaid_uk              green   (v1.1+; queued but never fires v1.0)
...
docs/architecture/agent-bundle-renderer-design.md:290:  agentmail:
agents/recruitment/cash-conductor/context.sh:14:#   CTX_OPEN_BANKING_PROVIDER truelayer | plaid-uk (per tenant config)
...
agents/recruitment/cash-conductor/context.sh:81:: "${CTX_OPEN_BANKING_PROVIDER:=truelayer}"
...
packages/agents-runtime/_shared/common-accounting.json:40:      "enum": ["truelayer", "tink", "none"],
...
docs/operations/goal-overnight-2026-05-31.md:70:- 0 `composio|agentmail` in `agents/` (grep)
...
docs/build-brief/00-MASTER-BRIEF.md:155:If a code review finds `composio.gmail.send_email` in an `agent.md` or `tools.yaml`, it's rejected. The adapter speaks vertical vocabulary (`candidate`, `placement`, `brief`); the execution layer is a black box behind it.
docs/build-brief/00-MASTER-BRIEF.md:267:- `intelforce-os-composio-and-agentmail-adoption.md` → `docs/architecture/COMPOSIO-AGENTMAIL-ADOPTION.md`
...
docs/operations/goal-week-4-track-1.md:354:| **Adapter boundary** | 0 Composio / AgentMail references in agent.md/tools.yaml/fixtures | `grep -rinE "composio\|agentmail" agents/` returns 0 |
...
docs/operations/w4-bilateral-pass-6-agent-md.md:141:  - **Likely:** FIX-IN-PLACE. Add `tools.yaml` + `cleanup.sh` to §8 sibling list. Annotate §4 with planned capability names (`xero.invoices.list`, `truelayer.transactions.fetch`, etc.).
...
packages/mcp-connectors/open-banking/src/auth.ts:34:  "https://auth.truelayer.com/connect/token";
packages/mcp-connectors/open-banking/src/auth.ts:36:  "https://auth.truelayer-sandbox.com/connect/token";
packages/mcp-connectors/open-banking/src/auth.ts:130:  if (config.provider === "plaid_uk") {
packages/mcp-connectors/open-banking/src/rate-limit.ts:4:// TrueLayer published limits (https://docs.truelayer.com/docs/api-overview):
packages/mcp-connectors/open-banking/src/rate-limit.ts:7:// Plaid UK published limits (https://plaid.com/docs/api/rate-limits/):
packages/mcp-connectors/open-banking/src/index.ts:1:// @ifos/open-banking — public API
packages/mcp-connectors/open-banking/src/index.ts:24:// (1) Bus-routed capabilities (set-equal with cash-conductor/tools.yaml §open-banking)
packages/mcp-connectors/open-banking/src/index.ts:28:// open_banking_truelayer_oauth (action_type: open_banking_truelayer, green tier).
packages/mcp-connectors/open-banking/src/index.ts:30:// provider='plaid_uk' throws NotImplementedError (v1.1+ stub kept in src/auth.ts).
packages/mcp-connectors/open-banking/src/index.ts:32:// open_banking_list_transactions (read-only)
packages/mcp-connectors/open-banking/src/index.ts:34:// open_banking_get_account_balance (read-only)
agents/recruitment/concierge/cycle.sh:137:# contact entities. ESC_RATE_LIMIT_HIT on 429; ESC_BULLHORN_AUTH on auth
packages/mcp-connectors/open-banking/src/transactions.ts:50:  if (config.provider === "plaid_uk") {
...
packages/mcp-connectors/open-banking/src/client.ts:47:    if (opts.config.provider === "plaid_uk") {
packages/mcp-connectors/open-banking/src/client.ts:49:      this.base_url = "https://plaid_uk-not-yet-implemented.invalid";
packages/mcp-connectors/open-banking/src/client.ts:80:    if (this.opts.config.provider === "plaid_uk") {
packages/mcp-connectors/open-banking/src/client.ts:82:        "Plaid UK provider client not yet implemented (v1.1+ deferred). Use provider: 'truelayer' for v1.0.",
...
packages/mcp-connectors/open-banking/src/types.ts:9:export type OpenBankingProvider = "truelayer" | "plaid_uk";
agents/recruitment/concierge/tools.yaml:51:        escalation: ESC_RATE_LIMIT_HIT  # warn; operator; payload.upstream='bullhorn'
packages/mcp-connectors/open-banking/src/balance.ts:34:  if (config.provider === "plaid_uk") {
...
packages/mcp-connectors/open-banking/README.md:19:| `open_banking_truelayer_oauth` | `refreshTokens(config, current, fetchFn?, now?)` | OAuth refresh against TrueLayer; concurrent-safe per (provider, connection_id); **refuses if consent is in PSD2 blocking window (≤7 days)** | Step 1 (auth refresh) | `open_banking_truelayer` | green |
packages/mcp-connectors/open-banking/README.md:20:| `open_banking_list_transactions` | `listTransactionsSince(client, config, options)` | List bank transactions on/after a timestamp; TrueLayer raw payload preserved in `raw_provider_payload` for v1.1+ provider-agnostic upgrade | Step 3 (transaction ingest) | n/a (read-only) | n/a |
packages/mcp-connectors/open-banking/README.md:21:| `open_banking_get_account_balance` | `getAccountBalance(client, config)` | Current account balance (available + cleared) | Step 10 (cash-flow forecast) | n/a (read-only) | n/a |
packages/mcp-connectors/open-banking/README.md:23:`open_banking_truelayer` action_type exists in `agents/_shared/autosend-policy.yaml` at green tier. `open_banking_plaid_uk` ALSO exists in the policy (registered 2026-06-01 in commit f414492) — kept registered to avoid churn when v1.1+ work re-promotes Plaid; not currently consumed.
packages/mcp-connectors/open-banking/README.md:34:| `getTokenAgeStage(tokens, now?)` | Pure predicate — returns `{stage: 'fresh'|'info'|'warn'|'blocking', days_until_consent_expiry}`; staged-severity hook for operator alerting via `ESC_OPEN_BANKING_TOKEN_AGING` |
packages/mcp-connectors/open-banking/README.md:54:`getTokenAgeStage()` is the pure classifier the agent uses to alert the operator BEFORE consent dies. Stages, with the cycle.sh / agent.md §6 mapping to `ESC_OPEN_BANKING_TOKEN_AGING`:
packages/mcp-connectors/open-banking/README.md:60:| 8-14 | `warn` | escalate to operator chat (`ESC_OPEN_BANKING_TOKEN_AGING` severity warn) |
packages/mcp-connectors/open-banking/README.md:79:} from "@ifos/open-banking";
packages/mcp-connectors/open-banking/README.md:82:  provider: "truelayer" as const,
packages/mcp-connectors/open-banking/README.md:85:  connection_id: "<truelayer-account-id>",
packages/mcp-connectors/open-banking/README.md:112:1. Register the IFOS app at https://console.truelayer.com/ (TrueLayer) → get `client_id` + `client_secret`.
packages/mcp-connectors/open-banking/README.md:115:4. POST to `https://auth.truelayer-sandbox.com/connect/token` (or `auth.truelayer.com` for prod) with `grant_type=authorization_code` + the code → receive `{access_token, refresh_token, expires_in}`.
packages/mcp-connectors/open-banking/README.md:116:5. Call `GET https://api.truelayer-sandbox.com/data/v1/accounts` with the access_token → get the `account_id`(s) for this connection.
packages/mcp-connectors/open-banking/README.md:128:- **TrueLayer:** https://docs.truelayer.com/docs/data-api-rate-limits — production prod-API limits are tier-dependent and not exposed as a single public number; sandbox is documented at 30/min/connection. We use the sandbox figure as the conservative ceiling for production too — it can be tuned upward when Cash Conductor cycle.sh telemetry shows actual usage patterns.
packages/mcp-connectors/open-banking/README.md:129:- **Plaid UK:** https://plaid.com/docs/errors/rate-limit-exceeded/ — Plaid Standard tier publishes a 600/min per-Item ceiling; we use 30/min as the conservative starting point until v1.1+ pilots show real load.
packages/mcp-connectors/open-banking/README.md:144:| Local hard-gate (100%) reached | `OpenBankingRateLimitError` thrown by `consume()`/client | `ESC_RATE_LIMIT_HIT` (warn; operator) | `{upstream: "truelayer" | "plaid_uk", retry_after_seconds: null, consecutive_429s: 0}` |
packages/mcp-connectors/open-banking/README.md:145:| Upstream 429 from provider API | `OpenBankingRateLimitError` thrown with `retry_after_seconds` from `Retry-After` header | `ESC_RATE_LIMIT_HIT` | `{upstream: "truelayer" | "plaid_uk", retry_after_seconds: <N>, consecutive_429s: <N>}` |
packages/mcp-connectors/open-banking/README.md:153:| `listTransactionsSince` / `getAccountBalance` | GET | 2 | Exponential w/ jitter (250-1000ms) | `OpenBankingError` / `OpenBankingRateLimitError` → `ESC_PROVIDER_FETCH_FAIL` or `ESC_RATE_LIMIT_HIT` (consumer-emitted) |
packages/mcp-connectors/open-banking/README.md:154:| `refreshTokens` | POST | **0** | n/a | `OpenBankingAuthError` → `ESC_OPEN_BANKING_AUTH` (**blocking**; consumer-emitted; routes operator + ifos_oncall per escalation-codes.md lines 288-294; payload `failure_type: 'refresh_failed'`) |
packages/mcp-connectors/open-banking/README.md:155:| `refreshTokens` blocked by PSD2 consent expiry | POST | **0** | n/a | `OpenBankingConsentExpiredError` → `ESC_OPEN_BANKING_AUTH` (blocking; payload `failure_type: 'consent_expired_90d'`); user must re-do SCA per Bootstrap § |
packages/mcp-connectors/open-banking/README.md:156:| 401 from any GET | — | force-refresh access_token, retry once | — | `OpenBankingAuthError` → `ESC_OPEN_BANKING_AUTH` |
packages/mcp-connectors/open-banking/README.md:157:| 429 from any GET | — | honour `Retry-After` header | jittered backoff if no header | `OpenBankingRateLimitError` → `ESC_RATE_LIMIT_HIT` |
packages/mcp-connectors/open-banking/README.md:161:The separate **PSD2 token-aging signal** — `ESC_OPEN_BANKING_TOKEN_AGING` (staged info → warn → blocking per consent-expiry distance) — is emitted by the consumer based on `getTokenAgeStage()` BEFORE refresh fails. That's a different code path from the refresh-failure mapping above; see `agents/_shared/escalation-codes.md` lines 296-307 for the staged severity definition and `agents/recruitment/cash-conductor/agent.md` §6 for the consumer flow.
packages/mcp-connectors/open-banking/README.md:219:   ├── Step 3  (bank transaction ingest)            ──┼─→ @ifos/open-banking (this package)
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:8:describe("open-banking rate-limit", () => {
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:14:    const s = check("truelayer", "acct-1");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:22:    for (let i = 0; i < 23; i++) consume("truelayer", "acct-1");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:23:    expect(check("truelayer", "acct-1").shouldBackoff).toBe(false);
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:27:    for (let i = 0; i < 24; i++) consume("truelayer", "acct-1");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:28:    const s = check("truelayer", "acct-1");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:34:    for (let i = 0; i < 30; i++) consume("truelayer", "acct-1");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:35:    expect(check("truelayer", "acct-1").reason).toBe("minute-hard");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:36:    expect(consume("truelayer", "acct-1")).toBe(false);
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:40:    for (let i = 0; i < 25; i++) consume("truelayer", "acct-A");
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:41:    expect(check("truelayer", "acct-A").shouldBackoff).toBe(true);
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:42:    expect(check("truelayer", "acct-B").shouldBackoff).toBe(false);
packages/mcp-connectors/open-banking/tests/rate-limit.test.ts:43:    expect(check("plaid_uk", "acct-A").shouldBackoff).toBe(false);
packages/mcp-connectors/open-banking/tests/scaffold.test.ts:25:describe("@ifos/open-banking package surface", () => {
packages/mcp-connectors/open-banking/tests/scaffold.test.ts:32:    expect(TRUELAYER_API_PROD).toBe("https://api.truelayer.com");
packages/mcp-connectors/open-banking/tests/scaffold.test.ts:33:    expect(TRUELAYER_API_SANDBOX).toBe("https://api.truelayer-sandbox.com");
packages/mcp-connectors/open-banking/tests/auth.test.ts:49:    provider: "truelayer",
packages/mcp-connectors/open-banking/tests/auth.test.ts:72:describe("open-banking auth", () => {
packages/mcp-connectors/open-banking/tests/auth.test.ts:161:    const config = makeConfig(token_file, { provider: "plaid_uk" });
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:25:import TL_TRANSACTIONS from "../fixtures/truelayer-transactions.json" with { type: "json" };
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:26:import TL_BALANCE from "../fixtures/truelayer-balance.json" with { type: "json" };
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:47:    provider: "truelayer",
...
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:166:      if (url.includes("auth.truelayer-sandbox.com/connect/token")) {
...
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:205:    const config = makeConfig({ provider: "plaid_uk" });
packages/mcp-connectors/open-banking/tests/capabilities.test.ts:213:    const config = makeConfig({ provider: "plaid_uk" });
...
codex
The package surface and policy references are present. I’m checking the stricter connector rules now: capability equality, action_type tier match, secret logging patterns, and whether the provider/rate-limit claims are internally consistent.
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/README.md | sed -n '1,240p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	# @ifos/open-banking
     2	
    ...
    14	
    15	**Provider scope (v1.0):** TrueLayer ONLY. Plaid UK exists as an internal stub in `src/` for v1.1+ work but is NOT a v1.0 bus-routed capability (per Codex F-R3 closure 2026-06-02 — `docs/decisions/2026-06-02-codex-cluster-f-r3-justification.md`; pre-R3 the v1.1+ stub was declared as a v1.0 capability for set-equality, which violated review-mcp-connector single-upstream-provider intent + had no success fixtures to back the surface). When the first IFOS tenant picks Plaid over TrueLayer in v1.1+, the Plaid implementation graduates to a bus-routed capability and re-registers here.
    16	
    17	| Capability ID (tools.yaml) | Function (src/index.ts) | Purpose | Cash Conductor cycle.sh step | action_type | Tier |
    18	|---|---|---|---|---|---|
    19	| `open_banking_truelayer_oauth` | `refreshTokens(config, current, fetchFn?, now?)` | OAuth refresh against TrueLayer; concurrent-safe per (provider, connection_id); **refuses if consent is in PSD2 blocking window (≤7 days)** | Step 1 (auth refresh) | `open_banking_truelayer` | green |
    20	| `open_banking_list_transactions` | `listTransactionsSince(client, config, options)` | List bank transactions on/after a timestamp; TrueLayer raw payload preserved in `raw_provider_payload` for v1.1+ provider-agnostic upgrade | Step 3 (transaction ingest) | n/a (read-only) | n/a |
    21	| `open_banking_get_account_balance` | `getAccountBalance(client, config)` | Current account balance (available + cleared) | Step 10 (cash-flow forecast) | n/a (read-only) | n/a |
    22	
    23	`open_banking_truelayer` action_type exists in `agents/_shared/autosend-policy.yaml` at green tier. `open_banking_plaid_uk` ALSO exists in the policy (registered 2026-06-01 in commit f414492) — kept registered to avoid churn when v1.1+ work re-promotes Plaid; not currently consumed.
    24	
    ...
    29	| Function | Purpose |
    30	|---|---|
    31	| `OpenBankingClient` (class) | Transport — constructed once by cycle.sh Step 1; provider-aware base URL (TrueLayer prod/sandbox; Plaid UK v1.1+) |
    32	| `loadTokens(config)` / `saveTokens(config, t)` | Token-file I/O — includes the load-bearing PSD2 `consent_expires_at_ms` field; surfaced for test setup + operator consent-bootstrap |
    ...
    34	| `getTokenAgeStage(tokens, now?)` | Pure predicate — returns `{stage: 'fresh'|'info'|'warn'|'blocking', days_until_consent_expiry}`; staged-severity hook for operator alerting via `ESC_OPEN_BANKING_TOKEN_AGING` |
    35	| `rateCheck(key, now?)` | Returns `RateState` — exposes soft-backoff signal at 80% (24/min) of the conservative 30/min ceiling; consumer responsible for honouring it |
    36	| `rateConsume(key, now?)` | Consumes a slot; returns false at the hard 100% gate |
    37	| `resetRateLimit(key?)` | Test/diagnostic reset |
    38	| `_resetInflightForTest()` | Clears in-flight OAuth-refresh dedup map; for tests only |
    39	| `OpenBankingCache` (class) | Disk cache — transactions TTL 5min; balance TTL 0 (always fresh) |
    40	
    41	---
    42	
    43	## PSD2 consent lifecycle (the load-bearing distinction)
    44	
    45	UK Open Banking is regulated by PSD2 + the OBIE standards. The KEY constraint vs other OAuth flows:
    46	
    47	> **PSD2 requires the user to re-authenticate to their bank every 90 days**, regardless of whether the refresh_token would otherwise still be valid.
    48	
    49	This means the connector tracks TWO expiry timestamps per token bundle:
    50	
    51	1. **`expires_at_ms`** — access_token expiry (typical ~1 hour). Routine refresh via `refreshTokens()` rotates this transparently.
    52	2. **`consent_expires_at_ms`** — PSD2 consent expiry (~90 days from the user's last successful re-authentication). When this hits, the refresh_token itself becomes invalid; the user MUST re-do the PSD2 consent dance.
    53	
    54	`getTokenAgeStage()` is the pure classifier the agent uses to alert the operator BEFORE consent dies. Stages, with the cycle.sh / agent.md §6 mapping to `ESC_OPEN_BANKING_TOKEN_AGING`:
    55	
    56	| Days until consent expiry | Stage | Agent behaviour |
    57	|---|---|---|
    58	| > 30 | `fresh` | (no alert) |
    59	| 15-30 | `info` | log + monitor (operator-aware; nothing breaks) |
    60	| 8-14 | `warn` | escalate to operator chat (`ESC_OPEN_BANKING_TOKEN_AGING` severity warn) |
    61	| 0-7 | `blocking` | **HARD STOP** — `refreshTokens()` refuses to issue a new access token; operator MUST re-do PSD2 consent dance with the user before Cash Conductor can resume |
    62	| < 0 | `blocking` (already expired) | same |
    63	
    64	The blocking-window refusal is enforced server-side by the bank too (refresh_token will 401 once consent expires), but the client-side check fires sooner and with better error messaging (`OpenBankingConsentExpiredError` with explicit guidance).
    65	
    66	`tests/token-aging.test.ts` exhaustively tests the day 0-100 boundary classification per goal-w4-day-26 Phase 2 Step 7 requirement.
    67	
    68	---
    69	
    70	## Quick start
    ...
    72	```typescript
    73	import {
    74	  OpenBankingClient,
    75	  listTransactionsSince,
    76	  getAccountBalance,
    77	  getTokenAgeStage,
    78	  loadTokens,
    79	} from "@ifos/open-banking";
    80	
    81	const config = {
    82	  provider: "truelayer" as const,
    83	  client_id: process.env.TRUELAYER_CLIENT_ID!,
    84	  client_secret: process.env.TRUELAYER_CLIENT_SECRET!,
    85	  connection_id: "<truelayer-account-id>",
    86	  environment: "sandbox" as const,
    87	  token_file_path: `${process.env.HOME}/.ifos-local-vault/<ifos-tenant>/ob-tokens-<account-id>.json`,
    88	};
    89	
    90	// Operator-side: check consent age first
    91	const tokens = await loadTokens(config);
    92	if (tokens) {
    93	  const age = getTokenAgeStage(tokens);
    94	  if (age.stage === "blocking") {
    95	    throw new Error("PSD2 consent expired — re-do consent dance before continuing");
    96	  }
    97	}
    98	
    99	const client = new OpenBankingClient({ config });
   100	const transactions = await listTransactionsSince(client, config, {
   101	  since: "2026-05-01T00:00:00Z",
   102	});
   103	const balance = await getAccountBalance(client, config);
   104	```
   105	
   106	---
   107	
   108	## OAuth bootstrap (one-time per bank connection)
   109	
   110	The connector handles the **refresh** half. The **initial consent dance** (user → bank login → consent → authorisation code → first token pair) is a one-time human-in-the-loop flow:
   111	
   112	1. Register the IFOS app at https://console.truelayer.com/ (TrueLayer) → get `client_id` + `client_secret`.
   113	2. Construct the authorise URL: scope=`accounts transactions balance offline_access`, response_type=code, redirect_uri=your callback, provider_id=`uk-ob-<bank>`.
   114	3. User → bank login → consent screen → bank redirects to your `redirect_uri?code=<auth_code>&scope=...`.
   115	4. POST to `https://auth.truelayer-sandbox.com/connect/token` (or `auth.truelayer.com` for prod) with `grant_type=authorization_code` + the code → receive `{access_token, refresh_token, expires_in}`.
   116	5. Call `GET https://api.truelayer-sandbox.com/data/v1/accounts` with the access_token → get the `account_id`(s) for this connection.
   117	6. Compute `consent_expires_at_ms = Date.now() + 90 * 24 * 60 * 60 * 1000` (PSD2 default). When the user re-consents in the future, recompute.
   118	7. Write the token bundle to `token_file_path` as JSON (mode 0600) — include both `expires_at_ms` AND `consent_expires_at_ms`. The connector handles all subsequent rotations automatically.
   119	
   120	For Plaid UK (v1.1+): pattern is similar but uses `POST /link/token/create` then exchange of public_token → access_token. Interface is in place; implementation is deferred — `NotImplementedError` thrown today.
   121	
   122	---
   123	
   124	## Rate limits
   125	
   126	Conservative per-provider buckets per `src/rate-limit.ts`. Upstream published-limits pages:
   127	
   128	- **TrueLayer:** https://docs.truelayer.com/docs/data-api-rate-limits — production prod-API limits are tier-dependent and not exposed as a single public number; sandbox is documented at 30/min/connection. We use the sandbox figure as the conservative ceiling for production too — it can be tuned upward when Cash Conductor cycle.sh telemetry shows actual usage patterns.
   129	- **Plaid UK:** https://plaid.com/docs/errors/rate-limit-exceeded/ — Plaid Standard tier publishes a 600/min per-Item ceiling; we use 30/min as the conservative starting point until v1.1+ pilots show real load.
   130	
   131	| Provider | Local hard cap (this connector) | Local soft (80%) | Upstream documented |
   132	|---|---|---|---|
   133	| TrueLayer | 30 calls/min per (provider, connection_id) | 24/min | sandbox 30/min; prod tier-dependent |
   134	| Plaid UK | 30 calls/min per (provider, item_id) | 24/min | 600/min per-Item (Standard tier) |
   135	
   136	**Hard gate at 100%** (`consume()` returns false → `OpenBankingRateLimitError`). **Soft signal at 80%** (24/min) is read-only and exposed via `rateCheck()` — `RateState.shouldBackoff === true` with `reason: "minute-soft"`. The consuming agent layer (Cash Conductor cycle.sh) is responsible for honouring the soft signal; the connector does not silently throttle.
   137	
   138	State is in-process and per-(provider, connection_id) — a multi-bank-account runtime that holds many `OpenBankingClient` instances in one process still gets correct isolation.
   139	
   140	**ESC contract on bucket exhaustion** (consumer-emitted via `agents/_shared/hook-helpers.sh`):
   141	
   142	| Failure | Surfaces as | ESC code (escalation-codes.md) | Payload contract |
   143	|---|---|---|---|
   144	| Local hard-gate (100%) reached | `OpenBankingRateLimitError` thrown by `consume()`/client | `ESC_RATE_LIMIT_HIT` (warn; operator) | `{upstream: "truelayer" | "plaid_uk", retry_after_seconds: null, consecutive_429s: 0}` |
   145	| Upstream 429 from provider API | `OpenBankingRateLimitError` thrown with `retry_after_seconds` from `Retry-After` header | `ESC_RATE_LIMIT_HIT` | `{upstream: "truelayer" | "plaid_uk", retry_after_seconds: <N>, consecutive_429s: <N>}` |
   146	
   147	---
   148	
   149	## Retry policy
   150	
   151	| Capability | Method | Max retries | Backoff | On exhaustion |
   152	|---|---|---|---|---|
   153	| `listTransactionsSince` / `getAccountBalance` | GET | 2 | Exponential w/ jitter (250-1000ms) | `OpenBankingError` / `OpenBankingRateLimitError` → `ESC_PROVIDER_FETCH_FAIL` or `ESC_RATE_LIMIT_HIT` (consumer-emitted) |
   154	| `refreshTokens` | POST | **0** | n/a | `OpenBankingAuthError` → `ESC_OPEN_BANKING_AUTH` (**blocking**; consumer-emitted; routes operator + ifos_oncall per escalation-codes.md lines 288-294; payload `failure_type: 'refresh_failed'`) |
   155	| `refreshTokens` blocked by PSD2 consent expiry | POST | **0** | n/a | `OpenBankingConsentExpiredError` → `ESC_OPEN_BANKING_AUTH` (blocking; payload `failure_type: 'consent_expired_90d'`); user must re-do SCA per Bootstrap § |
   156	| 401 from any GET | — | force-refresh access_token, retry once | — | `OpenBankingAuthError` → `ESC_OPEN_BANKING_AUTH` |
   157	| 429 from any GET | — | honour `Retry-After` header | jittered backoff if no header | `OpenBankingRateLimitError` → `ESC_RATE_LIMIT_HIT` |
   158	
   159	The only state-changing op is OAuth refresh (which has its own consent-aware refusal logic via `getTokenAgeStage`). All transaction/balance calls are read-only — banks don't generally expose write APIs in the scope Cash Conductor needs. The connector does NOT write `decision_log` rows (vault/Postgres split per ADR-002); the consuming `cycle.sh` catches the typed errors above and emits the right ESC via `hh_decision_action`/`hh_decision_output` from `agents/_shared/hook-helpers.sh`.
   160	
   161	The separate **PSD2 token-aging signal** — `ESC_OPEN_BANKING_TOKEN_AGING` (staged info → warn → blocking per consent-expiry distance) — is emitted by the consumer based on `getTokenAgeStage()` BEFORE refresh fails. That's a different code path from the refresh-failure mapping above; see `agents/_shared/escalation-codes.md` lines 296-307 for the staged severity definition and `agents/recruitment/cash-conductor/agent.md` §6 for the consumer flow.
   162	
   163	---
   164	
   165	## Error hierarchy
   166	
   167	```
   168	OpenBankingError                  // base
   169	├── OpenBankingAuthError          // OAuth refresh fail (4xx on token endpoint)
   170	│   └── OpenBankingConsentExpiredError  // PSD2 90-day consent dead; re-consent required
   171	├── OpenBankingRateLimitError     // 429 OR local bucket exhausted
   172	├── OpenBankingNotFoundError      // 404
   173	└── NotImplementedError           // Plaid UK during v1.0 (interface defined; impl deferred)
   174	```
   175	
   176	Errors NEVER include credential values; only key names, status codes, safe metadata.
   177	
   178	---
   179	
   180	## Tests
   181	
   182	```bash
   183	# Unit + fixture tests (fast; no network)
   184	pnpm test
   185	```
   186	
   187	**Fixture-first** per `review-mcp-connector.md` §6. The unit suite uses shape-pinned JSON fixtures under `fixtures/`.
   188	
   189	**Live tests are deferred** to the first TrueLayer dev signup — no `MCP_LIVE_TESTS`-gated `describe.skipIf(!LIVE)` block exists yet (honest-signal — review-mcp-connector §10 "Pre-build connector with `MCP_LIVE_TESTS` not yet wired: acceptable IF README marks the live tests as 'wired at first commercial signup'"). The live-test scaffold will land in the same commit as the first sandbox credentials per the W4 Track-1 /goal §1 commercial-gate; the fixture-first suite below is fully sufficient for the W4 ratification pass.
   190	
   191	Test counts:
   192	- `tests/scaffold.test.ts`: 5 (public surface, exports, full error hierarchy with NotImplementedError)
   193	- `tests/rate-limit.test.ts`: 5 (initial state, soft 24, hard 30, per-(provider, connection) isolation)
   194	- `tests/token-aging.test.ts`: 6 (property-based day 0-100 boundary: fresh, info, warn, blocking, negative/already-expired, days_until consistency)
   195	- `tests/auth.test.ts`: 8 (load missing, round-trip, shouldRefresh, refresh success + consent_expires preserved, 401 + no-token-leak, **blocking-consent refuses refresh**, concurrent dedup, Plaid-UK NotImplementedError)
   196	- `tests/capabilities.test.ts`: 7 (TrueLayer transactions + balance from fixtures; Plaid UK NotImplementedError for both; **listTransactionsSince 429 retry-exhaust, getAccountBalance 500 retry-exhaust, 401-forces-refresh-then-retry** — all 3 added per Codex F-R2 #2 + #3)
   197	
   198	**Total: 31 vitest** (target was ≥15 per `review-mcp-connector.md` §6 + the W4 Track-1 /goal §1).
   199	
   200	---
   201	
   202	## Build
   203	
   204	```bash
   205	pnpm build       # tsup → dist/index.{js,d.ts}
   206	pnpm typecheck   # tsc --noEmit
   207	```
   208	
   209	ESM-only; node 20+; target ES2022. Same toolchain as other MCP connectors.
   210	
   211	---
   212	
   213	## Where this fits in the IFOS architecture
   214	
   215	```
   216	agents/recruitment/cash-conductor/cycle.sh
   217	   │
   218	   ├── Step 1  (auth refresh + consent age check)  ──┐
   219	   ├── Step 3  (bank transaction ingest)            ──┼─→ @ifos/open-banking (this package)
   220	   └── Step 10 (weekly cash-flow forecast balance)  ──┘    │
   221	                                                          ↓
   222	                                                     TrueLayer or Plaid UK
   223	                                                     (provider chosen at config time)
   224	                                                          ↓
   225	                                                     OAuth bearer (PSD2-consented)
   226	```
   227	
   228	Cash Conductor's full bundle (cycle.sh + validate.sh + context.sh + cleanup.sh + tools.yaml + fixtures) lands later in W4-7 per `agents/recruitment/cash-conductor/agent.md` §8. This connector is one of the substrate dependencies (alongside the accounting connector chosen per tenant).
   229	
   230	---
   231	
   232	## Boundary checks
   233	
   234	Per `review-mcp-connector.md` §8:
   235	- ✓ No Composio / AgentMail references
   236	- ✓ No imports from `packages/harness/cortextos/` (submodule boundary)
   237	- ✓ No direct writes to `decision_log` (vault/Postgres split — cycle.sh handles persistence via `_shared/hook-helpers.sh`)
   238	- ✓ No hardcoded tenant slugs in `src/` (test fixtures only)
   239	
   240	---

exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/src/rate-limit.ts | sed -n '1,120p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
exec
/bin/zsh -lc "nl -ba agents/recruitment/cash-conductor/tools.yaml | sed -n '90,130p'; nl -ba agents/_shared/autosend-policy.yaml | sed -n '160,182p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    90	    cycle_step: 6
   ...
   120	
   121	  - id: open_banking_get_account_balance
   122	    package: "@ifos/open-banking"
   123	    purpose: "Current account balance (available + cleared) for weekly cash-flow forecast §10"
   124	    cycle_step: 13
   125	    state_changing: false
   126	
   127	  # ────────────────────────────────────────────────────────────────────────
   128	  # Autosend bridge (D1-B Telegram shim per 2026-05-31 founder decision;
   129	  # package scaffold landed 2026-06-01). Split into two rows because the two
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/open-banking/src/index.ts | sed -n '1,140p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	// @ifos/open-banking — public API
     2	//
     3	// v1.0 SCOPE: TrueLayer only. Plaid UK exists as an internal stub in src/ for
     4	// v1.1+ work but is NOT a v1.0 bus-routed capability — per Codex cluster F R3
     5	// closure 2026-06-02 (docs/decisions/2026-06-02-codex-cluster-f-r3-justification.md),
     6	// declaring a v1.1+ stub as a current capability violated review-mcp-connector
     7	// single-upstream-provider intent + had no success fixtures to back the surface.
     8	// The Plaid stub stays in src/ so v1.1+ work has scaffolding to graduate from;
     9	// it just isn't surfaced as a v1.0 capability.
    10	//
    11	// The exports below split into TWO groups per review-mcp-connector §1 +
    12	// README §"Capabilities":
    13	//   (1) BUS-ROUTED CAPABILITIES — each maps 1:1 to a tools.yaml capability ID
    14	//       on agents/recruitment/cash-conductor/tools.yaml AND (for state-changing
    15	//       capabilities) has an action_type entry in agents/_shared/autosend-policy.yaml.
    16	//   (2) INTERNAL HELPERS — exposed for consumer convenience + testing, but NOT
    17	//       declared as bus capabilities (no action_type; no authz check).
    18	//
    19	// PSD2 90-day consent tracking is THE load-bearing distinction from Xero / QB:
    20	// banks legally require user re-authentication every 90 days regardless of
    21	// refresh-token TTL. See auth.ts getTokenAgeStage + README §"PSD2 consent lifecycle".
    22	
    23	// ─────────────────────────────────────────────────────────────────────────
    24	// (1) Bus-routed capabilities (set-equal with cash-conductor/tools.yaml §open-banking)
    25	//     v1.0 = TrueLayer only.
    26	// ─────────────────────────────────────────────────────────────────────────
    27	
    28	// open_banking_truelayer_oauth (action_type: open_banking_truelayer, green tier).
    29	// The function dispatches internally by config.provider; calling with
    30	// provider='plaid_uk' throws NotImplementedError (v1.1+ stub kept in src/auth.ts).
    31	export { refreshTokens } from "./auth.js";
    32	// open_banking_list_transactions (read-only)
    33	export { listTransactionsSince } from "./transactions.js";
    34	// open_banking_get_account_balance (read-only)
    35	export { getAccountBalance } from "./balance.js";
    36	
    37	// ─────────────────────────────────────────────────────────────────────────
    38	// (2) Internal helpers (NOT bus-routed; surfaced for consumers + tests)
    39	// ─────────────────────────────────────────────────────────────────────────
    40	
    41	// Transport class — constructed once by cycle.sh Step 1; provider-aware base URL
    42	export {
    43	  OpenBankingClient,
    44	  TRUELAYER_API_PROD,
    45	  TRUELAYER_API_SANDBOX,
    46	  DEFAULT_TIMEOUT_MS,
    47	} from "./client.js";
    48	// Token-file I/O + pure predicates (shouldRefresh for access-token age;
    49	// getTokenAgeStage for PSD2 consent-expiry staged alerting)
    50	export {
    51	  loadTokens,
    52	  saveTokens,
    53	  shouldRefresh,
    54	  getTokenAgeStage,
    55	} from "./auth.js";
    56	// Disk cache (transactions TTL 5min; balance TTL 0)
    57	export { OpenBankingCache } from "./cache.js";
    58	// Rate-limit introspection — soft signal exposed via rateCheck() (read-only;
    59	// consumer responsible for honouring shouldBackoff at the 80% soft threshold;
    60	// hard 100% gate is enforced inside rateConsume()).
    61	export {
    62	  check as rateCheck,
    63	  consume as rateConsume,
    64	  reset as resetRateLimit,
    65	} from "./rate-limit.js";
    66	// Test/diagnostic
    67	export { _resetInflightForTest } from "./auth.js";
    68	// Error hierarchy (OpenBankingConsentExpiredError + NotImplementedError are
    69	// load-bearing for the consumer's branching logic)
    70	export {
    71	  OpenBankingError,
    72	  OpenBankingAuthError,
    73	  OpenBankingConsentExpiredError,
    74	  OpenBankingRateLimitError,
    75	  OpenBankingNotFoundError,
    76	  NotImplementedError,
    77	} from "./errors.js";
    78	export type {
    79	  OpenBankingProvider,
    80	  OpenBankingTokens,
    81	  OpenBankingConfig,
    82	  OpenBankingTransaction,
    83	  OpenBankingBalance,
    84	  TokenAgeStage,
    85	  TokenAgeReport,
    86	  OpenBankingClientOptions,
    87	} from "./types.js";
    88	export type { RateState } from "./rate-limit.js";
    89	
    90	export const VERSION = "0.1.0";

codex
One substantive inconsistency is emerging around the Open Banking provider scope and local rate-limit budget. I’m doing a final pass for build/test failures and credential-leak patterns before deciding the verdict.
exec
/bin/zsh -lc "rg -n \"console\\.(log|error|warn).*\\"'$'"\\{.*(KEY|SECRET|PASSWORD|TOKEN|BEARER)|throw\\s+new\\s+Error.*\\"'$'"\\{.*(KEY|SECRET|PASSWORD)|JSON\\.stringify\\(envWithSecrets\\)|composio|agentmail|packages/harness/cortextos|decision_log|migration-test|test-tenant-b\" packages/mcp-connectors/open-banking -S" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
packages/mcp-connectors/open-banking/README.md:159:The only state-changing op is OAuth refresh (which has its own consent-aware refusal logic via `getTokenAgeStage`). All transaction/balance calls are read-only — banks don't generally expose write APIs in the scope Cash Conductor needs. The connector does NOT write `decision_log` rows (vault/Postgres split per ADR-002); the consuming `cycle.sh` catches the typed errors above and emits the right ESC via `hh_decision_action`/`hh_decision_output` from `agents/_shared/hook-helpers.sh`.
packages/mcp-connectors/open-banking/README.md:236:- ✓ No imports from `packages/harness/cortextos/` (submodule boundary)
packages/mcp-connectors/open-banking/README.md:237:- ✓ No direct writes to `decision_log` (vault/Postgres split — cycle.sh handles persistence via `_shared/hook-helpers.sh`)

exec
/bin/zsh -lc 'pnpm typecheck' in /Users/madsadmin/code/CortexOS/packages/mcp-connectors/open-banking
exec
/bin/zsh -lc 'pnpm test' in /Users/madsadmin/code/CortexOS/packages/mcp-connectors/open-banking
 succeeded in 1630ms:
...
> vitest run
 ...
 ✓ tests/auth.test.ts (8 tests) 60ms
 ✓ tests/capabilities.test.ts (7 tests) 565ms
   ✓ open-banking capabilities — TrueLayer (v1.0) > getAccountBalance: persistent 500 surfaces as OpenBankingError after retries 402ms

 Test Files  5 passed (5)
      Tests  31 passed (31)
   Start at  12:15:15
   Duration  1.72s (transform 227ms, setup 0ms, collect 424ms, tests 634ms, environment 1ms, prepare 800ms)


exec
/bin/zsh -lc "nl -ba agents/recruitment/cash-conductor/cycle.sh | sed -n '88,130p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
1. The connector violates the single-upstream-provider contract. `packages/mcp-connectors/open-banking/README.md:3` defines a provider abstraction over TrueLayer plus Plaid UK, and `src/index.ts:3-9` keeps a Plaid stub for future v1.1+ work. `review-mcp-connector` defines an MCP connector as wrapping a single upstream provider, and top-level §3 rejects speculative scaffolding without a current consumer. Split Plaid UK into a separate future connector or remove all Plaid code/types/tests from this package.

2. The rate-limit budget is internally contradictory and may exceed the stated upstream cap. `README.md:128-134` claims TrueLayer sandbox is documented at 30/min and sets a 30/min local hard cap, but `src/rate-limit.ts:4-6` says sandbox is often constrained to ~10/min while still using a 30/min cap. This fails `review-mcp-connector` §3 and honest-signal checks because the safety margin is not verifiably conservative and could be >100% of the upstream cap. Verify the published TrueLayer limit, cite one canonical URL, and set `MINUTE_HARD`/README to an aligned sub-cap.

3. The README overstates current decision-log/ESC integration by describing future consumer behavior as present. `README.md:159` says the consuming `cycle.sh` catches typed errors and emits ESC rows via `hh_decision_action`/`hh_decision_output`, but `agents/recruitment/cash-conductor/cycle.sh:94-101` and `120-128` show W7-8 TODO stubs, not actual connector calls or typed-error catches. This fails top-level Rule 4 and Rule 5. Reword the README to mark this as proposed/wired-later, or land the cycle.sh integration in the same change.
tokens used
64,490
REJECTED
