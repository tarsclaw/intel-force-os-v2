...
session id: 019e87e8-4bd1-7151-89a1-9b0723f615e0
--------
user
...
codex
I’ll verify the connector against the live repo surface: exports, tools.yaml references, autosend policy, and boundary grep. Then I’ll return only the required ratification verdict.
exec
/bin/zsh -lc 'rg -n "composio|agentmail|packages/harness/cortextos|decision_log|migration-test|test-tenant-b" packages/mcp-connectors/quickbooks -S -i' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
packages/mcp-connectors/quickbooks/README.md:216:- ✓ No Composio / AgentMail references
packages/mcp-connectors/quickbooks/README.md:217:- ✓ No imports from `packages/harness/cortextos/` (submodule boundary)
packages/mcp-connectors/quickbooks/README.md:218:- ✓ No direct writes to `decision_log` (vault/Postgres split — cycle.sh handles persistence via `_shared/hook-helpers.sh`)

exec
/bin/zsh -lc 'rg -n "quickbooks|quickbooks_oauth|accounting_reconciliation_write|ESC_RATE_LIMIT_HIT|ESC_ACCOUNTING|ESC_PROVIDER_FETCH_FAIL|ESC_ACCOUNTING_WRITE_FAIL" agents packages/mcp-connectors/quickbooks -S' in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
...
packages/mcp-connectors/quickbooks/README.md:199:   ├── Step 5 (reconciliation pass)     ──┼─→ @ifos/quickbooks (this package)
packages/mcp-connectors/quickbooks/README.md:204:                                         https://quickbooks.api.intuit.com/v3/company/<realmId>
...
exec
/bin/zsh -lc 'rg -n "function |export |export class|export async function|export const|export interface" packages/mcp-connectors/quickbooks/src/index.ts packages/mcp-connectors/quickbooks/src/*.ts' in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "rg -n \"console\\.(log|error|warn).*\\"'$'"\\{.*(KEY|SECRET|PASSWORD|TOKEN|BEARER)|throw\\s+new\\s+Error.*\\"'$'"\\{.*(KEY|SECRET|PASSWORD)\" packages/mcp-connectors/quickbooks -S" in /Users/madsadmin/code/CortexOS
 exited 1 in 0ms:
 succeeded in 0ms:
packages/mcp-connectors/quickbooks/src/payments.ts:16:export interface ListPaymentsOptions {
...
packages/mcp-connectors/quickbooks/src/rate-limit.ts:16:export interface RateState {
packages/mcp-connectors/quickbooks/src/rate-limit.ts:24:function pruneMinute(arr: number[] | undefined, now: number): number[] {
...
packages/mcp-connectors/quickbooks/src/index.ts:65:export type {
packages/mcp-connectors/quickbooks/src/index.ts:74:export type { RateState } from "./rate-limit.js";
packages/mcp-connectors/quickbooks/src/index.ts:76:export const VERSION = "0.1.0";
packages/mcp-connectors/quickbooks/src/types.ts:9:export interface QbTokens {
packages/mcp-connectors/quickbooks/src/types.ts:22:export interface QbOAuthConfig {
packages/mcp-connectors/quickbooks/src/types.ts:33:export interface QbInvoice {
packages/mcp-connectors/quickbooks/src/types.ts:50:export interface QbQueryResponse<TEntity extends string, TBody> {
packages/mcp-connectors/quickbooks/src/types.ts:59:export interface QbPayment {
packages/mcp-connectors/quickbooks/src/types.ts:76:export interface QbPaymentWriteRequest {
packages/mcp-connectors/quickbooks/src/types.ts:88:export interface QbClientOptions {
packages/mcp-connectors/quickbooks/src/client.ts:18:export const QB_BASE_URL_PRODUCTION = "https://quickbooks.api.intuit.com";
packages/mcp-connectors/quickbooks/src/client.ts:19:export const QB_BASE_URL_SANDBOX = "https://sandbox-quickbooks.api.intuit.com";
packages/mcp-connectors/quickbooks/src/client.ts:20:export const DEFAULT_TIMEOUT_MS = 15_000;
packages/mcp-connectors/quickbooks/src/client.ts:31:function sleep(ms: number): Promise<void> {
packages/mcp-connectors/quickbooks/src/client.ts:35:function backoff(attempt: number): number {
...
packages/mcp-connectors/quickbooks/src/invoices.ts:15:export interface ListOpenInvoicesOptions {
...
codex
The initial grep shows the connector is not purely pre-build: Cash Conductor already references QuickBooks capabilities. I’m checking the exact tools.yaml and policy tiers now because set-equality and action_type matching are hard gates for this skill.
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/quickbooks/README.md | sed -n '1,230p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   ...
   163	pnpm test
   164	
   165	# Live API tests (requires real QB sandbox credentials; off by default)
   166	MCP_LIVE_TESTS=true pnpm test
   167	```
   168	
   169	**Fixture-first** per `review-mcp-connector.md` §6. The unit suite uses shape-pinned JSON fixtures under `fixtures/`; live tests are gated on `MCP_LIVE_TESTS=true`.
   170	
   171	Test counts (W4 Day-26 scaffold):
   172	- `tests/scaffold.test.ts`: 5 (public surface, exports, error hierarchy)
   173	- `tests/rate-limit.test.ts`: 5 (initial state, soft 400, hard 500, per-realm isolation, etc.)
   174	- `tests/auth.test.ts`: 7 (load missing, round-trip, shouldRefresh, refreshTokenNearExpiry, refresh success, 401 + no-token-leak, concurrent dedup)
   175	- `tests/capabilities.test.ts`: 6 (list/get invoice happy + 404, list/write payment happy + 400)
   176	
   177	**Total: 23 vitest** (target was ≥15 per `review-mcp-connector.md` §6 + the W4 Track-1 /goal §1).
   178	
   179	---
   180	
   181	## Build
   182	
   183	```bash
   184	pnpm build       # tsup → dist/index.{js,d.ts}
   185	pnpm typecheck   # tsc --noEmit
   186	```
   187	
   188	ESM-only; node 20+; target ES2022. Same toolchain as @ifos/companies-house + @ifos/xero.
   189	
   190	---
   191	
   192	## Where this fits in the IFOS architecture
   193	
   194	```
   195	agents/recruitment/cash-conductor/cycle.sh
   196	   │
   197	   ├── Step 1 (auth refresh)            ──┐
   198	   ├── Step 4 (invoice register ingest) ──┤
   199	   ├── Step 5 (reconciliation pass)     ──┼─→ @ifos/quickbooks (this package)
   200	   ├── Step 6 (reconciliation write)    ──┤    │
   201	   └── Step 9 (chase-draft validation)  ──┘    │
   202	                                              ↓
   203	                                         QuickBooks Online v3 REST API
   204	                                         https://quickbooks.api.intuit.com/v3/company/<realmId>
   205	                                              ↓
   206	                                         OAuth bearer (no tenant header — realm in URL)
   207	```
   208	
   209	Cash Conductor's full bundle (cycle.sh + validate.sh + context.sh + cleanup.sh + tools.yaml + fixtures) lands later in W4-7 per `agents/recruitment/cash-conductor/agent.md` §8. This connector is one of three accounting substrate options consumed (Xero / QuickBooks / Sage — Sage deferred per Cash Conductor §9 Q8).
   210	
   211	---
   212	
   213	## Boundary checks
   214	
   215	Per `review-mcp-connector.md` §8:
   216	- ✓ No Composio / AgentMail references
   217	- ✓ No imports from `packages/harness/cortextos/` (submodule boundary)
   218	- ✓ No direct writes to `decision_log` (vault/Postgres split — cycle.sh handles persistence via `_shared/hook-helpers.sh`)
   219	- ✓ No hardcoded tenant slugs in `src/` (test fixtures only — `fixture-realm`, `test-realm-rate-limit`)
   220	
   221	---
   222	
   223	*v0.1.0 — scaffold landed 2026-06-01 (W4 Day-26).*

exec
/bin/zsh -lc "nl -ba agents/recruitment/cash-conductor/tools.yaml | sed -n '50,95p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    50	    cycle_step: 5
...
exec
/bin/zsh -lc "nl -ba agents/_shared/autosend-policy.yaml | sed -n '155,170p'; nl -ba agents/_shared/autosend-policy.yaml | sed -n '235,248p'" in /Users/madsadmin/code/CortexOS
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/quickbooks/src/client.ts | sed -n '1,210p'; nl -ba packages/mcp-connectors/quickbooks/src/payments.ts | sed -n '1,90p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
   155	  # ───────────────────────────────────────────────────────────
   156	
   157	  xero_oauth:
 ...
 succeeded in 0ms:
   ...
   139	      }
   140	
   141	      // 401: token bad — force a refresh next iteration
   142	      if (res.status === 401 && attempt < max_retries) {
   143	        this.current_tokens = null;
   144	        await sleep(backoff(attempt));
   145	        continue;
   146	      }
   147	
   148	      // 429: rate-limited; honour Retry-After then retry (GET only)
   149	      if (res.status === 429 && attempt < max_retries) {
   150	        const retryAfter = parseInt(res.headers.get("Retry-After") ?? "0", 10);
   151	        const waitMs = retryAfter > 0 ? retryAfter * 1000 : backoff(attempt);
   152	        await sleep(waitMs);
   153	        continue;
   154	      }
   155	      if (res.status === 429) {
   156	        const retryAfter = parseInt(res.headers.get("Retry-After") ?? "0", 10);
   157	        throw new QbRateLimitError(
   158	          `QuickBooks returned 429 after ${attempt + 1} attempt(s)`,
   159	          retryAfter > 0 ? retryAfter : null,
   160	        );
   161	      }
   162	
   163	      // 5xx: retry with backoff (GET only)
   164	      if (res.status >= 500 && attempt < max_retries) {
   165	        await sleep(backoff(attempt));
   166	        continue;
   167	      }
   168	
   169	      // 4xx (non-401/429): typed error, no retry
   170	      const safeBody = await res.text().catch(() => "");
   171	      if (res.status === 404) {
   172	        throw new QbNotFoundError(`QuickBooks 404 on ${method} ${path}`);
   173	      }
   174	      if (res.status === 400) {
   175	        throw new QbValidationError(
   176	          `QuickBooks rejected ${method} ${path} (HTTP 400)`,
   177	          safeBody.length > 0 && safeBody.length < 2000 ? [safeBody] : [],
   178	        );
   179	      }
   180	      throw new QbError(
   181	        `QuickBooks ${method} ${path} failed (HTTP ${res.status})`,
   182	        res.status,
   183	      );
   184	    }
   185	    throw new QbError(
   186	      `QuickBooks ${method} ${path} exhausted retries (${last_error ?? "unknown"})`,
   187	    );
   188	  }
   189	}
     1	// QuickBooks Payments capability surface — read + write (write is the only
     ...
     5	
     6	import type { QbClient } from "./client.js";
     7	import { QbCache } from "./cache.js";
     8	import type {
     9	  QbPayment,
    10	  QbPaymentWriteRequest,
    11	  QbQueryResponse,
    12	} from "./types.js";
    13	
...
codex
The cross-file policy checks line up, but I found a possible behavior/documentation mismatch around 401 handling. I’m running the local test/typecheck commands to see whether the artifact is otherwise mechanically sound before I decide whether that single mismatch is ratification-blocking.
exec
...
> vitest run
 ...
 ✓ tests/scaffold.test.ts (5 tests) 5ms
 ✓ tests/auth.test.ts (7 tests) 74ms
 ✓ tests/capabilities.test.ts (8 tests) 695ms
   ✓ quickbooks capabilities — payments > listPayments: persistent 500 surfaces as QbError after retries 402ms

 Test Files  4 passed (4)
      Tests  25 passed (25)
   Start at  11:37:34
   Duration  1.74s (transform 299ms, setup 0ms, collect 520ms, tests 797ms, environment 2ms, prepare 445ms)


codex
The commands pass, so this is not a compile/test failure. The remaining concerns are ratification issues: stale or false contract statements and a 401 retry path that does not implement the policy it documents.
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/quickbooks/package.json | sed -n '1,15p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
     1	{
...
exec
/bin/zsh -lc "nl -ba packages/mcp-connectors/quickbooks/tests/capabilities.test.ts | sed -n '75,175p'" in /Users/madsadmin/code/CortexOS
 succeeded in 0ms:
    75	afterEach(async () => {
    ...
    78	});
    79	
    80	describe("quickbooks capabilities — invoices", () => {
    81	  it("listOpenInvoices: returns parsed array from fixture", async () => {
    82	    const fakeFetch: typeof fetch = async () => makeOkResponse(INVOICES_PAGE_1);
    83	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
    84	    const invoices = await listOpenInvoices(client, { cache, no_cache: true });
    85	    expect(invoices.length).toBe(2);
    86	    expect(invoices[0]?.Id).toBe("1001");
    87	    expect(invoices[0]?.Balance).toBe(1500);
    88	    expect(invoices[1]?.CustomerRef.name).toBe("Beta Search Partners");
    89	  });
    90	
    91	  it("getInvoice: returns single invoice when present (single-entity endpoint shape)", async () => {
    92	    const fakeFetch: typeof fetch = async () =>
    93	      makeOkResponse({ Invoice: INVOICES_PAGE_1.QueryResponse.Invoice[0] });
    94	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
    95	    const inv = await getInvoice(client, "1001", { cache, no_cache: true });
    96	    expect(inv).not.toBeNull();
    97	    expect(inv?.DocNumber).toBe("INV-1001");
    98	  });
    99	
   100	  it("getInvoice: 404 surfaces as QbNotFoundError", async () => {
   101	    const fakeFetch: typeof fetch = async () => new Response("", { status: 404 });
   102	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
   103	    await expect(
   104	      getInvoice(client, "99999", { cache, no_cache: true }),
   105	    ).rejects.toBeInstanceOf(QbNotFoundError);
   106	  });
   107	
   108	  // Error-path coverage for listOpenInvoices (per review-mcp-connector §6 +
   109	  // Codex F-R1 issue #5: every capability needs ≥1 happy + ≥1 error fixture).
   110	  it("listOpenInvoices: persistent 429 surfaces as QbRateLimitError after retries", async () => {
   111	    let calls = 0;
   112	    const fakeFetch: typeof fetch = async () => {
   113	      calls += 1;
   114	      return new Response("", {
   115	        status: 429,
   116	        headers: { "Retry-After": "0" }, // 0s = no wait; just exhausts retries fast
   117	      });
   118	    };
   119	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
   120	    await expect(
   121	      listOpenInvoices(client, { cache, no_cache: true }),
   122	    ).rejects.toBeInstanceOf(QbRateLimitError);
   123	    expect(calls).toBeGreaterThanOrEqual(2); // initial + ≥1 retry per max_retries=2 default
   124	  });
   125	});
   126	
   127	describe("quickbooks capabilities — payments", () => {
   128	  it("listPayments: returns parsed array from fixture", async () => {
   129	    const fakeFetch: typeof fetch = async () => makeOkResponse(PAYMENTS_RECENT);
   130	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
   131	    const payments = await listPayments(client, { cache, no_cache: true });
   132	    expect(payments.length).toBe(1);
   133	    expect(payments[0]?.TotalAmt).toBe(2000);
   134	    expect(payments[0]?.Line[0]?.LinkedTxn[0]?.TxnId).toBe("1002");
   135	  });
   136	
   137	  it("writePaymentReceived: success path returns created payment", async () => {
   138	    const fakeFetch: typeof fetch = async () => makeOkResponse(PAYMENT_WRITE_OK);
   139	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
   140	    const payload: QbPaymentWriteRequest = {
   141	      CustomerRef: { value: "200" },
   142	      TotalAmt: 1500.0,
   143	      TxnDate: "2026-05-22",
   144	      PaymentRefNum: "BACS-2026-05-22-002",
   145	      Line: [
   146	        {
   147	          Amount: 1500.0,
   148	          LinkedTxn: [{ TxnId: "1001", TxnType: "Invoice" }],
   149	        },
   150	      ],
   151	    };
   152	    const created = await writePaymentReceived(client, payload);
   153	    expect(created.Id).toBe("p-502");
   154	    expect(created.TotalAmt).toBe(1500);
   155	  });
   156	
   157	  it("writePaymentReceived: 400 surfaces as QbValidationError (no retry)", async () => {
   158	    let calls = 0;
   159	    const fakeFetch: typeof fetch = async () => {
   160	      calls += 1;
   161	      return new Response("Invoice Id does not exist", { status: 400 });
   162	    };
   163	    const client = new QbClient({ config: makeConfig(), fetchFn: fakeFetch });
   164	    const payload: QbPaymentWriteRequest = {
   165	      CustomerRef: { value: "0" },
   166	      TotalAmt: 100.0,
   167	      Line: [{ Amount: 100.0, LinkedTxn: [{ TxnId: "0", TxnType: "Invoice" }] }],
   168	    };
   169	    await expect(writePaymentReceived(client, payload)).rejects.toBeInstanceOf(
   170	      QbValidationError,
   171	    );
   172	    expect(calls).toBe(1); // write was NOT retried
   173	  });
   174	
