...
# APPENDIX A — Harvest reconciliation (2026-08-21)

Added during the CortexOS → new-estate extraction. Reconciles this catalogue against every `ESC_*` token
actually present in `agents/`, `packages/` and `scripts/` (`.sh`, `.ts`, `.yaml`, `.json`, `.sql`; `node_modules`
and `dist` excluded). **Do not merge this appendix into §2 without a founder ruling** — §2's update protocol says
codes are catalogued before they are wired, and A.1 records where that was not honoured.

**Result:** 55 catalogued · 52 distinct tokens found in code · 6 genuine drift · 5 grep artefacts · 14 catalogued
but never fired.

## A.1 — Wired but never catalogued (6) — protocol violations

Each is self-documented in-place as pending registration, so this is known deferred work, not a surprise. All six
date from the connector build (W5–W6) and all six are connector-scoped.

| Code | Wired at | In-place note |
|---|---|---|
| `ESC_GRANOLA_PLAN_TIER` | `scribe/tools.yaml:230` | "warn; QUEUED for catalogue registration at W6; consumer degrades to notes-only ingest" |
| `ESC_GRANOLA_AUTH` | `scribe/tools.yaml` | connector auth failure |
| `ESC_WORKOS_AUTH` | `packages/mcp-connectors/workos/src/errors.ts:27` | "pending W5 Phase 4 addition" |
| `ESC_WORKOS_VALIDATION_FAIL` | workos connector | — |
| `ESC_CVLIBRARY_VALIDATION_FAIL` | cv-library connector | — |
| `ESC_REED_VALIDATION_FAIL` | reed connector | — |

**Disposition for the new repo:** these six belong to Phase B (the connectors). Register them in the new
`docs/registers/escalation-codes.md` at the same time the connectors land, not before — registering codes for
connectors that are not yet in the repo would invert the same protocol in the other direction.

## A.2 — Grep artefacts, not codes (5)

Recorded so a future reconciliation does not re-flag them.

| Token | What it actually is |
|---|---|
| `ESC_BLOCKED_COUNT` | shell variable, `scripts/run-live-migration.sh:355` |
| `ESC_PAYLOAD` | shell variable, `diagnostic/validate.sh:284` |
| `ESC_NONEXISTENT_CODE` | deliberate negative-path fixture, `_shared/tests/test-hook-helpers.sh:236` |
| `ESC_AGENT_TOOL_FAILURE` | a "phantom" explicitly **removed** 2026-06-10; surviving mentions are comments recording its absence (`concierge/tools.yaml:159`) |
| `ESC_SOURCING_DNC_FILTER` | a named backlog item in `sourcing-scout/agent.md:331`, never wired |

## A.3 — Catalogued but never fired (14)

Not defects. Two coherent families plus strays, all designed and never wired.

- **Vault cascade (6):** `ESC_VAULT_CASCADE_PARTIAL_FAILURE`, `ESC_VAULT_CASCADE_TIMEOUT`, `ESC_VAULT_CONCURRENCY`,
  `ESC_VAULT_LOCK_TIMEOUT`, `ESC_VAULT_RENAME_RACE`, `ESC_VAULT_VERSION_MISMATCH`
- **cortextOS integration (3):** `ESC_CORTEXTOS_DEGRADED`, `ESC_CORTEXTOS_HANDOFF`, `ESC_CORTEXTOS_RESTART_REQUESTED`
- **Strays (5):** `ESC_APPROVAL_ESCALATED_HOP`, `ESC_AUTOSEND_ORANGE_PENDING`, `ESC_HUMAN_EDITING_LOCK`,
  `ESC_JSL_RED_FLAG`, `ESC_LINKEDIN_AUTH`

**Disposition:** the cortextOS family is dead — the new estate reaches cortextOS through one adapter at the
integration seam, so these three do not travel. The vault family and the strays carry forward as designed-but-unwired;
review each against the new harness before registering, since several (`ESC_APPROVAL_ESCALATED_HOP`,
`ESC_AUTOSEND_ORANGE_PENDING`) describe queue behaviour the new authoriser owns natively.

## A.4 — Build-loop codes, separate namespace

`~/Desktop/Hand-Off/claude-global-setup/repo-scaffold/escalation-codes.md` defines 5 codes governing the *build
process*, not product runtime: `ESC_BDL_ROW_INVALID`, `ESC_SPEC_DRIFT_DETECTED`, `ESC_RULING_ORPHANED`,
`ESC_FOREMAN_PROPOSAL`, `ESC_ESTIMATE_BREACH`. Different domain, no collision with any code above. Keep them as a
distinct section in the new register rather than interleaving.
